...
infraestrutura do {{site.data.keyword.cloud_notm}}. As licenças são pedidas em incrementos de 10 pacotes de licença de máquina virtual (VM), com base no número de VMs que você especificou para licenciar ao pedir o serviço IBM Spectrum Protect Plus on {{site.data.keyword.cloud_notm}}. Se você quiser trazer uma licença existente do IBM Spectrum Proteger Plus, nenhum pedido
...
servidores ESXi.
5. As VMs do IBM Spectrum Protect Plus são implementadas, ativadas e configuradas no cluster padrão da instância.

...
**Importante**: deve-se assegurar que as regras de firewall do FSA sejam definidas para permitir que comunicações HTTPS (Porta TCP 443) de saída que são iniciadas pelos componentes de gerenciamento, como o Zerto Virtual Manager, se comuniquem com o banco de dados de gerenciamento externo na infraestrutura do {{site.data.keyword.cloud_notm}} pela Internet. As comunicações HTTPS de saída (porta TCP 443) se originam do endereço IP público do VMware NSX Edge Services Gateway (ESG) dos serviços de gerenciamento em sua instância.

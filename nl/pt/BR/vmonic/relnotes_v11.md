...
As informações de status detalhadas estão, agora, disponíveis para instâncias do Cloud Foundation. Quando você clica no nome de uma instância, as informações de status que são exibidas fornecem mais detalhes sobre o progresso da implementação. Se ocorrer um erro, serão exibidas mensagens para ajudá-lo com o problema. Para obter mais informações, veja [Visualizando instâncias do Cloud Foundation](/docs/services/vmwaresolutions/sddc?topic=vmware-solutions-sd_viewinginstances).

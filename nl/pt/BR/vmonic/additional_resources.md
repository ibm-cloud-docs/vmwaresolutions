---

copyright:

  years:  2016, 2018

lastupdated: "2018-08-17"

---

# Recursos adicionais

Revise mais recursos que possam ajudá-lo a iniciar mais rapidamente. Esses recursos incluem:
* Informações de referência da arquitetura
* Vídeos
* Demonstrações
* Análise técnica aprofundada
* Tutoriais

## Folhas de dados

* [Planilha de dados de proteção do VMware vCenter Server on {{site.data.keyword.cloud}}](https://www.ibm.com/software/reports/compatibility/clarity-reports/report/html/softwareReqsForProduct?deliverableId=236C87407E7411E6BA51E79BE9476040)
* [Planilha de dados de proteção do VMware Cloud Foundation on {{site.data.keyword.cloud_notm}}](https://www.ibm.com/software/reports/compatibility/clarity-reports/report/html/softwareReqsForProduct?deliverableId=C87A0EC07E7311E6BA51E79BE9476040)

## Extensões da plataforma

Revise a arquitetura de todas as [Extensões
do Virtualization Platform](https://www.ibm.com/cloud/garage/architectures/virtualizationArchitecture/allvirtualizationextensions), tais como:
* FortiGate Security Appliance
* {{site.data.keyword.cloud_notm}}  armazenamento de arquivo e de bloco
* Gateway de Serviços de Edge NSX
* Virtualização segura
* backup e replicação de Veeam
* vRealize Automation
* Recuperação de desastre Zerto

## Vídeos e demonstrações

* [ Recursos e Vídeos ](https://www.ibm.com/cloud/garage/architectures/virtualizationArchitecture/resources)
* [ Compromisso Técnico Digital ](https://ibm-dte.mybluemix.net/ibm-vmware)
* [ Recursos Apresentados e Estudos de Caso ](https://www.ibm.com/cloud/vmware/resources)

## Test drive do IBM Cloud for VMware Solutions

Quer ver por si mesmo? Faça um test drive curto do produto seguindo a demo guiada passo a passo [Experimente o VMware vCenter Server](https://cloudcontent.mybluemix.net/cloud/garage/demo/try-vmware-solutions) do {{site.data.keyword.vmwaresolutions_short}}.

## receitas do developerWorks

* [{{site.data.keyword.vmwaresolutions_short}} - Tenha uma visão detalhada](https://www.ibm.com/developerworks/cloud/library/cl-ibm-cloud-for-vmware-solutions-trs/)
* [Etapas para conectar armazenamento dedicado ao VMware Solutions on {{site.data.keyword.cloud_notm}}](https://developer.ibm.com/recipes/tutorials/steps-to-attach-dedicated-storage-to-existing-ic4v-deployments-on-ibm-cloud/)

## Documentação do VMware

* [Documentação do VMware Hybrid Cloud Extension](https://hcx.vmware.com/#vm-documentation)

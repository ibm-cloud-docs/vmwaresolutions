...
**Nota**: a exclusão de instâncias do vCenter Server with Hybridity Bundle que fazem parte de uma configuração multissite requer planejamento especial. Para obter mais informações, veja [Excluindo instâncias do vCenter Server with Hybridity Bundle em uma configuração multisite](vc_hybrid_deletinginstance_multi.html).

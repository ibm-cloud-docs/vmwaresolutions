...
# Migrando instâncias do VMware pré-V2.5 para contas do IBM Cloud

As instâncias do VMware Federal que são implementadas na V2.5 e liberações mais recentes em sua conta do {{site.data.keyword.cloud}} são incluídas automaticamente em sua conta e são gerenciadas pelo {{site.data.keyword.cloud_notm}} Identity and Access Management (IAM). 
...
2. Quando a migração da instância for concluída, ela será exibida na página **Instâncias implementadas** sob a conta para a qual ela foi migrada. A instância migrada não é mais exibida na conta original da qual foi migrada.

...
# Pedindo, visualizando e excluindo instâncias de Single-node Trial for Data Protection and Disaster Recovery
{: #dr_backup_bundle_orderinginstance}

Revise os requisitos de planejamento antes de pedir uma instância do Single-node Trial for Data Protection and Disaster Recovery.

Essa avaliação é excluída automaticamente depois de 90 dias. Uma contagem regressiva de expiração para a instância do Single-node Trial for Data Protection and Disaster Recovery é exibida na página de detalhes da instância.
{:important}

## Requisitos e planejamento para pedir instâncias do Single-node Trial for Data Protection and Disaster Recovery
...
* Para usar o {{site.data.keyword.vmwaresolutions_short}} para pedir instâncias, deve-se ter uma conta de infraestrutura do {{site.data.keyword.cloud_notm}}. O custo dos componentes que são pedidos em suas instâncias é faturado para aquela conta {{site.data.keyword.cloud_notm}}.
...
## Procedimento para pedir instâncias do Single-node Trial for Data Protection and Disaster Recovery
{: #dr_backup_bundle_orderinginstance-procedure}

1. No catálogo do {{site.data.keyword.cloud_notm}}, clique no ícone **VMware** da área de janela de navegação esquerda e, em seguida, clique no cartão **Single-node Trial for Data Protection and Disaster Recovery** da seção **Data centers virtuais do VMware**.
2. Na página **Single-node Trial for Data Protection and Disaster Recovery**, clique em **Continuar**.
...
### Resultados depois de pedir instâncias do Single-node Trial for Data Protection and Disaster Recovery
...
* Quando a instância for implementada com êxito, os componentes descritos em [Especificações técnicas para instâncias do Single-node Trial for Data Protection and Disaster Recovery](/docs/services/vmwaresolutions/services?topic=vmware-solutions-dr_backup_bundle_overview#dr_backup_bundle_overview-tech-specs) serão instalados.
...
4. Use a chave de ativação no local anotada na etapa 1 para ativar a MV do HCX Enterprise Manager local.
  ...
  Siga as instruções no [Guia do Usuário do VMware HCX](https://docs.vmware.com/en/VMware-HCX/services/user-guide/GUID-BFD7E194-CFE5-4259-B74B-991B26A51758.html){:external}.
...
5. Um certificado SSL autoassinado foi gerado pelo serviço HCX on {{site.data.keyword.cloud_notm}}. Deve-se importar o certificado para o HCX Manager local concluindo as etapas a seguir:
    ...
    4. Clique em **APLICAR**.
6. Continue a configuração inicial e construa a interconexão. Siga as instruções no [Guia do Usuário do VMware HCX](https://docs.vmware.com/en/VMware-HCX/services/user-guide/GUID-BFD7E194-CFE5-4259-B74B-991B26A51758.html){:external}.
7. Amplie as redes no VMware HCX do local para o {{site.data.keyword.cloud_notm}}. Siga as instruções no [Guia do Usuário do VMware HCX](https://docs.vmware.com/en/VMware-HCX/services/user-guide/GUID-BFD7E194-CFE5-4259-B74B-991B26A51758.html){:external}.
8. Migre as MVs entre o local e o {{site.data.keyword.cloud_notm}}. Siga as instruções no [Guia do Usuário do VMware HCX](https://docs.vmware.com/en/VMware-HCX/services/user-guide/GUID-BFD7E194-CFE5-4259-B74B-991B26A51758.html){:external}.
...
## Procedimento para excluir instâncias do Single-node Trial for Data Protection and Disaster Recovery
{: #dr_backup_bundle_orderinginstance-deleting-procedure}

Ao excluir uma instância do Single-node Trial for Disaster Recovery, os componentes a seguir são liberados em sequência:
...
Conclua as etapas a seguir para excluir uma instância do Single-node Trial for Data Protection and Disaster Recovery:
...
* [Abrir um chamado para o I{{site.data.keyword.cloud_notm}} Private](https://www.ibm.com/mysupport/s/?language=en_US){:external}

...
# Ordenando o vCenter Server com instâncias do NSX-T
...
A licença do vSphere Enterprise Plus 6.7u2 é selecionada por padrão.
...
* NSX-T 2.4 (edição Base, Advanced ou Enterprise)
...
Para instâncias implementadas, é possível incluir compartilhamentos de armazenamento NFS em um cluster NFS ou vSAN existente. Para obter mais informações, consulte a seção *Incluindo armazenamento NFS em instâncias do
vCenter Server* em
[Expandindo
...
O vSAN está disponível somente para as configurações de Bare Metal **Skylake**, **Cascade** e **Broadwell**. Especifique as seguintes opções vSAN:
...
7. Conclua as configurações de Bare Metal Server.
    ...
    2. Selecione a configuração do Bare Metal Server e especifique o modelo de CPU e o tamanho da RAM.
...
* Quando a instância for implementada com êxito, os componentes que estão descritos em [Especificações técnicas para instâncias do vCenter Server com NSX-T](/docs/services/vmwaresolutions?topic=vmware-solutions-vc_nsx-t_overview#vc_nsx-t_overview-specs) serão instalados em sua plataforma virtual VMware.
...
Visualize e gerencie a instância do vCenter Server com NSX-T que você pediu. Para obter mais informações sobre como visualizar a instância, consulte [Visualizando instâncias do vCenter Server](/docs/services/vmwaresolutions/vcenter?topic=vmware-solutions-vc_viewinginstances).

...
2. Execute o comando `su -` e insira a senha do usuário `root` (igual à senha do administrador) para mudar para raiz.
...
4. Execute o comando `list` dentro do shell `ccli` para listar os componentes da frota registrados com o gerenciador HCX.
5. Especifique o ID da frota para o `ccli` digitando o ID listado para o componente da frota. Por exemplo,  ` go 8 `.
6. Execute o comando `debug remoteaccess enable` para se conectar ao componente da frota desejado usando SSH.
7. Saia do `ccli` e conecte-se usando SSH para o endereço IP do componente da frota ativado para SSH.

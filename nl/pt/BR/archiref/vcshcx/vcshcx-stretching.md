...
Além disso, como parte do fluxo de trabalho de esticamento de rede, o NSX no lado da nuvem está autorizado a construir uma VXLAN que é, então, conectada a uma interface criada no dispositivo L3 do lado da nuvem especificado (DLR ou ESG deixado em um estado desconectado) e no dispositivo L2C do lado da nuvem.

Normalmente, ao migrar um determinado aplicativo, todas as redes em uso pelas máquinas virtuais (VMs) aplicáveis devem ser esticadas pela instância do {{site.data.keyword.cloud}}.
...
- Para qualquer aplicativo de tipo de cluster que requer UUIDs de disco, tais como os clusters RAC do Oracle. Observe que o vMotion não mudou os UUIDs de disco no destino.
...
- Durante o período de energia desligada, ocorre a replicação final de quaisquer mudanças de dados.
- Ligação da nova MV no lado de destino.
- Renomeação e movimentação da VM original para a pasta em nuvem para a qual ela foi movida.

A seguir estão as vantagens da migração em massa sobre o vMotion:
- Migração de várias VMs simultaneamente.
...
- Permitir migração de VMs que estão atualmente usando recursos de CPU virtual que diferem do lado da nuvem. A migração do vMotion poderia falhar nesses casos.
...
Clusters do Oracle RAC, do MS Exchange e do MS-SQL são exemplos de aplicativos em que duas ou mais MVs participam de um cluster que requer disco compartilhado em todas as MVs ou nós do cluster. A sinalização de vários gravadores do VMware precisa ser ativada em todos os nós da VM para discos que fazem parte do cluster de aplicativo (discos virtuais não do S.O.). As VMs com a sinalização de vários gravadores ativada para qualquer disco virtual não são suportadas.
...
- O cluster incorre em tempo de inatividade antes do início da migração e após a conclusão da migração para remontar a configuração de vários gravadores em VMs do cluster.
...
o cluster.
  ...
  Quando a migração for concluída, use o UUID anotado para mapear as informações do disco, para recriar a ordem de nomenclatura do disco e o ID do SCSI, se necessário. O aplicativo deve funcionar de qualquer maneira. Isso é usado em locais em que uma instância do Oracle tem muitos discos virtuais que são mapeados para resolução de problemas do aplicativo.
4. Remova os discos virtuais de todas as VMs do cluster, exceto a que é considerada primária.
5. Remova a sinalização de vários gravadores da VM do cluster principal que deve ser a única proprietária dos discos do cluster atualmente.
6. Ligue o cluster principal, se necessário, para tempo de inatividade mínimo.
7. Migrar todos os nós do cluster com vMotion. Migre o cluster primário primeiro. Migre todos os outros nós após eles serem desligados.
8. Quando o nó de propriedade do disco primário concluir a migração, desligue-o.
...
12. Mapeie o disco/ative o sinalizador de vários gravadores em todas as outras VMs do cluster e ligue-as.
13. Verifique outra operação de cluster.
...
Após a migração, os volumes iSCSI podem ser espelhados com o S.O. para outra solução de armazenamento em nuvem local e os dados do NFS podem ser replicados para qualquer solução de nuvem. As considerações são:
...
Após o ciclo de vida de migração, teste os aplicativos de desenvolvimento ou preparação antes de tentar a produção. A QoS pode ser empregada para o tráfego do túnel de sobreposição (udp 500 / 4500) entre os dispositivos L2C HCX que suportam redes L2 estendidas sensíveis à latência.

## Network swing
{: #vcshcx-stretching-network-swing}

Se o objetivo for a evacuação do data center para o {{site.data.keyword.cloud_notm}}, a próxima etapa até a última antes da remoção do HCX será a troca de rede. A troca de rede atinge uma migração da sub-rede da rede que hospeda as VMs migradas do data center de origem para uma rede de sobreposição do NSX dentro do {{site.data.keyword.cloud_notm}}.

A troca de rede envolve as etapas a seguir:
- Verifique se a rede foi evacuada de todas as cargas de trabalho e se quaisquer dispositivos de rede não VM foram movidos para outra rede, migrados funcionalmente para a nuvem ou descontinuados.
...
- Execute um fluxo de trabalho de rede HCX não estendido na IU e selecione o dispositivo NSX de roteamento apropriado para assumir o controle do gateway padrão da rede não estendida.
- Execute qualquer mudança de roteamento externo, que pode incluir: inserir o roteamento mudado para redes que foram migradas, remover rotas para o site de origem da rede migrada e assegurar que o roteamento para a sub-rede migrada ao longo da WAN ainda funcione para aplicativos que não foram migrados.
...
Digamos que você deseja trocar a rede de um aplicativo específico que tenha todas as VMs completamente migradas para a nuvem. Por exemplo:
- Você está usando um Vyatta no lado da rede privada para inserir rotas em sua nuvem MPLS e para canalizar para os dispositivos de roteamento de borda no MPLS para que seja possível evitar o espaço de IP do {{site.data.keyword.cloud_notm}}.
- Sua conta é configurada com um VRF do {{site.data.keyword.cloud_notm}}.
- Alguns aplicativos estão atrás de um IP virtual (vIP) balanceado de carga. Esses vIPs estão em sua sub-rede própria que reside em um F5 virtual atrás do vyatta.

Enquanto a inclusão de roteamento mais específico no MPLS para redes que são trocadas para o {{site.data.keyword.cloud_notm}} por meio do HCX funciona bem para outras redes, ela não funciona para vIPs individuais, pois uma rota de /32 está sendo incluída.

Solução: é comum que os provedores WAN filtrem as rotas de /32 que são incluídas. Trabalhe com o fornecedor da WAN para permitir.
...
- Os administradores do VMware, os administradores de rede (incluindo clientes e fornecedores de WAN) e os proprietários de aplicativos precisam estar envolvidos, mesmo que não haja impacto planejado em um sistema ou equipamento de rede específico.

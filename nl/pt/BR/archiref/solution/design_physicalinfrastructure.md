...
<dl class="dl">
  ...
  <dd class="dd">A rede física fornece a conectividade de rede para o ambiente que é usado, então, pela virtualização de rede. A rede é fornecida pela rede de serviços do {{site.data.keyword.cloud_notm}} e inclui serviços adicionais, como DNS e NTP.</dd>
</dl>
...
O host físico se refere ao {{site.data.keyword.baremetal_short}} no ambiente que entrega recursos de cálculo. O {{site.data.keyword.baremetal_short}} aplicado nesta solução é certificado pelo VMware e listado no [VMware HCG](http://www.vmware.com/resources/compatibility/search.php).
...
O host físico emprega dois discos conectados localmente para serem alocados para o hypervisor do vSphere ESXi. É possível alocar discos adicionais usando o vSAN conforme descrito na seção _Design de armazenamento físico_ nesta página ou usando o NetApp ONTAP conforme descrito em [Arquitetura do NetApp ONTAP Select](https://www.ibm.com/cloud/garage/files/IBM_Cloud_for_VMware_Solutions_NetApp_Architecture.pdf). Cada
host físico tem conexões de rede redundantes de 10 Gbps para acesso à rede pública e privada.
...
Os {{site.data.keyword.CloudDataCents_notm}} e os Points of Presence (PoPs) de rede têm múltiplas conexões de 1 Gbps ou 10 Gbps para o trânsito de camada superior e as transportadoras de rede de peering.
...
Esse design multicamada permite que a rede seja escalada entre racks, linhas e pods dentro de um {{site.data.keyword.CloudDataCent_notm}}.
...
* Os endereços IP primários são designados a dispositivos, Bare Metal e servidores virtuais provisionados pelo {{site.data.keyword.cloud_notm}}. Você não deve designar nenhum endereço IP nesses blocos.
...
Também é possível configurar a conta do {{site.data.keyword.slportal}} como uma conta do VRF para fornecer funcionalidade semelhante ao VLAN Spanning, permitindo o roteamento automático entre blocos de IP de sub-rede. Todas as contas com conexões do Direct Link devem ser convertidas para uma conta do VRF ou criadas como tal.
...
Além da VLAN privada A, uma segunda VLAN privada (aqui designada VLAN privada B) existe para suportar recursos do VMware, como vSAN e vMotion, e para conectividade com o armazenamento conectado à rede (NAS). Como tal, a VLAN é dividida em duas ou três sub-redes móveis.
...
Todas as sub-redes configuradas como parte de uma implementação automatizada do vCenter Server ou do Cloud Foundation usam os intervalos gerenciados pelo {{site.data.keyword.cloud_notm}}. Isso é para assegurar que qualquer endereço IP possa ser roteado para qualquer data center dentro da conta do {{site.data.keyword.cloud_notm}} quando você precisar da conexão agora ou no futuro.
...
| VLAN | Tipo | Descrição |
|:---- |:---- |:----------- |
| Público| Primário  | Designada a hosts físicos para acesso à rede pública. Não utilizada na implementação inicial. |
...
Quando usado, o VMware vSAN é configurado usando uma configuração totalmente em flash. Esse design permite várias opções de configuração, incluindo chassi 2U e 4U, uma variedade de números de discos e uma variedade de tamanhos de disco. Todas as configurações usam dois grupos de discos vSAN, com um disco de estado sólido (SSD) para cache e um ou mais SSDs para capacidade. Todas as unidades alocadas para consumo de vSAN são configuradas em RAID-0 de disco único.

Para obter informações sobre as configurações suportadas, veja a Lista de Materiais para a [instância do Cloud Foundation](../../sddc/sd_bom.html) ou [instância do vCenter Server](../../vcenter/vc_bom.html).

### Armazenamento de nível de arquivo compartilhado entre hosts

Ao usar o armazenamento de nível de arquivo compartilhado, um compartilhamento NFS de 2 TB é conectado aos hosts que compõem o cluster inicial do VMware. Esse compartilhamento, conhecido como compartilhamento de gerenciamento, é usado para componentes de gerenciamento, como o VMware vCenter Server, o Platform Services Controller e o VMware NSX. O armazenamento é conectado por meio do protocolo NFSv3 e pode suportar até 4000 IOPS.
...
É possível alocar e montar compartilhamentos de arquivo adicionais para suas cargas de trabalho no momento da compra ou posteriormente dentro do console. É possível selecionar entre as opções de capacidade de armazenamento de arquivo do {{site.data.keyword.cloud_notm}} Endurance disponíveis e as camadas de desempenho no {{site.data.keyword.CloudDataCent_notm}} correspondente. Todos os compartilhamentos são anexados usando o protocolo NFSv3. Além disso, é possível anexar compartilhamentos de arquivo NFSv3 ao aplicar a oferta NetApp ONTAP Select.

Os {{site.data.keyword.CloudDataCents_notm}} que oferecem a camada de desempenho de 10 IOPS/GB também incluem criptografia gerenciada por provedor de dados em repouso (criptografia AES-256) e são suportados por armazenamento totalmente em flash. A camada de desempenho de 10 IOPS/GB é limitada a uma capacidade máxima de 4 TB. Para obter mais informações sobre o NAS compartilhado usado nessa solução, veja a [Arquitetura de armazenamento compartilhado](https://www.ibm.com/cloud/garage/files/AttachedStorageSolutionArchitecture_v1.0.pdf).

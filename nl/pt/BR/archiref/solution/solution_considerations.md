...
- Planeje se sua carga de trabalho deve ser implementada em VLANs do {{site.data.keyword.cloud_notm}} com [endereços IP móveis do {{site.data.keyword.cloud_notm}}](/docs/infrastructure/subnets?topic=subnets-getting-started) ou [em comutadores lógicos do NSX (VXLANs) usando seus próprios endereços IP](/docs/services/vmwaresolutions/archiref/nsx?topic=vmware-solutions-nsx_overview). Observe que o uso da rede definida por software (SDN) do NSX oferece a maior flexibilidade para gerenciar e proteger sua rede de carga de trabalho no {{site.data.keyword.cloud_notm}}.

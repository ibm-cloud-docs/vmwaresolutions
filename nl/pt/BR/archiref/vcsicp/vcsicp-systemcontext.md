...
![Diagrama de contexto do sistema - VCS ICP CAM](vcsicp-syscontext-vcs-icp-cam.svg)

A seguir estão os componentes principais:

- **Virtualização no local** - Esse componente é um ambiente VMware hospedado nas instalações do cliente ou em um local de terceiro e hospeda atualmente as VMs que executam os aplicativos a serem modernizados. Ele é o ambiente de origem para migrações de VM e é fracamente acoplado a uma instância do IBM Cloud por meio do VMware Hybridity (HCX).
- **vCenter Server** – o VMware vCenter Server on IBM Cloud (VCS) é uma instância do IBM Cloud for VMware Services que é o destino para VMs migradas do ambiente no local. Junto ao ambiente virtualizado no local, ele forma um ambiente híbrido que permite que as VMs se movam de um site para o outro.
- **IBM Cloud Kubernetes Service** - O IKS usa o Kubernetes como a solução de orquestração do contêiner. A IBM opera e gerencia o nó principal do Kubernetes enquanto os nós do trabalhador são implementados na infraestrutura gerenciada pelo cliente. A IBM fornece ferramentas de gerenciamento para implementação de correção do sistema operacional, upgrades de mecanismo de Docker e novas versões do Kubernetes. O IKS fornece uma plataforma isolada e segura para gerenciar contêineres que são móveis, extensíveis e com capacidade de recuperação automática no caso de ocorrerem failovers.
...
- **IBM Cloud Automation Manager** - CAM é uma plataforma de infraestrutura como código (IaC) pronta para a empresa que fornece uma única área de janela de vidro para provisionar cargas de trabalho baseadas no VMware ao lado de cargas de trabalho baseadas no Kubernetes. A automação de fornecimento de carga de trabalho para máquinas virtuais, contêineres e seus pré-requisitos de infraestrutura é ativada por meio do CAM.
...
Watson | No caso dessa arquitetura de referência e da Acme Skateboards, o Watson representa o serviço de AI usado na arquitetura de "Carro conceito".

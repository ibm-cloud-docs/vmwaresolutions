...
O backup adequado de todos os componentes do NSX será crucial para restaurar o sistema para seu estado de funcionamento se ocorrer falha. Não é suficiente fazer backup das máquinas virtuais (VMs) do NSX. Em vez disso, a função de backup do NSX dentro do NSX Manager deve ser empregada para um backup adequado. Um servidor FTP ou SFTP deve ser especificado para o repositório de dados de backup do NSX.
...
Os backups para uma implementação do {{site.data.keyword.cloud_notm}} Private (ICP) serão cruciais para restaurar o sistema para seu estado de funcionamento se uma falha ocorrer. Além de um backup de VM tradicional, cada nó principal do ICP executa o etcd. A documentação do etcd indica claramente para não usar o backup de VM tradicional para restaurá-lo.

Para ICP no nível de plataforma, é necessário fazer backup destes componentes:
...
-	**Armazenamento persistente** usado para serviços de gerenciamento que usam o Volume Persistente do ICP ou do Kubernetes e o provedor de armazenamento. É possível usar a tecnologia de backup ou restauração que é fornecida pelo provedor de armazenamento. Um processo semelhante pode ser estendido no aplicativo de seu usuário também.
...
####	Expansão do ICP
Os nós de VM do trabalhador do ICP são escalados para expandir o cálculo ou aplicativo:

  - O cliente provisiona uma nova VM na mesma VXLAN em que o ICP é implementado.
  - Os clientes podem escalar os nós do trabalhador, provisionando nova VM e, em seguida, efetuando login no nó de inicialização e executando um comando para trazer os novos nós para o cluster.
  - Use o CAM para automatizar o fornecimento da VM e executando os comandos a serem incluídos no Cluster do ICP.

###  Expansão IKS
Os usuários podem provisionar um ambiente do IKS por meio do {{site.data.keyword.cloud_notm}} Portal para ampliar ou usar um ambiente de contêiner.

As implementações de aplicativos no IKS podem ser feitas por meio de:

- A conexão ou os serviços do IKS são desenvolvidos no CAM e publicados no catálogo do ICP.

...
2.	O DNS é usado para determinar o endereço IP. Esse endereço IP será um endereço público do {{site.data.keyword.cloud}} em uma sub-rede móvel que foi designada à Instância do VCS.
...
6.	Se o aplicativo estiver no mesmo nó do trabalhador, o iptables será usado para determinar qual interface interna será usada para encaminhar a solicitação. Se o aplicativo estiver em um nó do trabalhador diferente, o Calico vRouter será roteado para o nó do trabalhador aplicável, usando o encapsulamento IP em IP. O pacote IP-in-IP será encapsulado em um quadro VXLAN para transporte para o host vSphere ESXi no qual o nó do trabalhador está localizado.
...
O ICP e o CAM por padrão requerem conectividade da Internet para recuperar imagens do Docker, gráficos de Helm, modelos do Terraform e gerenciadores de pacotes do sistema operacional.
Nas liberações mais recentes, agora há suporte para instalações baseadas em proxy para instalações não conectadas diretamente à Internet e há opções para instalar em um modo off-line.

...
## Emparelhamento de site no ambiente do IBM Cloud for VMware Solutions
...
O Emparelhamento de site é registrado e exibido na IU.

## Criando perfis no local do HCX
{: #hcxclient-vcs-mesh-deployment-profiles}

### Criando perfis de rede do Service Mesh no local
...
5. Em **Criar perfil de rede**:
   ...
   2. Forneça intervalos de endereço IP para **IPs externos**, **Comprimento do prefixo de sub-rede externa**, **Gateway externo** e **Detalhes do DNS**.
   3. Configure a MTU como 1500.
   4. Clique em
**Criar**.
6. Repita as etapas anteriores para Gerenciamento e Redes vMotion.
   Nota: configure o valor de MTU como 9000.
...
### Criando perfis de cálculo do Service Mesh no local
...
Um perfil de cálculo para a combinação de cluster e armazenamento é criado e está disponível com a criação do Service Mesh.

## HCX on-premises Service Mesh
{: #hcxclient-vcs-mesh-deployment-servicemesh}

## Criando um Service Mesh no local
...
6. Observe a lista de tarefas para a criação do Service Mesh. Após a conclusão bem-sucedida, o local físico tem três dispositivos HCX e o local da nuvem tem três dispositivos HCX.

### Resultados
{: #hcxclient-vcs-mesh-deployment-servicemesh-results}

Uma HCX Service Mesh é a configuração efetiva de serviços do HCX para um site de origem e de destino. Um Service Mesh pode ser incluído em um Par de sites conectado com um Perfil de cálculo válido criado em ambos os sites.
...
Além disso, como parte do fluxo de trabalho de esticamento de rede, o NSX no lado da nuvem está autorizado a construir uma VXLAN. A VXLAN é, então, conectada a uma interface criada no dispositivo L3 do lado da nuvem especificado (DLR ou ESG deixado em um estado desconectado) e no dispositivo de extensão de rede do lado da nuvem.
...
O adaptador de rede de backup do cliente não está sendo acessado porque isso significaria acessar cada VM para desligar o planejamento de backup do cliente in-guest. Portanto, se uma rede de backup for usada, o backup poderá falhar. Essa é uma situação provisória até que todas as MVs possam ser acessadas após a migração para desativar o cliente de backup dentro do guest.

A largura da banda de uma única extensão de rede é teoricamente de 4 Gbps. No entanto, esse valor pode ser o limite para todas as redes estendidas dentro de um único par de extensões de rede e não pode ser alcançado por uma única rede estendida. Uma única rede estendida pode alcançar ~1 Gbps, pois há largura de banda de subjacência suficiente atribuída e a latência é baixa (< ~10 ms).

### Opção de Roteamento de Proximidade
{: #hcxclient-vcs-mesh-deployment-stretching-prox-routing}

Sem qualquer tipo de otimização de rota, as redes estendidas são roteadas de volta para o lado do cliente para qualquer acesso da L3. Esse efeito "trombone" introduz um padrão de tráfego ineficiente, pois os pacotes precisam viajar de um lado para o outro entre o cliente (origem) e a nuvem. Isso é verdadeiro mesmo para casos em que as VMs de origem e de destino estão na nuvem. O recurso de Roteamento de proximidade do HCX foi projetado para tratar disso e o egresso local de tráfego.

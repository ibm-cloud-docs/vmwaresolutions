...
- **Virtualização no local** - um cluster do VMware que hospeda atualmente as MVs da Acme Skateboards. Essas MVs estão hospedando atualmente os aplicativos que serão modernizados. Esse cluster deve atender aos pré-requisitos documentados na [Arquitetura da solução VMware HCX on {{site.data.keyword.cloud_notm}}](/docs/services/vmwaresolutions/services?topic=vmware-solutions-hcx-archi-intro#hcx-archi-intro) para permitir que os clientes migrem as VMs para a instância do VMware vCenter Server on {{site.data.keyword.cloud_notm}} em execução no {{site.data.keyword.cloud_notm}} e revertam a migração, se necessário.

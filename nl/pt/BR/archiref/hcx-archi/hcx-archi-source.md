...
Revise a arquitetura de cada componente do HCX que está implementado no ambiente de origem.
...
* O HCX cria uma WAN otimizada e definida por software para aumentar o desempenho da rede estendida, permitindo o desempenho que se aproxima da velocidade da LAN.
...
A migração de baixo tempo de inatividade depende do vSphere Replication, que é uma tecnologia distribuída implementada no hypervisor VMware ESX/ESXi. O HCX cria uma réplica de uma máquina virtual em tempo real, move-a para o IBM Cloud e executa uma comutação para desligar a máquina virtual de origem e ligar a máquina virtual migrada.
...
* Segurança - O Cloud Gateway oferece o AES-GCM compatível com o Conjunto B com IKEv2, transferência AES-NI e controle de admissão baseado em fluxo. O HCX possui os processos de criptografia e decriptografia de origem e de destino, garantindo uma segurança consistente e fornecendo admissão para fluxos de trabalho híbridos, como migração da máquina virtual e extensão de rede.
...
O HCX suporta um relacionamento muitos-para-muitos entre os vCenters no local e o IBM Cloud. O vCenter Server no Modo Vinculado é suportado.

Durante a instalação, o dispositivo virtual HCX Manager é importado e configurado como um plug-in para o vCenter no local. Esse plug-in é então usado para configurar a implementação do serviço WAN definido por software. A configuração automatizada provisiona cada dispositivo de serviço híbrido como uma máquina virtual no vCenter local e implementa uma máquina virtual correspondente no IBM Cloud.
...
As etapas a seguir são tarefas básicas de instalação:
...
O planejamento de arquitetura inclui as VMs a serem migradas, as redes usadas para o tráfego de máquina virtual e as redes a serem estendidas. 

Os valores máximo e mínimo para os componentes de implementação são:
...
### Número máximo de migrações e extensões de rede
...
A configuração mínima para executar a migração requer somente o HCX Manager e os dispositivos Cloud Gateway. É possível migrar máquinas virtuais sem extensão de rede. Nesse caso, a máquina virtual obtém um novo endereço IP usando o serviço Guest Customization depois de ser migrada.
...
O WAN Optimization pode melhorar a velocidade nas situações descritas. A configuração do Cloud Gateway para usar uma linha de alta velocidade (como um Direct Connect) melhora a velocidade, fornecendo um link de largura de banda mais alta para o tráfego otimizado da WAN.
...
O Roteamento de proximidade é um recurso de rede que pode ser ativado quando o Cloud Gateway é configurado.

O Roteamento de proximidade assegura que o encaminhamento entre máquinas virtuais conectadas a redes estendidas e roteadas, tanto no local quanto na nuvem, seja simétrico. Esse recurso requer que o Roteamento dinâmico seja configurado entre as instalações do cliente e a nuvem.
...
Para evitar o tromboning, o HCX usa o gerenciamento de rota inteligente para escolher rotas apropriadas para o estado da máquina virtual. O Cloud Gateway mantém um inventário de máquinas virtuais na nuvem. Ele também entende o estado da máquina virtual, que pode ser:
...
R1 é o gateway padrão para N1-b. Portanto, o N1-b deve retornar para R1 para rotear tráfego por meio de R2. Para evitar roteamento assimétrico, o HCX injeta rotas do host dentro da sobreposição de NSX da implementação do IBM Cloud VCS. Se a máquina virtual tiver sido recém-criada na nuvem ou tiver sido movida com migração de baixo tempo de inatividade, a rota do host será injetada imediatamente.

Se a máquina virtual tiver sido transferida usando o vMotion, a rota não será injetada até que a máquina virtual seja reiniciada. Aguardar até após a reinicialização assegura que os dispositivos stateful locais continuem a atender à sessão existente até a reinicialização da máquina virtual. Após a reinicialização, as informações de roteamento ficam consistentes tanto no local quanto na nuvem.
...
* A retenção do endereço MAC é necessária pelas razões a seguir:
  ...
  * Ordem da NIC do Linux: no Linux, se o endereço de Controle de Acesso à Mídia for mudado em uma máquina virtual, o número do dispositivo Ethernet da NIC poderá mudar após uma reinicialização. Uma mudança de número do dispositivo pode alterar a ordem de apresentação de NIC dentro do sistema operacional, quebrando aplicativos ou scripts que dependem da ordem de NIC.
  ...
  * A caixa de seleção fica na página **Selecionar rede de destino** durante a operação de migração.

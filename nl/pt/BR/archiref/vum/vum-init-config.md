...
Para localizar a sub-rede móvel privada do cliente:
...
| IP do BCR | bcr ip | Na mesma página em que você selecionou o IP do proxy acima, anote o endereço identificado como Gateway. Esse é o endereço IP do {{site.data.keyword.cloud_notm}} Backend Customer Router e é o gateway para 10.0.0.0/8 e 161.26.0.0/16. Você usará esse endereço abaixo em uma rota estática no servidor proxy para que ele possa acessar o VCSA e o servidor AD/DNS. |
| IP da NAT | customer-nsx-edge public uplink ip | O endereço público do NSX ESG do cliente servirá como o endereço NAT público para o proxy. Esse endereço IP pode ser localizado revisando a guia **Configurações** para **customer-nsx-edge**. |

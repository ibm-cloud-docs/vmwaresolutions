...
![Diagramme général de l'architecture](../../images/vcsiks-aod.svg "Diagramme général de l'architecture")
...
- **Virtualisation sur site** – Cluster VMware qui héberge actuellement les machines virtuelles Acme Skateboards. Ces machines virtuelles hébergent actuellement les applications qui doivent être modernisées. Ce cluster doit respecter les prérequis de l'architecture [VMware HCX on {{site.data.keyword.cloud_notm}} solution architecture](/docs/services/vmwaresolutions?topic=vmware-solutions-hcx-archi-intro#hcx-archi-intro) de manière à pouvoir exécuter HCX.
...
- **{{site.data.keyword.icpfull_notm}}** - {{site.data.keyword.icpfull_notm}} est une plateforme applicative pour le développement et la gestion d'applications conteneurisées. Il s'agit d'un environnement intégré qui inclut l'orchestrateur de conteneurs Kubernetes, un référentiel d'images privé, une console de gestion, ainsi que des infrastructures préfabriquées de surveillance et une interface graphique à partir de laquelle Acme Skateboards peut déployer, gérer, surveiller et mettre à l'échelle ses applications de façon centralisée. L'instance vCenter Server héberge les composants {{site.data.keyword.icpfull_notm}}, les noeuds maître, les noeuds worker, etc. et les exécute en tant que machines virtuelles. {{site.data.keyword.icpfull_notm}} héberge les composants suivants :
  ...
  - Les **applications Acme Skateboards applications** conteneurisées que les clients veulent déployer dans cet environnement.

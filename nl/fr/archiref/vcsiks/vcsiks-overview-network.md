...
Par défaut, IKS configure le cluster avec un accès à un VLAN public et à un VLAN privé. 
...
Figure 1. Mise en réseau de vCenter Server et d'IKS
![Diagramme de la mise en réseau de vCenter Server et d'IKS](vcsiks-networking.svg)

### Intégration IKS et vCenter Server

Actuellement, les scénarios suivants permettent d'intégrer IKS et VMware vCenter Server sur la mise en réseau d'{{site.data.keyword.cloud_notm}} :
- **Routage VRA** - Ce scénario nécessite que les noeuds worker IKS soient déployés sur le même VLAN que l'instance vCenter. Ce scénario permet l'appairage d'une passerelle ESG avec VRA via BGP et active le routage entre le réseau dissocié et le réseau sous-jacent entre vCenter Server et IKS. Une route statique est nécessaire sur les noeuds worker IKS pour chaque réseau VXLAN afin de réacheminer ces demandes vers BCR/VRA. 
- **Réseau privé virtuel strongSwan** – Ce scénario utilise la solution de connectivité IKS vers l'entreprise standard. Un conteneur strongSwan fournit une passerelle VPN pour le cluster qui réachemine les paquets vers les réseaux distants le long d'un tunnel IPSec vers la passerelle éloignée. Cette passerelle éloignée est une passerelle ESG sur l'instance vCenter Server. Sur les passerelles, les routes sont configurées en envoyant toutes les plages d'adresses IP de service et de cluster vers le conteneur StrongSwan et toutes les adresses vCenter Server BYOIP vers la passerelle ESG. Les adresses IP cible pour les passerelles sont l'adresse IP portable privée du service d'équilibrage de charge qui est affecté au conteneur strongSwan et l'adresse IP portable privée de la passerelle ESG.

#### VLAN de mise en réseau IKS
...
#### Niveau Web hébergé dans un conteneur dans IKS vers un groupe de serveurs d'application de base de données hébergé dans une machine virtuelle dans vCenter Server.
...
### Développement IKS
Les utilisateurs peuvent mettre à disposition un environnement IKS via le portail {{site.data.keyword.cloud_notm}} pour étendre ou utiliser un environnement de conteneur. Les déploiements d'application dans IKS peuvent être réalisés via :

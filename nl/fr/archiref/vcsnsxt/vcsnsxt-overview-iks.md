...
IBM Kubernetes Service fournit une plateforme isolée et sécurisée de gestion de conteneurs. Cette plateforme est portable, extensible et se répare spontanément en cas de basculement. 
...
Les concepts qui s'appliquent à IKS sont les suivants :
-	**Service** - Un service est une ressource Kubernetes qui regroupe un ensemble de pods et fournit la connectivité du réseau à ces pods sans exposer l'adresse IP privée réelle de chaque pod. Vous pouvez utiliser un service pour mettre votre application à disposition dans votre cluster ou pour l'Internet public. 
...
Le modèle de réseau Kubernetes stipule simplement trois exigences fondamentales :
...
Pour que l'interface CNI puisse ajouter un conteneur à un réseau, le contexte d'exécution de conteneur doit d'abord créer un nouvel espace de nom de réseau pour le conteneur, puis démarrer un ou plusieurs des plug-in définis. La configuration de réseau est au format JSON et inclut des zones obligatoires telles que le nom et le type, ainsi que des zones spécifiques au type de plug-in. Un plug-in CNI est censé affecter une adresse IP à l'interface et configurer des routes de réseau pertinentes. IKS utilise Calico comme plug-in de réseau pour l'interface CNI. Calico affecte à chaque charge de travail une adresse IP. Dans IKS, Calico utilise la tunnellisation IP-in-IP de sorte que les adresses IP de réseau de conteneur sont masquées à partir du réseau {{site.data.keyword.cloud_notm}}. 

Nous distinguons quatre types de trafic réseau dans IKS, comme illustré dans le diagramme ci-après.
...
- **Mise en réseau entre les pods** – On parle également de communications de pod à pod. Il existe trois types de trafic Est-Ouest :
  - Les pods peuvent communiquer directement avec d'autres pods sur le même sous-réseau. Dans IKS, chaque pod possède une adresse IP qui est affectée à partir d'une plage fournie par IKS. Chaque noeud worker se voit affecter un sous-réseau lors de la mise à disposition. La communication de pod à pod sans proxy, tunnellisation ou NAT se produit alors que les pods se trouvent dans le même sous-réseau et le même hôte. 
  - Les pods peuvent communiquer directement avec d'autres pods sur différents sous-réseaux. L'encapsulation IP-in-IP est configurée automatiquement dans IKS pour encapsuler uniquement les paquets qui transitent par des sous-réseaux. Cette encapsulation masque l'espace d'adresse de réseau de pod à partir du réseau {{site.data.keyword.cloud_notm}}. L'encapsulation utilise l'adresse IP à partir du sous-réseau privé principal d'{{site.data.keyword.cloud_notm}}. 
...
Il existe trois options pour le trafic Nord-Sud dans IKS :
...
Comme le réseau {{site.data.keyword.cloud_notm}} Private transmet uniquement des schémas d'adressage IP {{site.data.keyword.cloud_notm}}, Calico doit utiliser l'encapsulation IP-in-IP du trafic interne aux charges de travail dans IKS pour masquer les adresses IP de réseau de pod. IKS utilise l'encapsulation IP-in-IP en mode sous-réseau. 

### Calico dans IKS

Calico est installé et configuré automatiquement dans IKS. Des règles par défaut sont créées pour protéger le cluster Kubernetes, avec la possibilité de créer vos propres règles pour protéger des services spécifiques. L'encapsulation IP-in-IP est configurée automatiquement pour encapsuler uniquement les paquets qui transitent par des sous-réseaux et elle utilise NAT pour les connexions sortantes à partir de vos conteneurs. Le trafic Charge de travail-vers-WAN est également activé automatiquement dans {{site.data.keyword.cloud_notm}} Kubernetes Service, par conséquent, aucune configuration supplémentaire de Calico n'est requise. 
...
Par défaut, IKS configure le cluster avec un accès à un VLAN public et à un VLAN privé, avec ce qui suit :

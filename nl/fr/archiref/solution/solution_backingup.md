...
Vous êtes chargé de configurer, gérer et surveiller tous les composants logiciels, y compris la sauvegarde et la disponibilité de votre infrastructure de gestion et de vos charges de travail. 
...
1. Commandez un sous-réseau portable privé depuis l'infrastructure {{site.data.keyword.cloud_notm}} située sur le même réseau local virtuel que vos composants système. Il s'agit du réseau local virtuel privé sur lequel résident les adresses IP de gestion de vos hôtes. 
...
Après avoir déployé votre instance et déployé le service de sauvegarde IBM Spectrum Protect Plus ou Veeam, vous devez configurer une tâche de sauvegarde pour vos machines virtuelles de gestion. Prévoyez de sauvegarder les machines virtuelles suivantes avec au moins 7 jours de sauvegardes quotidiennes :
...
Si vous choisissez de déployer votre serveur AD/DNS en tant qu'instance de serveur virtuel {{site.data.keyword.cloud_notm}}, vous ne pouvez pas le sauvegarder à l'aide de Veeam ou IBM Spectrum Protect Plus. Dans ce cas, vous devez utiliser votre solution de sauvegarde Windows préférée pour les opérations de sauvegarde et de restauration ou vous devez prévoir de déployer votre instance à l'aide de machines virtuelles AD/DNS dans votre cluster VMware. Cette instance peut être sauvegardée par Veeam ou IBM Spectrum Protect Plus. 

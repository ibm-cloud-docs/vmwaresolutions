...
La conception vous permet d'accéder aux hôtes virtuels via DCUI (Direct Console User Interface), ESXi Shell et SSH (Secure Shell). 
...
| Synchronisation d'horloge   | Utilisation d'{{site.data.keyword.cloud}} NTP Server |
...
* Après le déploiement, vous pouvez effectuer une mise à l'échelle pour l'instance jusqu'à 32 hôtes ESXi au maximum.
...
* déployant de nouvelles instances vCenter Server ou Cloud Foundation avec leur propre dispositif vCenter Server Appliance.

Pour plus d'informations sur les clusters, voir le [ document {{site.data.keyword.cloud_notm}} running VMware Clusters Solution Architecture](https://www.ibm.com/cloud/garage/files/IBM-Cloud-for-VMware-Solutions-Multicluster-Architecture.pdf).
...
La configuration RAID 6 requiert au moins 6 hôtes. Le **dédoublonnage** et la **compression** sont également activés dans la règle de stockage par défaut. 
...
Les paramètres vSAN sont définis selon les meilleures pratiques relatives au déploiement de solutions VMware dans {{site.data.keyword.cloud_notm}}. Cela inclut les paramètres SIOC, le groupe de ports des paramètres de basculement explicite et les paramètres de cache-disque. 
...
La virtualisation de réseau fournit un réseau dissocié qui existe dans la couche virtuelle. Cela offre à l'architecture des fonctions telles que la mise à disposition, le déploiement, la reconfiguration et la destruction rapides de réseaux virtuels à la demande. Cette conception utilise le commutateur VDS (vSphere Distributed Switch) et VMware NSX for vSphere pour implémenter la mise en réseau virtuelle. 
...
| Réseau         | Privé A portable conçu pour les composants de gestion |
...
| VLAN2 | Privé A   | ESXi Management, Management, VXLAN (VTEP) |
...
|  Nom de commutateur<br>vSphere Distributed | Fonction | Contrôle E-S<br>réseau| Mode <br>d'équilibrage de charge |Ports <br>NIC physiques| MTU |
|:------------- |:------------- |:------------- |:------------- |:------------- |:------------- |
| SDDC-Dswitch-Private | ESXi Management, Virtual SAN, vSphere vMotion, VXLAN Tunnel Endpoint, NFS (VTEP) | Activé | Route basée sur le basculement explicite (vSAN, vMotion), le port virtuel d'origine (tout le reste) | 2 | 9 000<br>(trames Jumbo) |
...
**Remarque :** les noms, le nombre et la commande des NIC hôte peuvent varier en fonction de l'{{site.data.keyword.CloudDataCent_notm}} et du matériel hôte choisi. 
...
\* **Remarque ** : le groupe de ports vSAN utilise le basculement explicite avec les liaisons actives/en veille car il ne prend ps en charge l'équilibrage de charge du trafic de stockage vSAN. 

...
* HCX s'intègre à vSphere vCenter et est géré depuis le client Web vSphere.
...
Le routage de proximité garantit que l'acheminement entre les machines virtuelles qui sont connectées aux réseaux étendus et routés, à la fois en local et dans le cloud, est symétrique.

### Sécurité

La passerelle cloud offre un déchargement AES-GCM avec IKEv2, compatible Suite B, AES-NI et un contrôle d'admission basé sur les flux.
...
2. Depuis le client Web vSphere, installer le dispositif virtuel de service HCX Manager sur le site vCenter local qui se connecte à IBM Cloud.
3. Depuis le client Web vSphere, enregistrer un noeud final IBM Cloud avec le plug-in HCX. L'enregistrement établit la relation un-à-un entre l'instance HCX locale et l'instance HCX dans IBM Cloud.
...
### Passerelle cloud HCX

La passerelle cloud HCX établit et gère un canal sécurisé entre vSphere et IBM Cloud.
...
Dans le diagramme, les composants N*a sur la gauche résident dans le centre de données local et les composants N*b
sur la droite résident dans le cloud.
...
* [Installation et configuration sur la source](/docs/services/vmwaresolutions/archiref/hcx-archi/hcx-archi-install-cfg-src.html)

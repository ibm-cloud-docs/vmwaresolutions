...
La plateforme vRNI fournit des analyses, une interface utilisateur et une gestion des données et se connecte au dispositif de contrôle qui collecte les données des différentes sources telles que NSX Edges, vCenter, et autres. Tous les composants vRNI utilisent des adresses IP portables privées {{site.data.keyword.cloud}}. vRLI est configuré comme serveur syslog pour vRNI.

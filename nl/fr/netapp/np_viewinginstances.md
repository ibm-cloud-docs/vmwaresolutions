...
| Statut | Statut de l'instance. Ce statut peut prendre l'une des valeurs suivantes :<br>Création en cours : l'instance est en cours de création.<br>Génération en cours ; l'instance est en cours de configuration.<br>Prêt à l'emploi : l'instance est prête à l'emploi.<br>Modification en cours : l'instance est en cours de modification.<br>Echec : le processus de création, de configuration ou de modification a échoué.<br>Suppression en cours : l'instance est en cours de suppression.<br>Erreur de suppression : une erreur s'est produite au cours de la suppression de l'instance.<br>Supprimé : l'instance est supprimée. |
{: caption="Tableau 1. Eléments d'une instance NetApp ONTAP Select" caption-side="top"}
...
{: caption="Tableau 2. Propriétés d'instance NetApp ONTAP Select" caption-side="top"}
...
{: caption="Tableau 3. Informations d'accès relatives aux composants des instances NetApp ONTAP Select" caption-side="top"}
...
{: caption="Tableau 4. Historique de déploiement d'une instance NetApp ONTAP Select" caption-side="top"}
...
| Stockage effectif | Capacité disque totale des serveurs ESXi dans le cluster. |
| Mémoire | Taille de mémoire totale des serveurs ESXi du cluster. |
| Emplacement de centre de données | Centre de données dans lequel le cluster est hébergé. Il s'agit du même centre de données que pour l'instance. |
| Pod | Pod dans lequel le cluster est créé. |
| Statut | Statut du cluster. Ce statut peut prendre l'une des valeurs suivantes :<br>Initialisation en cours : le cluster est en cours de création et de configuration.<br>Modification en cours : le cluster est en cours de modification.<br>Prêt à l'emploi : le cluster est prêt à l'emploi. |
| Actions | Cliquez sur l'icône **Supprimer** pour supprimer le cluster. |
{: caption="Tableau 5. Eléments des clusters NetApp ONTAP Select" caption-side="top"}
...
{: caption="Tableau 6. Détails des serveurs ESXi d'un cluster NetApp ONTAP Select" caption-side="top"}
...
* [Liaison d'un stockage dédié à des déploiements NetApp ONTAP Select](https://developer.ibm.com/recipes/tutorials/steps-to-attach-dedicated-storage-to-existing-ic4v-deployments-on-ibm-cloud/){: external}

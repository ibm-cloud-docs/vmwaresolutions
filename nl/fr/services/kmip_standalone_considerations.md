...
* KMIP for VMware on {{site.data.keyword.cloud_notm}} utilise le service IBM Key Protect for {{site.data.keyword.cloud_notm}} pour créer, chiffrer et déchiffrer les clés de chiffrement. Par conséquent, avant d'installer KMIP for VMware on {{site.data.keyword.cloud_notm}}, vérifiez les points suivants :
   * Vous avez commandé un service Key Protect utilisable dans la région {{site.data.keyword.cloud_notm}} où votre instance KMIP for VMware on {{site.data.keyword.cloud_notm}} doit être hébergée. Pour plus d'informations, voir [Provisioning the service](/docs/services/key-protect/provision.html).
   * Un ID de service {{site.data.keyword.cloud_notm}} a été créé en suivant les étapes décrites dans [Création d'un ID de service](/docs/iam/serviceid.html). Cet ID de service permet d'accéder à l'instance de service Key Protect que vous avez créée.
...
* Assurez-vous que votre compte d'infrastructure {{site.data.keyword.cloud_notm}} est activé pour le service Routage et transfert virtuel (VRF) et pour la connectivité aux noeuds finaux de service. Pour plus d'informations, voir :
   * [Aperçu du service Routage et transfert virtuel (VRF) on IBM Cloud](/docs/infrastructure/direct-link/vrf-on-ibm-cloud.html)
   * [Activation de votre compte pour l'utilisation des noeuds finaux de service à l'aide de l'interface CLI IBM Cloud](/docs/services/service-endpoint/enable-servicepoint.html#getting-started)
...
Pour plus d'informations, voir [KMIP for VMware on IBM Cloud solution architecture](/docs/services/vmwaresolutions/archiref/kmip/overview.html).

...
Chaque paire de dispositifs HTCC est déployée sur le même sous-réseau portable privé que celui spécifié pour les machines virtuelles de gestion, par exemple, NSX Manager, vCenter Server Appliances et Platform Services Controller. La paire de dispositifs agit comme un proxy pour les hôtes vSphere, le dispositif vCenter Server et le gestionnaire NSX Manager d'une instance. Par conséquent, les utilisateurs accèdent aux hôtes vSphere, à vCenter Server Appliance et à NSX Manager via une adresse IP publiée (PIP) affectée par l'administrateur ou lieu de l'adresse IP réelle (RIP) affectée par IBM Cloud.
...
Avant de retirer le service HyTrust DataControl on {{site.data.keyword.cloud_notm}}, prenez soin de désactiver la **la mise en lieu sûr par mot de passe root**, si elle est configurée, et de supprimer tous les hôtes protégés sur DataControl. 

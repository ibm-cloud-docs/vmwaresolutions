...
Le service IBM Spectrum Protect Plus on {{site.data.keyword.cloud_notm}} est préconfiguré avec une tâche de sauvegarde de gestion qui s'exécute automatiquement. Cette tâche s'exécute tous les jours pour sauvegarder les composants de gestion suivants avec jusqu'à sept points de restauration :
...
En cas d'incidents sur les composants de gestion, vous pouvez ouvrir un ticket en [contactant le support IBM](../vmonic/trbl_support.html) afin de demander une restauration des composants de gestion sur une sauvegarde précédente. 

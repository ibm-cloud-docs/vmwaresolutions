...
# ID utente IBM e messaggi della cronologia di distribuzione per le istanze vCenter Server
{: #ibm_use}

{{site.data.keyword.cloud}} richiede l'accesso al tuo account per scopi di automazione e di registrazione delle operazioni. 
...
Gli ID utente NSX Manager vengono utilizzati per aggiungere le regole ESG per il servizio. 
...
Gli ID utente host ESXi vengono utilizzati per aggiungere l'archiviazione, per applicare le patch, per configurare i cluster vSAN e per eseguire tutto il codice di convalida del server. 

Tabella 3. ID utente host ESXi

| Utente     | ID utente    | Descrizione |
|:---------|:-------------|:------------|
| IBM      | ic4vroot     | Utilizzato nella post-distribuzione per aggiungere altra archiviazione NFS e per configurare le rotte per tale archiviazione. |
| Cliente | root         | Creato solo per l'utilizzo da parte del cliente. |

### ID utente Microsoft Active Directory
{: #ibm_use-user-ids-ad}

Gli ID utente Active Directory vengono utilizzati per configurare le voci DNS. 
...
### Messaggi della cronologia di VSI (Virtual Server Instance)
{: #ibm_use-msgs-vsi}
...
### Messaggi della cronologia del servizio
...
### Messaggi della cronologia di Cloud Foundation
{: #ibm_use-msgs-cf}

{{site.data.keyword.vmwaresolutions_short}} emette i seguenti messaggi per le istanze Cloud Foundation:

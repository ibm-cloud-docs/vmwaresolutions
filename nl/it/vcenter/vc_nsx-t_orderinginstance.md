...
# Ordine di istanze vCenter Server with NSX-T
{: #vc_nsx-t_orderinginstance}

Per distribuire una piattaforma virtualizzata VMware flessibile e personalizzabile che soddisfi al meglio le tue esigenze del carico di lavoro, ordina un'istanza VMware vCenter Server with NSX-T.

## Requisiti per vCenter Server with NSX-T
...
La licenza vSphere Enterprise Plus 6.7u2 è selezionata per impostazione predefinita.
...
* NSX-T 2.4 (edizione Base, Advanced o Enterprise)
...
Per le istanze distribuite, puoi aggiungere delle condivisioni di archiviazione NFS a un cluster NFS o vSAN esistente. Per ulteriori informazioni, vedi la sezione *Aggiunta dell'archiviazione NFS alle istanze vCenter Server* in [Espansione e contrazione della capacità per le istanze vCenter Server](/docs/services/vmwaresolutions/vcenter?topic=vmware-solutions-vc_addingremovingservers#adding-nfs-storage-to-vcenter-server-instances).
{:note}

### Archiviazione vSAN
{: #vc_nsx-t_orderinginstance-vsan-storage}

vSAN è disponibile solo per le configurazioni Bare Metal **Skylake**, **Cascade** e **Broadwell**. Specifica le seguenti opzioni vSAN:
...
## Procedura per ordinare le istanze vCenter Server with NSX-T
...
9. Completa le impostazioni dell'interfaccia di rete.
   1. Immetti il prefisso del nome host per l'istanza di cui viene eseguito il provisioning, l'etichetta del dominio secondario e il nome del dominio root. Per un'istanza secondaria, il nome di dominio viene completato automaticamente.
...
## Risultati dopo l'ordine di istanze vCenter Server with NSX-T
...
* Una volta che l'istanza è stata distribuita correttamente, i componenti descritti in [Specifiche tecniche per le istanze vCenter Server with NSX-T](/docs/services/vmwaresolutions?topic=vmware-solutions-vc_nsx-t_overview#vc_nsx-t_overview-specs) vengono installati sulla tua piattaforma virtuale VMware.
...
Visualizza e gestisci l'istanza vCenter Server with NSX-T che hai ordinato. Per ulteriori informazioni sulla visualizzazione dell'istanza, vedi [Visualizzazione delle istanze vCenter Server](/docs/services/vmwaresolutions/vcenter?topic=vmware-solutions-vc_viewinginstances).
...
* [Eliminazione di istanze vCenter Server with NSX-T](/docs/services/vmwaresolutions/vcenter?topic=vmware-solutions-vc_nsx-t_deletinginstance)

...
5. Un certificato SSL autofirmato è stato generato dal servizio HCX on {{site.data.keyword.cloud_notm}}. Devi importare il certificato nell'HCX Manager in loco completando le seguenti istruzioni:
    ...
    3. Fai clic su **URL** e immetti quindi l'URL del certificato che vuoi applicare. Questo è l'**IP cloud HCX** (``https://<cloud-side public IP>``) che puoi trovare nella pagina dei dettagli del servizio HCX on {{site.data.keyword.cloud_notm}} nella console {{site.data.keyword.vmwaresolutions_short}}.

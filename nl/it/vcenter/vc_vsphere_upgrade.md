...
2. Esegui il backup del tuo PSC.
...
1. Dall'interfaccia utente vCenter, passa al nome del cluster che contiene gli host ESXi di cui eseguire l'upgrade.
2. Fai clic su **remediate**.
3. Seleziona **Upgrade Baselines** sul lato sinistro della casella di selezione e quindi seleziona la baseline **VMware ESXi 6.7.0 U1** sul lato destro della schermata di selezione. Fai clic su **next**. Nota: è stata creata una baseline VUM per ESXi 6.7.0 U1 quando è stata caricata l'immagine ISO nel repository VUM.
4. Seleziona gli host di cui vuoi eseguire l'upgrade e fai clic su **next**.
5. Seleziona l'opzione per accettare l'EULA e fai clic su **next**.
6. Dalla schermata delle opzioni avanzate, fai clic su **next** per accettare i valori predefiniti. 
7. Dalla schermata delle impostazioni di correzione dell'host, seleziona, facoltativamente, di disabilitare il supporto rimovibile e fai clic su **next**.
8. Dalla schermata delle opzioni avanzate di correzione del cluster, fai clic su **next** per accettare i valori predefiniti. 
9. Dalla schermata Ready to complete, fai clic su **Pre-check Remediation**. Viene restituita un'avvertenza relativa al controllo di ammissione HA.   
10. Fai clic su **finish** per avviare gli upgrade dell'host. 
...
1. Dall'interfaccia utente vCenter, seleziona il cluster che contiene gli host a cui deve essere applicata la patch e seleziona la scheda **Update Manager**.
2. Seleziona **remediate** e seleziona **Non-Critical Host Patches**.
3. Procedi come è stato fatto in precedenza con l'upgrade di ESXi. Tieni presente che potresti dover arrestare di nuovo le VM Zerto ZVRA come parte di questo processo. 
...
Una volta completato il tuo upgrade, aggiorna il tuo ticket di supporto con il supporto IBM. Il supporto IBM aggiorna quindi la console {{site.data.keyword.vmwaresolutions_short}} per riprendere l'automazione {{site.data.keyword.vmwaresolutions_short}} a un livello 6.7. Ciò include l'aggiunta e la rimozione, i servizi, gli host, il cluster e le istanze vCenter Server secondarie. 

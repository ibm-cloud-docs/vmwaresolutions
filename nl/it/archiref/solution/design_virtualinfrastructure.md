...
Il cluster vSphere ospita le macchine virtuali (VM) che gestiscono il cloud centrale così come le risorse di calcolo per i carichi di lavoro dell'utente.

Per le istanze Cloud Foundation:
* Un'istanza contiene 4 host ESXi nella distribuzione iniziale.
* Dopo la distribuzione, puoi ridimensionare l'istanza fino a un massimo di 32 host ESXi.
...
* Distribuzione di cluster aggiuntivi gestiti dallo stesso vCenter Server Appliance
...
Le funzioni vSAN disponibili dipendono dall'edizione della licenza che selezioni quando ordini l'istanza. Per ulteriori informazioni, vedi [Confronto delle edizioni di VMware vSAN](appendix.html#vmware-vsan-edition-comparison).
...
Per ulteriori informazioni sulle connessioni NIC fisiche, vedi la Figura 2. Connessioni NIC all'host fisico in [Progettazione dell'infrastruttura fisica](design_physicalinfrastructure.html).
...
Le impostazioni vSAN sono impostate in base alle procedure ottimali per la distribuzione di soluzioni VMware all'interno di {{site.data.keyword.cloud_notm}}. Ciò include le impostazioni SIOC, le impostazioni di failover esplicito per il gruppo di porte e le impostazioni della cache del disco.
...
Il cluster vSphere utilizza due switch distribuiti vSphere (VDS) configurati come nelle seguenti tabelle.

Tabella 4. Switch distribuiti del cluster convergente

| Nome VDS (vSphere Distributed<br> Switch) | Funzione | Controllo <br>I/O di rete | Modalità di bilanciamento <br> del carico | Porte NIC <br>fisiche | MTU |
|:------------- |:------------- |:------------- |:------------- |:------------- |:------------- |
| SDDC-Dswitch-Private | Gestione ESXi, SAN virtuale, vSphere vMotion, VXLAN Tunnel Endpoint, NFS (VTEP) | Abilitato | Rotta basata sulla porta virtuale di origine con failover esplicito (vSAN, vMotion) (principale) | 2 | 9.000<br>(Frame Jumbo) |
...
**Nota:** i nomi, il numero e l'ordine delle NIC host possono variare in base al {{site.data.keyword.CloudDataCent_notm}} e alla selezione dell'hardware host.
...
\* **Nota**: il gruppo di porte vSAN utilizza il failover esplicito con attivo/standby poiché non supporta il bilanciamento del carico del traffico di archiviazione vSAN.

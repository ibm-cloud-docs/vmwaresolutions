...
Questa sezione fornisce ulteriori dettagli sull'architettura di rete che viene utilizzata in questa architettura di riferimento. Si articola nelle seguenti sezioni:
* **Panoramica di VCS** – Questa sezione descrive i punti salienti della piattaforma VCS.
* **Panoramica della rete** – Queste sezioni forniscono informazioni sui componenti di rete utilizzati in questa architettura di riferimento:
  - **Rete di IBM Cloud** – {{site.data.keyword.cloud}} fornisce la rete fisica ed è completamente gestito da {{site.data.keyword.cloud_notm}}. Questa rete ha alcune caratteristiche fondamentali che devono essere comprese per comprendere i flussi di rete tra siti in loco e non in loco e tra carichi di lavoro ospitati in servizi differenti.
  - **NSX-V** – La rete VCS è sviluppata sulla rete {{site.data.keyword.cloud_notm}} con una sovrapposizione fornita da VMware NSX-V. Questa sovrapposizione segmenta il traffico dalla rete di sovrapposizione {{site.data.keyword.cloud_notm}} consentendo un maggior controllo e una maggiore configurabilità, incluso BYOIP.
  - **IBM Kubernetes Service** - IKS utilizza Calico come suo plug-in CNI (Container Network Interface).
  - **IBM Cloud Private** - ICP utilizza Calico come suo plug-in CNI (Container Network Interface).

* **Integrazione** – questa sezione descrive l'architettura dei componenti di rete per consentire i flussi e l'indirizzamento del traffico di rete richiesti:
  - Assegnazione di indirizzi IP
  - Flussi di traffico di rete

## Panoramica di VCS

VCS (VMware vCenter Server) on {{site.data.keyword.cloud_notm}} with Hybridity Bundle è un cloud privato ospitato che consente di estendere in modo semplice e rapido la tua infrastruttura in loco nel cloud per un'ibridità dell'infrastruttura senza interruzioni e sicura e una reale mobilità dell'applicazione.

VCS Hybridity Bundle è distribuito in aggiunta a un minimo di quattro {{site.data.keyword.cloud_notm}} {{site.data.keyword.baremetal_short}}, offre l'archiviazione dedicata tramite una VSAN (virtual storage area network) e include la configurazione e la distribuzione automatica di un'infrastruttura di rete definita dal software di facile gestione (NSX-V). VCS Hybridity Bundle è un'architettura di riferimento distribuita attraverso l'automazione, che garantisce la coerenza, le prestazioni e la conformità. In molti casi, può essere eseguito il provisioning dell'intero ambiente in meno di un giorno e l'infrastruttura bare metal può ridimensionare rapidamente ed elasticamente la capacità di calcolo a seconda delle necessità.
...
Le opzioni di integrazione della piattaforma di VCS Hybridity Bundle non sono limitate alle opzioni disponibili da VMware quali vRealize Suite o vSphere with Operations Management, ma si estendono a più offerte di servizio {{site.data.keyword.cloud_notm}} quali [vCenter Server e IBM Kubernetes](../vcsiks/vcsiks-intro.html) e [vCenter Server e {{site.data.keyword.cloud_notm}} Private](../vcsicp/vcsicp-intro.html), che utilizzano Terraform open source per gestire e fornire IaC (Infrastructure as Code).

Il vasto portafoglio di servizi e opzioni di integrazione di più offerte disponibili per VCS Hybridity Bundle forniscono una piattaforma davvero ibrida che rende l'ibridità un servizio possibile.

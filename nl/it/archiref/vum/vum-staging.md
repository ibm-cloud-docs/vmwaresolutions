...
5. Facoltativamente, cancella le patch e le estensioni da escludere dall'operazione di preparazione. Per ricercare all'interno dell'elenco di patch ed estensioni, inserisci il testo nella casella di testo della casella di filtro. Fai clic su **Avanti**.
...
* Per qualsiasi VM sull'host che utilizza una politica di archiviazione della VM con un'impostazione uguale a 1 per **Number of failures to tolerate**, la VM diventa non ridondante quando l'host entra in modalità di manutenzione. Se questo non è accettabile, vedi [Ridondanza vSAN della VM (Virtual Machine)](/docs/services/vmwaresolutions/archiref/vum?topic=vmware-solutions-vum-vsan-redundancy).
...
- **Spegni le VM (Virtual Machine)** - Spegni tutte le VM e i dispositivi virtuali prima della correzione.
- **Sospendi VM (Virtual Machine)** - Sospendi tutte le VM e i dispositivi virtuali in esecuzione prima della correzione.

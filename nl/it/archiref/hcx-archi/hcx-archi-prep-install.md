...
* [Requisiti di accesso alla porta](/docs/services/vmwaresolutions/archiref/hcx-archi?topic=vmware-solutions-hcx-archi-port-req) elenca le porte che devono essere aperte in modo che le applicazioni virtuali HCX possano essere installate correttamente.
...
* Il vSphere vCenter deve soddisfare i seguenti requisiti per supportare l'estensione di livello 2:
  ...
  * Se la rete deve essere estesa su internet pubblico o su una VPN (su un percorso alternativo) la VM (Virtual Machine) L2C in VCF/VCS richiede un indirizzo IP. L'indirizzo IP remoto è obbligatorio per configurare il concentratore di livello 2.

...
Il programma di installazione esegue il provisioning e configura una VM (Virtual Machine) per ogni dispositivo virtuale del servizio. Le VM (Virtual Machine) del servizio vengono distribuite sia in loco che nel cloud.
...
La procedura di configurazione presuppone che saranno configurati tutti i dispositivi virtuali del servizio. Tuttavia, non sono tutti necessari.
...
* Per configurare il servizio Network Extension, consulta la sezione _Configurazione del servizio Network Extension_. La distribuzione del dispositivo facoltativo può essere rinviata ritornando alla pagina Hybrid Services e installando il dispositivo successivamente.

## Avvio dell'installazione e della configurazione del dispositivo virtuale del servizio ibrido
...
Configura il dispositivo virtuale del servizio Hybrid Cloud Gateway. Prima di cominciare, segui la procedura in _Avvio dell'installazione e della configurazione del dispositivo virtuale del servizio ibrido_ e seleziona Hybrid Cloud Gateway.
...
* In **Extended (optional)**, scegli la rete vMotion se applicabile e imposta le password **admin** e **root**. Queste password sono specifiche per il dispositivo Hybrid Cloud Gateway. Il nome utente e la password non devono corrispondere a quelli configurati per il dispositivo Hybrid Cloud Services.
...
## Monitoraggio della distribuzione del dispositivo del servizio
...
### Procedura per monitorare la distribuzione del dispositivo del servizio
...
2. Sul pannello **Hybrid Cloud Services**, fai clic sulla scheda **Hybrid Services**. La distribuzione del dispositivo virtuale può essere monitorata dalla console delle attività.
...
8. Controlla il riepilogo della configurazione per i dispositivi virtuali del servizio ibridi.
...
Estendi una rete di livello 2 dal data center in loco al cloud abilitato VCS HCX.

...
VMware HCX on IBM Cloud integra senza interruzioni le reti vSphere® vCenter™ in loco nelle distribuzioni IBM Cloud for VMware Solutions. La rete ibrida estende le reti vSphere vCenter in loco in IBM Cloud, supportando la mobilità della macchina virtuale (VM) bidirezionale.
...
HCX fornisce tre metodi di spostamento delle macchine virtuali: migrazione con basso tempo di inattività, migrazione vSphere vMotion e migrazione di tipo cold (a freddo).

### Migrazione con basso tempo di inattività

La migrazione con basso tempo di inattività si basa su vSphere Replication, che è una tecnologia distribuita implementata nell'hypervisor VMware ESX®/ESXi®. La distribuzione HCX in loco crea una replica di una macchina virtuale live in IBM Cloud ed esegue un cambio per spegnere la macchina virtuale di origine ed accendere la macchina virtuale migrata.
...
Le macchine virtuali possono essere trasferite utilizzando la migrazione vMotion su una rete estesa a IBM Cloud. La migrazione vMotion è anche nota come migrazione senza tempo di inattività o vMotion tra cloud.
...
L'instradamento di prossimità garantisce che l'inoltro tra le macchine virtuali connesse alle reti estese ed instradate, sia in loco che nel cloud, sia simmetrico. Questa funzione richiede che i servizi di rete avanzati con l'instradamento dinamico siano configurati tra il cloud e la locale del cliente.
...
Il tromboning non è efficiente perché i pacchetti devono andare avanti e indietro tra la rete di origine e il cloud, anche quando sia la macchina virtuale di origine che di destinazione risiedono nel cloud.

In aggiunta all'inefficienza, se il percorso di inoltro include dei firewall con stato o un'altra apparecchiatura incorporata che deve vedere entrambi i lati della connessione, la comunicazione potrebbe non riuscire. L'errore nella comunicazione della macchina virtuale (senza l'ottimizzazione dell'instradamento) si verifica quando il percorso in uscita verso il cloud può essere la rete di livello 2 estesa o la rete instradata dell'organizzazione. La rete in loco non conosce la "scelta rapida" della rete estesa. Questo problema viene denominato instradamento asimmetrico. La soluzione è di abilitare l'instradamento di prossimità in modo che la rete in loco possa conoscere le rotte da IBM Cloud.

Il gateway cloud conserva un inventario di macchine virtuali nel cloud. Conosce inoltre lo stato della VM, che può essere uno dei seguenti:
...
Il gateway cloud offre AES-GCM conforme con Suite B con IKEv2, offload AES-NI e controllo di ammissione basato sul flusso. HCX gestisce inoltre il processo di crittografia e decrittografia dell'origine e della destinazione, garantendo una protezione coerente e la gestione dei flussi di lavoro ibridi come la migrazione della macchina virtuale e l'estensione della rete. Le politiche di sicurezza definite e assegnate a una macchina virtuale in loco, possono essere migrate con la macchina virtuale in IBM Cloud.
...
L'applicazione virtuale HCX Manager è un'estensione al vCenter in loco. Viene distribuita come una macchina virtuale e la sua struttura file contiene le altre applicazioni virtuali del servizio ibride. HCX Manager controlla la distribuzione e la configurazione del gateway cloud, i concentratori di livello 2 e l'applicazione virtuale WAN Optimization sia in loco che all'interno di IBM Cloud.
...
* Le reti devono consentire alle applicazioni di comunicare con le applicazioni virtuali remote e locali e con altre macchine virtuali.
...
* [Introduzione di VMware HCX on IBM Cloud](/docs/services/vmwaresolutions/archiref/hcx-archi/hcx-archi-intro.html)

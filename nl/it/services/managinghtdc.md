...
1. Nel client web vSphere, individua le VM (Virtual Machine) denominate **KC1** e **KC2**.
...
## Abilitazione dell'accesso a internet per le VM HyTrust DataControl
{: #managinghtdc-internet-access}

Per HTDC 4.3.2 e versioni successive, {{site.data.keyword.vmwaresolutions_short}} fornisce un supporto di rinnovo automatico per le licenze HyTrust per cui è abilitata la funzione Call Home. Per le istanze vCenter Server che non sono solo private, HTDC viene distribuito con le regole firewall e SNAT (Source Network Address Translation) definite nell'**mgmt-nsx-edge** dell'ESG dei servizi di gestione.

Queste regole ti consentono di abilitare l'accesso a internet per le macchine virtuali (VM, Virtual Machine) HyTrust. Se l'accesso a internet non è abilitato, la licenza che viene applicata all'installazione HTDC scadrà dopo un anno.
...
### Procedura per abilitare la connettività a internet per HTDC
{: #managinghtdc-proc-enable-internet}

1. Completa i passi da 1 a 3 nella procedura precedente.
2. Fai clic su **Settings** e quindi su **Interfaces**. Prendi nota dell'indirizzo IP per l'uplink privato. Questo indirizzo diventa il nuovo gateway predefinito.
...
4. Accedi alla console utilizzando le credenziali della console che puoi trovare nella pagina dei dettagli del servizio HyTrust DataControl on {{site.data.keyword.cloud_notm}} nella console {{site.data.keyword.vmwaresolutions_short}}.
...
9. Per confermare che la VM ha accesso a internet, esegui il ping di un sito web o di un indirizzo IP pubblico. Fai clic su **Manage Network Settings > Network Diagnostic Tools > Test Inbound Ports of Another Server**. Immetti un indirizzo di sito web pubblico, ad esempio `www.ibm.com`, fai clic su **OK**, immetti `80 443` per le porte (o qualsiasi altra porta che vuoi testare) e dovresti ottenere una risposta immediata che mostra le porte in entrata con un messaggio simile a `80 (OK) 443 (OK)`.

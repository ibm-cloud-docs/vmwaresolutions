...
* Für jede virtuelle Maschine (VM) auf dem Host, die eine VM-Speicherrichtlinie mit einer Einstellung für "Anzahl der zu tolerierenden Fehler=1" verwendet, wird die virtuelle Maschine nicht redundant, wenn der Host in den Wartungsmodus wechselt. Wenn dies nicht akzeptabel ist, finden Sie weitere Informationen im Abschnitt [Redundanz virtueller vSAN-Maschinen](vum-vsan-redundancy.html).
...
10.	In einer vCenter Server-Instanz ist es nicht erforderlich, das Kontrollkästchen unter den ESXi-Patcheinstellungen auszuwählen, damit Update Manager eingeschaltete, mit PXE gebootete ESXi-Hosts korrigiert.

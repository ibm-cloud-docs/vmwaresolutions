...
Das Design ermöglicht Ihnen, direkt über Console User Interface (DCUI), ESXi Shell und Secure Shell (SSH) auf die virtuellen Hosts zuzugreifen.
...
| Domänennamensauflösung | Verwendet DNS wie in [Design der allgemeinen Services](design_commonservice.html) beschrieben. |
...
* Nach der Bereitstellung können Sie die Instanz auf maximal 32 ESXi-Hosts skalieren.
...
Weitere Informationen zu Clustern finden Sie im Dokument zur [	Architektur von {{site.data.keyword.cloud_notm}} mit
VMware-Clustern](https://www.ibm.com/cloud/garage/files/IBM-Cloud-for-VMware-Solutions-Multicluster-Architecture.pdf).
...
Die verfügbaren vSAN-Features hängen von der Lizenzedition ab, die Sie bei der Bestellung der Instanz auswählen. Weitere Informationen finden Sie unter [Vergleich der VMware vSAN-Editionen](appendix.html#vmware-vsan-edition-comparison).
...
Weitere Informationen zu physischen NIC-Verbindungen enthält Abbildung 2, "Physische NIC-Hostverbindungen im Abschnitt [Design der physischen Infrastruktur](design_physicalinfrastructure.html)".
...
vSAN-Einstellungen werden nach bewährten Verfahren für die Bereitstellung von VMware-Lösungen in {{site.data.keyword.cloud_notm}} festgelegt. Dazu gehören SIOC-Einstellungen, die Einstellungen für die Portgruppe für explizites Failover und Plattencacheeinstellungen.
...
Die Netzvirtualisierung stellt ein Netzoverlay zur Verfügung, das in der virtuellen Schicht vorhanden ist. Dies stattet die Architektur mit Funktionen aus, wie schnelle Einrichtung, Bereitstellung, Rekonfiguration und Löschung von virtuellen On-Demand-Netzen. In diesem Design wird der vSphere Distributed Switch (VDS) und VMware NSX for vSphere zur Implementierung eines virtuellen Netzes verwendet.
...
Neben den Controllern bereitet die {{site.data.keyword.cloud_notm}}-Automatisierung die bereitgestellten vSphere-Hosts mit NSX-VIBs vor, um die Verwendung eines virtualisierten Netzes durch VXLAN-Tunnelendpunkte (VTEPs) einzurichten. Den VTEPs wird eine VLAN-gestützte IP-Adresse aus dem portierbaren IP-Adressbereich von 'Privat A' zugeordnet, der für VTEPs angegeben ist, wie in der [VLAN- und Teilnetzzusammenfassung](design_physicalinfrastructure.html#table_vlan_subnet_summary) aufgeführt. Der VXLAN-Datenverkehr befindet sich im nicht mit Tags versehenen VLAN und wird dem privaten vSphere Distributed Switch (VDS) zugewiesen.
...
| vSphere Distributed<br>Switch-Name | Funktion | Netz<br>E/A-Steuerung | Lastausgleichs-<br>modus | Physische NIC-<br>Ports | MTU |
|:------------- |:------------- |:------------- |:------------- |:------------- |:------------- |
| SDDC-Dswitch-Private | ESXi-Management, virtuelles SAN, vSphere vMotion, VXLAN-Tunnelendpunkt, NFS (VTEP) | Aktiviert | Route auf Basis des virtuellen Ursprungsports für Explicit Failover (vSAN, vMotion) (andernfalls alle) | 2 | 9000<br>(Jumbo-Frames) |
| SDDC-Dswitch-Public | Externer Managementdatenverkehr (Nord-Süd) | Aktiviert | Route auf Basis des virtuellen Ursprungsports | 2 | 1500<br>(Standard) |

**Hinweis:** Die Namen, die Anzahl und die Anordnung der Host-NICs kann abhängig vom {{site.data.keyword.CloudDataCent_notm}} und Ihrer Auswahl der Host-Hardware variieren.
...
| vSphere Distributed Switch | Zweck   | Verbundene Portgruppe | Aktivierte Services | MTU |
|:-------------------------- |:------- |:-------------------- |:---------------- |:--- |
...
* [Architektur der Lösung von {{site.data.keyword.cloud_notm}} mit VMware-Clustern](https://www.ibm.com/cloud/garage/files/IBM-Cloud-for-VMware-Solutions-Multicluster-Architecture.pdf)
* [Architektur der NSX Edge on {{site.data.keyword.cloud_notm}}-Lösung](https://www.ibm.com/cloud/garage/files/IBM_Cloud_for_VMware_Solutions_NSX_Edge_Services_Gateway.pdf)

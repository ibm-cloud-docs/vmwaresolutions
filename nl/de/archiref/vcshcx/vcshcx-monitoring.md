...
Sie können jetzt die Webbenutzerschnittstelle des WAN-Optimierungsprogramm verwenden, um Durchsatzraten, Komprimierungsverhältnisse und Einschränkungen der Bandbreite zu überwachen.
...
3.	Geben Sie im Feld für maximale Bandbreite die maximale Bandbreite, die für die WAN-Optimierungs-Appliance verfügbar ist, in Kb/s an. Die maximale Bandbreite der WAN-Verbindung darf dabei nicht überschritten werden.     
  - Zum Festlegen für MB/s (Basis 10) = MB/s x 1000
  - Zum Festlegen für Gb/s = 1000^2 = Gb/s x 1000^2
  - Standard = 10 Gb/s (10000000)
...
- vMotion Traffic wird am besten über die Webbenutzerschnittstelle für das WAN-Optimierungsprogramm realisiert. Das WAN-Optimierungsprogramm reduziert den Datenverkehr, der über das WAN geht, drastisch und reduziert den Paketverlust durch das Senden von redundanten Paketen. Es wurde beobachtet, dass das typische Verhältnis von LAN- zu WAN-Bandbreite ungefähr 3:1 (350 MB/s LAN = 90-120 MB/s WAN) beträgt.

- Die replikationsbasierte (Massen-) Migration von VMs in HCX führt zur Überlastung von VMs. Während dies nicht wünschenswert sein kann, zeigt die Benutzerschnittstelle des WAN-Optimierungsprogramms ein hohes Verhältnis zwischen LAN und WAN, wenn Sie "leere" Plattendaten verschieben. Umgekehrt ist zu beobachten, dass bei der Migration von nicht komprimierbaren Daten, wie z. B. DB-Daten und digitalen Medien, die WAN-Verwendung am höchsten ist, da sie der Verwendung der LAN-Eingabe näher kommt.

Beobachtungen:
- Die vMotion einer VM in HCX bewirkt nicht mehr als den Durchsatz des vMotion-Netzes an einen einzigen ESX-Host.
...
Der Layer 2-Konzentrator der HCX-Paketkomponente hat eine Bandbreite von insgesamt ungefähr 4 Gb/s für den gesamten L2-Netzverkehr, der ihn durchquert. Einzelne erweiterte Netze haben je nach Datenverkehrstyp eine Bandbreitenbegrenzung von maximal ca. 1 Gb/s. Es ist möglich, mehrere erweiterte L2-Netze über ein einziges L2C-Paar zu betreiben (theoretisch zulässig sind maximal 4096 Netze pro L2C-Paar). Während der L2C so konstruiert ist, dass kleine Datenflüsse erkannt und vor großen Datenflüssen im selben L2C-Paar geschützt werden, kann es von Vorteil sein, festzustellen, ob diese Situation auftritt, und weitere L2Cs zur Erhöhung der verfügbaren Gesamtbandbreite aufzubringen. Die Bereitstellung mehrerer L2Cs kann auch dann vorteilhaft sein, wenn zwischen dem Kundenstandort und {{site.data.keyword.cloud}} mehrere Pfade vorhanden sind, z. B. eine direkte Verbindung und das Internet, sodass beide verwendet werden können, vorausgesetzt, Sie verfügen über mehrere Netze, die darüber verteilt werden können. Ein einzelnes Netz kann nicht redundant gemacht werden oder mit einer höheren Bandbreite über mehrere L2C-Paare hinweg ausgestattet werden.

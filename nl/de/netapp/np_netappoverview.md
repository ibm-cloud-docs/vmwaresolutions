...
# Überblick zu NetApp ONTAP Select
...
Auf dieser Schicht wird die physische Infrastruktur (Berechnungs-, Netz- und Speicherressourcen) bereitgestellt, die von der virtuellen Infrastruktur genutzt wird.

### Virtualisierungsinfrastruktur (Berechnung, Netz und NetApp ONTAP Select)

Diese Schicht virtualisiert die physische Infrastruktur durch verschiedene VMware-Produkte und das Produkt "NetApp ONTAP Select":
...
NetApp ONTAP Select wird in einem VMware-Cluster ausgeführt und virtualisiert den lokalen Speicher auf den Hosts. NetApp ONTAP Select wird im dedizierten Modell bereitgestellt; hierbei wird davon ausgegangen, dass keine anderen Workloads denselben Cluster in diesem Modell gemeinsam nutzen. Der Umfang der Hardwarekonfiguration des Produktangebots "NetApp ONTAP Select on {{site.data.keyword.cloud_notm}}" richtet sich daher nur nach den Voraussetzungen für NetApp ONTAP Select.
...
## Komponenten der NetApp ONTAP Select-Instanz

...
**Wichtig**: Sie müssen sicherstellen, dass die FSA-Firewallregeln so definiert sind, dass abgehende HTTPS-Übertragungen (TCP-Port 443) zulässig sind, die von Managementkomponenten wie der virtuellen Maschine von IBM CloudDriver oder von Zerto Virtual Manager eingeleitet werden, um mit der externen Managementdatenbank in der {{site.data.keyword.cloud_notm}}-Infrastruktur über das Internet zu kommunizieren. Die abgehende HTTPS-Kommunikation (TCP-Port 443) stammt aus der öffentlichen IP-Adresse der Management-Services für VMware NSX Edge Services Gateway (ESG) in Ihrer Instanz.
...
* [Übersicht über FortiGate Security Appliance on {{site.data.keyword.cloud_notm}}](fsa_considerations.html)

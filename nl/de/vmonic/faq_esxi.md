...
Bei V2.2 und höhere Releases können Sie maximal 51 ESXi-Server zu einem ersten Cluster und maximal 59 ESXi-Server zu zusätzlichen Clustern hinzufügen.
...
3. Beachten Sie, dass Sie möglicherweise die Größe für vCenter Server erhöhen müssen, damit die zusätzlichen virtuellen Maschinen und ESXi-Server aufgenommen werden können.
4. Öffnen Sie ein IBM Support-Ticket; geben Sie dabei an, dass Sie die vSAN-Änderungen durch Ausführen der Schritte 1 bis 3 angewendet haben und dass Sie eine Aktivierung Ihrer Instanz, für die Sie ein Upgrade durchgeführt haben, für zusätzliche ESXi-Server über eine Anzahl von 32 hinaus wünschen.
...
Es wird empfohlen, den Rootzugriff auf ESXi-Servern aktiviert zu lassen, da es andernfalls zu Störungen bei der Funktionalität von {{site.data.keyword.vmwaresolutions_short}} kommen kann.

Sollte es unbedingt erforderlich sein, können Sie den Rootzugriff inaktivieren, nachdem die ESXi-Server den Status **Bereit** in der {{site.data.keyword.vmwaresolutions_short}}-Konsole erreicht haben.

Für nachfolgende Automatisierungsoperationen, beispielsweise beim Hinzufügen und Entfernen von Dateifreigaben oder beim Installieren zusätzlicher Services wie Zerto on {{site.data.keyword.cloud_notm}} müssen Sie den Rootzugriff wieder aktivieren.
...
**Hinweis**: Das Hinzufügen statischer Routen für vMotion wird nicht unterstützt. Änderungen an der vMotion-Teilnetzkonfiguration könnten zu Störungen bei der Funktionalität von {{site.data.keyword.vmwaresolutions_short}} führen.

...
**注**: 次の情報は、V2.3 以降でデプロイ (または、V2.3 以降にアップグレード) されたインスタンスへの IBM Spectrum Protect Plus のインストールに適用されます。 V2.2 インスタンスの場合、このサービスはワークロード VM のデータのみを保護します。

IBM Spectrum Protect Plus on {{site.data.keyword.cloud_notm}} サービスには、自動的に実行される管理バックアップ・ジョブが事前構成されています。このジョブは毎日実行され、次の管理コンポーネントを、最大 7 個の回復ポイントを使用してバックアップします。

...
*  [オンプレミス HCX on {{site.data.keyword.cloud_notm}} インスタンスをインストールする際の考慮事項](/docs/services/vmwaresolutions/services?topic=vmware-solutions-standalone_considerations)に記載されている、すべての考慮事項を確認する。
...
サービス・アクティベーション・キーの注文が自動的に開始されます。 お客様は、注文を処理していることを示す確認メッセージを受信します。インスタンスの詳細を参照して、注文状況を確認できます。

アクティベーション・キーを使用する準備ができると、インスタンスの状況が**「インストール済み」**に変更され、お客様は E メールで通知を受け取ります。

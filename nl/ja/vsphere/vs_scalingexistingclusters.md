...
*  拡張対象のクラスターを使用する準備ができたこと確認する E メールを受信する。
...
6. クラスターのパブリック VLAN 上に FortiGate 300 シリーズ Security Appliance の HA ペアが含まれていない場合は、アプライアンスを注文できます。 これを行うには、**「FortiGate 物理アプライアンス 300 シリーズ HA ペア (FortiGate Physical Appliance 300 Series HA Pair)」**の下の**「購入に含める (Include with purchase)」**チェック・ボックスを選択します。
...
拡張対象のクラスターを使用する準備ができていない場合は、エラー・メッセージを受け取る可能性があります。

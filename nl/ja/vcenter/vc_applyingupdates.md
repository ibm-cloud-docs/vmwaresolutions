...
このセクションに記載する手順は、V2.1 から V2.4 までのリリースでデプロイされたインスタンスの IBM 管理コンポーネントを更新する手順です。

V2.5 以降のリリースでデプロイ (または V2.5 以降のリリースにアップグレード) されたインスタンスの場合、IBM 管理コンポーネントのための更新とパッチは、必要に応じて自動的に適用されます。

V2.0 以前でデプロイされたインスタンスの場合は、更新を手動で適用する必要があります。

また、インスタンス操作時の以下の動作に注意してください。
* 新しいサービスを注文すると、インスタンスが最新バージョンに更新される。
* 新しいクラスターを追加すると、それらのクラスターには最新の VMware コンポーネントがプロビジョンされるが、既存のクラスターは変わらない。
* 新しい ESXi サーバーを追加すると、それらの ESXi サーバーには最新の VMware コンポーネントがプロビジョンされるが、既存の ESXi サーバーは変わらない。

{{site.data.keyword.vmwaresolutions_short}} では、VMware コンポーネントに対して更新やパッチを適用するためのサポートは提供されません。それらの更新は、お客様が監視して適用する必要があります。
{:important}

## IBM 管理コンポーネントの更新を適用する前に
{: #vc_applyingupdates-prereq}

下矢印をクリックして更新エントリーを展開し、以下の情報を確認します。
...
4. 左のナビゲーション・ペインにある**「更新およびパッチ」**をクリックしてから、下矢印をクリックして、適用する IBM 管理の更新を展開し、次のいずれかの手順を実行します。
...
3. 特定のステップで更新プロセスが失敗する場合は、[IBM サポートへのお問い合わせ](/docs/services/vmwaresolutions/vmonic?topic=vmware-solutions-trbl_support)によって支援を得ることができます。 問題の解決方法に関するアドバイスが与えられ、失敗したステップから更新とパッチを適用する方法が提示されます。

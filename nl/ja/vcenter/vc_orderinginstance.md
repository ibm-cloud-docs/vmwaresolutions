...
7. ストレージ設定を行います。
   * **「vSAN Storage」**を選択するときは、**「Number of vSAN Capacity Disks」**、**「Disk Type and Size for vSAN Capacity Disks」**、**vSAN ライセンス**の提供方法を指定します。
   * **「NFS Storage」**を選択するときは、**「Number of Shares」**、**「Size」**、**「Performance」**を指定します。この設定はすべてのファイル共有に適用されます。

...
プライベート・ネットワーク接続は、サイズが 9000 のジャンボ・フレーム MTU (最大伝送単位) を使用するように構成されるので、ストレージや VMware vMotion などの大量のデータ転送のパフォーマンスが向上します。 この値は、VMware 内で IBM Cloud によって許可される最大の MTU です。
...
**注**: V2.0 以前でデプロイされたインスタンスの場合、手動でパブリック・スイッチ MTU 設定を 1500 に更新することをお勧めします。  

表 5. vCenter Server インスタンスとクラスターの MTU 構成設定

| 分散仮想スイッチ (DVS) | V2.1 以降  | V2.0 以前 (または V2.0 以前からアップグレードされたもの) |   
|:-------------- |:-------------- |:------------- |

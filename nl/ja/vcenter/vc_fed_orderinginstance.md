...
ワークロードのニーズに合わせて最適化できる、柔軟でカスタマイズ可能な VMware 仮想化プラットフォームをデプロイするために、開かれた管理接続から切り離してデプロイ済みインスタンスを保護することができる VMware Federal インスタンスを注文します。

**注:** 現時点では、VMware Federal on {{site.data.keyword.cloud}} をサポートしているのは vCenter Server インスタンスだけです。
...
**重要**: 注文時およびインスタンスのデプロイ時に設定した値は変更しないでください。 変更すると、インスタンスが使用できなくなる可能性があります。 例えば、パブリック・ネットワークがシャットダウンしたり、プロビジョンの進行中にサーバーや仮想サーバー・インスタンス (VSI) が Vyatta の内側に移動したり、IBM CloudBuilder VSI が停止したり、削除されたりすることがあります。
...
新しいプライマリー・インスタンスを注文します。 現在、高可用性のためにセカンダリー・インスタンスをデプロイすることはサポートされていません。
...
ストレージ設定は、vSAN、NFS、カスタム NFS ストレージのどれを選択するかによって異なります。

### vSAN ストレージ

vSAN の場合、以下のストレージ・オプションを指定します。
...
**重要:** 2 つの Microsoft Windows 仮想マシンを使用するようにインスタンスを構成する場合は、2 つの Microsoft Windows Server 2012 R2 ライセンスを提供する必要があります。 Microsoft Windows Server 2012 R2 Standard エディションのライセンスと Microsoft Windows Server 2012 R2 Datacenter エディションのライセンスを使用してください。

現在、各ライセンスは単一の物理サーバーにのみ割り当てられ、最大 2 つの物理プロセッサーをカバーします。 1 つの Standard エディション・ライセンスでは、2 プロセッサーのサーバーで 2 つの Microsoft Windows 仮想マシンを実行できます。 したがって、ライセンスは 2 つ必要になります。2 つの異なるホストに 2 つの Microsoft Windows 仮想マシンがデプロイされるからです。
...
7. ストレージ構成を行います。
      * **「vSAN ストレージ」**を選択した場合は、**「vSAN 容量ディスクのディスク・タイプとサイズ」**、**「vSAN 容量ディスクの数」**、VMware vSAN ライセンス・エディションを選択します。
      * **「NFS ストレージ」**を選択した場合は、**「共有の数」**、**「サイズ」**、**「パフォーマンス」**を選択します。
...
インスタンスが正常にデプロイされると、[VMware Federal インスタンスのコンポーネント](../vcenter/vc_fed_overview.html#vcenter-server-instance-components-for-vmware-federal-on-ibm-cloud)に記載しているコンポーネントが VMware 仮想プラットフォームにインストールされます。 注文した ESXi サーバーは、デフォルトでは **cluster1** としてグループ化されます。

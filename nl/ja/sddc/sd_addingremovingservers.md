...
* VMware vSphere Web クライアントから ESXi サーバーの追加や削除を行わないでください。 VMware vSphere Web クライアントで加えた変更は、{{site.data.keyword.vmwaresolutions_short}} コンソールと同期されません。
...
* 追加した ESXi サーバーを削除できます。 デフォルトの ESXi サーバーは削除できません。
...
2. **「Cloud Foundation インスタンス」**の表で、容量を拡張または縮小するインスタンスをクリックします。
...
ESXi サーバーが追加または削除されるときには、E メールで通知されます。 サーバーを削除する際には、ESXi サーバーは、{{site.data.keyword.cloud_notm}} 請求処理サイクル (標準的には 30 日) の終わりに {{site.data.keyword.cloud_notm}} インフラストラクチャーによって完全にレクラメーション処理されることに注意してください。

...
標準化された**「スモール」**と**「ラージ」**のベアメタル・サーバー構成ではデフォルトのポッドが使用され、変更できません。
{:note}
...
**「Skylake」**、**「Broadwell」**、または**「事前構成型」**のいずれかを選択できます。
...
6. ベア・メタル構成を次の手順で実行します。
   * **「Skylake」**または**「Broadwell」**を選択した場合は、**「CPU モデル」**と**「RAM」**のサイズを選択します。

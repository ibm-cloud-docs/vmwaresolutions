...
使用できるデータ・センターは、デプロイメントで選択したベア・メタル・サーバー構成に応じて異なります。 **「カスタマイズ型」**構成を選択すると、クラスターを別の {{site.data.keyword.cloud_notm}} インフラストラクチャー・ポッドにデプロイすることもできます (ただし、選択したデータ・センターに他のポッドが含まれている場合)。 この構成は、初期インスタンスがデプロイされているデフォルトの {{site.data.keyword.cloud_notm}} インフラストラクチャー・ポッドが最大容量に達している場合に便利です。

...
使用できるデータ・センターは、デプロイメントで選択したベア・メタル・サーバー構成に応じて異なります。 **「カスタマイズ型」**構成を選択すると、クラスターを別の {{site.data.keyword.cloud}} インフラストラクチャー・ポッドにデプロイすることもできます (ただし、選択したデータ・センターに追加ポッドが含まれている場合)。 この方法は、初期インスタンスがデプロイされているデフォルトの {{site.data.keyword.cloud_notm}} インフラストラクチャー・ポッドが最大容量に達している場合に便利です。

**注:** 標準化された**「スモール」**と**「ラージ」**のベア・メタル・サーバー構成では、デフォルトのポッドが使用され、変更できません。

クラスターを別のデータ・センターまたはポッドにデプロイする場合は、注文した{{site.data.keyword.baremetal_short}}で使用するために 3 つの追加の VLAN を注文します。

### ベア・メタル・サーバーの設定

**「カスタマイズ型」**または**「事前構成型」**のいずれかを選択できます。
...
**「事前構成型」**のベア・メタル・サーバー構成の場合、vSAN ストレージ設定は変更できません。
* **「スモール」**構成の場合は、2 ディスク・ドライブ (1.9 TB SSD SED) が注文されます。
* **「ラージ」**構成の場合は、4 ディスク・ドライブ (3.8 TB SSD SED) が注文されます。

**「カスタマイズ型」**のベア・メタル・サーバー構成の場合は、ストレージ要件に合わせて vSAN 容量ディスクの数、およびディスク・タイプ、サイズを指定して vSAN ストレージをカスタマイズできます。

### ライセンス交付の設定

クラスター内の VMware コンポーネント (VMware vSphere と VMware vSAN を含む) のライセンス・オプションを指定できます。
* ビジネス・パートナーであるユーザーの場合、vSphere ライセンス (Enterprise Plus エディション) と vSAN ライセンスが自動的に含められて購入されます。 ただし、vSAN ライセンスのエディションは指定する必要があります。
* ビジネス・パートナーでないユーザーの場合、**「購入に含める」**を選択してコンポーネントに IBM 提供の VMware ライセンスを使用することも、**「自分で提供する」**を選択し、所有するライセンス・キーを入力してコンポーネントに「ライセンス持ち込み (BYOL)」を適用することもできます。
...
6. ベア・メタル構成を次の手順で実行します。
   * **「カスタマイズ型」**を選択した場合は、**「CPU モデル」**と**「RAM」**のサイズを選択します。
   * **「事前構成型」**を選択した場合は、**「ベア・メタル・サーバー構成」**を選択します。
7. ストレージ構成を次の手順で実行します。
   * ベア・メタル構成に**「カスタマイズ型」**を選択した場合は、**「vSAN キャパシティー・ディスクの数」**と**「vSAN キャパシティー・ディスクのディスク・タイプおよびサイズ」**を指定します。
   * ベア・メタル構成に**「事前構成型」**を選択した場合は、**「スモール」**と**「ラージ」**のベア・メタル・サーバー構成でのストレージ設定は変更できないことに注意してください。
...
## Cloud Foundation インスタンス内のクラスターの表示
...
4. クラスター名をクリックして、ESXi サーバーのリストと各サーバーの情報を表示します。
   ...
   * vSAN ライセンスと vSphere ライセンスの詳細。 vSphere ライセンスの詳細は、クラスターの注文時に、所有している VMware ライセンスを使用すること (BYOL) を選択した場合にのみ表示されます。
       ...
       * **空きキャパシティー (CPU)**: ライセンスで現在使用できるキャパシティー。

...
**重要:** 注文時およびインスタンスのデプロイ時に設定した値は変更しないでください。 変更すると、インスタンスが使用できなくなる可能性があります。 また、インスタンスがデプロイされた後は、インスタンス名、ルート・ドメイン・ネーム、サブドメイン・ラベル、ホスト名接頭部を変更しないでください。

## システム設定

Cloud Foundation インスタンスを注文する際は、次のシステム設定を指定する必要があります。
...
Cloud Foundation インスタンスでサポートされるのは、vSAN ストレージだけです。
...
* **「カスタマイズ型」**のベア・メタル・サーバー構成を選択した場合、**「vSAN ストレージ」**で次の設定を指定して、インスタンスの VMware vSAN ストレージをカスタマイズできます。
...
Cloud Foundation インスタンスを注文する際は、次のネットワーク・インターフェース設定を指定する必要があります。
...
Cloud Foundation インスタンスを注文するときに、追加のサービスを注文することもできます。 使用可能なサービスについて詳しくは、[Cloud Foundation インスタンス用サービス](sd_planning.html#services-for-cloud-foundation-instances)を参照してください。

## 注文のサマリー

選択したインスタンスとアドオン・サービスの構成に基づき、ただちに見積もりコストが生成されて右側のペインに表示されます。 右側のペインの下部の**「料金詳細」**をクリックすると、見積もりの詳細を示す PDF 文書を生成できます。
...
4. インスタンス・タイプを選択します。
   ...
   * 可用性を向上させる場合は、**「セカンダリー・インスタンス」**をクリックし、環境内の既存の (プライマリー) インスタンスにインスタンスを接続して、以下の手順を実行します。
     ...
     2. 選択したプライマリー・インスタンスが V2.5 リリースにアップグレードされた場合、またはプライマリー・インスタンスが V2.4 以前のリリースにデプロイまたはアップグレードされた場合は、事前入力済みの**「プライマリー・インスタンス PSC の管理者パスワード (Administrator Password for the Primary Instance PSC)」**が正しいことを確認してください。
     
         **注:** **「プライマリー・インスタンス PSC の管理者パスワード (Administrator Password for the Primary Instance PSC)」**フィールドは、V2.5 以降のリリースでデプロイされているプライマリー・インスタンスでは使用できません。     
...
7. ストレージ設定を行います。
   * ベア・メタル構成に**「事前構成型」**を選択した場合は、**「スモール」**と**「ラージ」**の標準ベア・メタル・サーバー構成でのストレージ設定は変更できないことに注意してください。
   * ベア・メタル構成に**「カスタマイズ型」**を選択した場合は、**「vSAN 容量ディスクのディスク・タイプとサイズ」**と**「vSAN 容量ディスクの数」**を指定します。
...
9. インスタンスにデプロイするアドオン・サービスを、対応するサービス・カードをクリックして選択します。 サービスに構成が必要な場合は、サービス固有の設定を入力し、ポップアップ構成ウィンドウの**「サービスの追加」**をクリックします。 サービスの設定方法については、対応するサービス注文トピックを参照してください。

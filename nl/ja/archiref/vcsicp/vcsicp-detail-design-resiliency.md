...
障害が発生した場合にシステムをその動作状態にリストアするためには、{{site.data.keyword.cloud_notm}} Private (ICP) のデプロイメントをバックアップしておくことが不可欠です。 従来の VM バックアップに加えて、すべての ICP マスター・ノードは etcd を実行します。etcd の文書には、リストアを行う際に従来の VM バックアップを使用してはならないと明確に記載されています。

プラットフォーム・レベルの ICP では、以下のコンポーネントをバックアップする必要があります。
...
-	**永続ストレージ**は、ICP または Kubernetes の永続ボリュームとストレージ・プロバイダーを利用する管理サービスで使用されます。ストレージ・プロバイダーが提供するバックアップ・テクノロジーまたはリストア・テクノロジーを使用できます。 同様のプロセスをユーザーのアプリケーションに拡張することもできます。
...
####	ICP の拡張
以下のように ICP ワーカー VM ノードをスケーリングして、コンピュートまたはアプリケーションを拡張します。

  - お客様は ICP がデプロイされている VXLAN に新規 VM をプロビジョンします。
...
###  IKS の拡張
ユーザーは、{{site.data.keyword.cloud_notm}} Portal を介して IKS 環境をプロビジョンし、コンテナー環境を拡張または使用することができます。

IKS へのアプリケーションのデプロイメントは、以下の方法で行うことができます。

- IKS 接続またはサービスを CAM で開発して、ICP カタログに公開する。
...
* [既存のワーカー・プールのサイズを変更してワーカー・ノードを追加する](../../../../containers/cs_clusters.html#resize_pool)
...
* [ICP バックアップ GitHub](https://github.com/ibm-cloud-architecture/icp-backup/)

...
{{site.data.keyword.cloud_notm}} 上の VMware Cloud Foundation および vCenter Server は、基本的なビルディング・ブロックを提供します。これに含まれる VMware vSphere、vCenter Server、NSX や、vSAN などの共有ストレージ・オプションは、ワークロードに最適な VMware ソフトウェア定義のデータ・センター・ソリューションを柔軟に設計するのに必要です。高度な自動化と単一テナント・ベアメタル・インフラストラクチャーを適用することで、VMware 環境全体を迅速に {{site.data.keyword.cloud_notm}} にデプロイし、数時間のうちに任意のツールを使用して仮想環境にアクセスできるようになります。現時点では、ネイティブ VMware クライアント、コマンド・ライン・インターフェース (CLI)、既存のスクリプト、または使い慣れた vSphere API 互換ツールを使用して、IBM ホスト環境にアクセスし、管理できます。

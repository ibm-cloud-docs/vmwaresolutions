...
| ゲートウェイ| 	customer-nsx-edge private uplink ip |	これは、customer-nsx-edge プライベート・アップリンク・アドレスであるプロキシー・サーバーのデフォルトのゲートウェイ設定です。IP は、customer-nsx-edge の「Settings」タブで確認できます。|
| DNS サーバー |	AD/DNS ip | この IP アドレスは、[IC4VS Cloud コンソール](https://console.bluemix.net/infrastructure/vmware-solutions/console/vcenters/)の「インスタンス」ページの「デプロイ済みインスタンス」にあります。|
...
3.	**「Download Settings」**を選択して、_「Proxy settings」_ で**「Edit」**ボタンをクリックします。
4.	**「Use Proxy」**ボックスにチェック・マークを付けて、_プロキシー・サーバーの IP アドレス_ と_ポート 3128_ を入力して、**「OK」**をクリックします
5.	接続状況が_「Validating」_、_「Connected」_ と変わります。
6.	**「Download Now」**ボタンをクリックして、_「Recent Tasks」_ ペインでこのアクティビティーが完了したことを確認します。

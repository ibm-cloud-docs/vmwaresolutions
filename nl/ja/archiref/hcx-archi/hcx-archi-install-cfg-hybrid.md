...
5. **「宛先ゲートウェイの選択 (Select destination gateway)」**ページで、以下の手順を実行します。
  ...
  3. **「近接ルーティング (Proximity Routing)」**は無効なままにして、VCF/VCS ハイブリッド・クラウド・サービスに対応したクラウドの内部にある VM のインターネット・アクセスに常にオンプレミスのゲートウェイを使用させます。 デフォルトでは、VCF/VCS ハイブリッド・クラウド・サービスに対応したクラウドの内部にある VM から発信されるトラフィックは、レイヤー 2 のデータ・パスを横断してオンプレミスのデータ・センターに戻ってから、デフォルトのゲートウェイに向かいます。 **「近接ルーティング (Proximity Routing)」**にチェック・マークを付けた場合は、VCF/VCS ハイブリッド・クラウド・サービスに対応したクラウドの内部にある VM は、vSphere へのレイヤー 2 データ・パスを横断せずに、インターネットにアクセスできます。

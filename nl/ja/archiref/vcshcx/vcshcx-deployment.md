...
この資料の冒頭で説明したとおり、HCX インストールの最小構成は、1 つのクラウド・サイドと 1 つのクライアント・サイドのデプロイメントです。 HCX のクライアント・サイドは、HCX でサポートされているどのバージョンの vSphere にもインストールできます。
...
クライアント・サイドとクラウド・サイドが相互接続する構成はサポートされていません。 HCX エンドポイント接続のためにプライベート・ネットワークを使用する場合は、
...
クライアント・サイドにダウンロードするためのリンクという形で URL が提供されます。 そのリンクは、クラウド・サイド・マネージャーの HCX ユーザー・インターフェース (UI) に表示されます。
...
フリート・コンポーネントのデプロイメントがバックアウトされ、削除されます。 失敗の原因を確認できたら、クライアント・サイドの HCX vCenter Web UI にある**「Interconnect」**タブをクリックし、

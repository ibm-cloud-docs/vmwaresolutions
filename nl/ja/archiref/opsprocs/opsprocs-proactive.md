...
| VM ツールがない VM の確認 | VM ツールは OS の操作性が高いので (VM のグレースフル・シャットダウンなど)、VM ツールをインストールすることをお勧めします。クラスターにナビゲートし、「関連オブジェクト」、「VM (VMs)」を選択してから、テーブル内で実行中の VM ツールの列と VM ツールのバージョンの列を有効にすると、vCenter を使用して VM ツールがインストールされていない VM を確認できます。 リストを確認し、必要な場合は VM ツールをインストールします。|

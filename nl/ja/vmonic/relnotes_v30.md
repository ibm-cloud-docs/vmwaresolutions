...
V3.0 リリース以降、複数のクラスター間で同時にネットワーク・ファイル・システム (NFS) ストレージを追加または削除できます。これは、すべての vCenter Server と vCenter Server with NSX-T のインスタンスに適用されます。
...
* {{site.data.keyword.cloud_notm}} の自動化機能で使用するために {{site.data.keyword.vmwaresolutions_short}} が維持するユーザー ID を記載した参照資料が追加されました。インスタンスの履歴ログに表示される可能性があるメッセージも参照用に記載しています。詳しくは、[vCenter Server インスタンスの IBM ユーザー ID およびデプロイメント履歴のメッセージ](/docs/services/vmwaresolutions?topic=vmware-solutions-ibm_use#ibm_use)を参照してください。

...
選択内容に応じて、管理コンポーネント用の Active Directory サーバーとして、1 つの Microsoft Windows VSI または 2 つの Microsoft Windows 仮想マシンがクラスター内にデプロイされます。オプションで、管理アクセス用の追加 ID ソースとして、独自の Active Directory サーバーを追加することもできます。

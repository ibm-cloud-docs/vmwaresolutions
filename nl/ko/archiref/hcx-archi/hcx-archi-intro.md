...
VMware HCX on IBM Cloud 서비스를 사용하면 IBM Cloud for VMware Solutions 인스턴스와 온프레미스 VMware 가상화 데이터 센터 간에 원활한 연결을 작성할 수 있습니다. 

IBM Cloud for VMware Solutions를 통해 IBM Cloud에 VMware vCenter Server on IBM Cloud(VCS) 또는 VMware Cloud Foundation on IBM Cloud(VCF) 구성을 완전히 자동화하고 빠르게 배치할 수 있습니다. 이 오퍼링은 온프레미스 인프라를 보완하고, 온프레미스에서 사용하는 동일한 도구, 스킬 및 프로세스를 사용하여 변환 없이 IBM Cloud에서 기존 워크로드 및 이후의 워크로드가 실행될 수 있도록 합니다. 자세한 정보는 [IBM Architecture Center](https://www.ibm.com/devops/method/content/architecture/virtualizationArchitecture)를 참조하십시오.

VMware HCX on IBM Cloud 서비스는 워크로드에 대한 원활한 네트워크 확장 및 양방향 마이그레이션을 작성하여 VCS 또는 VCF 중 하나의 인스턴스를 기존의 온프레미스 가상화 데이터 센터와 결합시킴으로써 이 하이브리디티를 더욱 발전시킵니다. 
...
그림 1. VMware Cloud Foundation – 하이브리드 클라우드 서비스
![VMware Cloud Foundation – 하이브리드 클라우드 서비스](cloudfoundation_hybrid_cloud_services.svg)

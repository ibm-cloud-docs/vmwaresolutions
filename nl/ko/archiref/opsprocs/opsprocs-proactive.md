...
| VM Tools 없이 VM 확인 | 우수 사례는 OS에서 더 큰 상호작용 즉, VM의 정상 종료를 허용할 때 VM Tools를 설치하는 것입니다. vCenter를 사용하여 VM 도구가 설치되지 않은 VM을 확인할 수 있습니다. 클러스터로 이동하고 **관련 오브젝트**, **VM**을 선택하고 테이블에서 **실행 중인 VM 도구** 및 **VM 도구 버전**에 대한 열을 사용으로 설정하십시오. 목록을 검토하고 필요에 따라 VM Tools를 설치하십시오. |

...
* **모든 vSAN 디스크가 정상이어야 합니다.**
  ...
  - 업그레이드 프로세스 시작 시 **홈** > **호스트 및 클러스터**를 클릭한 후 **vSAN 클러스터**를 선택하고 **vSAN 탭** > **컴포넌트 재동기화**를 클릭하여 활성화된 재동기화가 없어야 합니다. _컴포넌트 재동기화 수가 0이어야 합니다._ 호스트를 다시 부팅한 후에 데이터를 동기화해야 하므로 업그레이드 프로세스 중에 일부 재동기화 활동이 예상됩니다.

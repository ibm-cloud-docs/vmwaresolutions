...
14.	설치가 계속되면 루트 비밀번호를 설정하고 사용자를 설정하십시오.
...
4. **프록시 사용** 상자를 선택하고 _프록시 서버 IP 주소_ 및 _포트 3128_을 입력한 후 **확인**을 클릭하십시오. 연결 상태가 _유효성 검증 중_으로 변경된 후 _연결됨_으로 변경됩니다.
    **참고**: 프록시 서버에 대한 연결을 테스트하는 동안 ``Some URLs have been accessed successfully, but others are inaccessible`` 메시지를 수신하는 경우 걱정하지 마십시오.

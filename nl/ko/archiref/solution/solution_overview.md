...
VMware Cloud Foundation 및 vCenter Server on {{site.data.keyword.cloud_notm}}는 기본 구성 요소를 제공하며, 여기에는 워크로드에 가장 적합한 VMware 소프트웨어-정의 데이터 센터 솔루션을 유연하게 구축하는 데 필요한 vSAN을 포함한 공유 스토리지 옵션과 VMware vSphere, vCenter Server, NSX가 포함됩니다. 고급 자동화 및 단일 테넌트 베어메탈 인프라를 적용함으로써, 사용자는 전체 VMware 환경을 {{site.data.keyword.cloud_notm}}에 신속하게 배치하고 몇 시간 내에 선호하는 도구를 사용하여 가상 환경에 액세스할 수 있습니다. 이 시점에서 사용자는 고유 VMware 클라이언트, 명령행 인터페이스(CLI), 기존 스크립트 또는 기타 친숙한 vSphere API-호환 가능 도구를 통해 IBM 호스팅 환경에 액세스하고 이를 관리할 수 있습니다. 

배치 후에 사용자는 ESXi 호스트 노드를 추가하고 일부 관리 컴포넌트에 대한 패치와 백업을 관리할 수 있습니다. {{site.data.keyword.cloud_notm}} Professional Services 및 Managed Services를 활용하면 마이그레이션, 구현 및 온보딩 서비스 등의 오퍼링을 사용하여 클라우드로의 이동 속도를 높이는 데 도움이 됩니다. 

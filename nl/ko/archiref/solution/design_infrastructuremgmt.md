...
* 이 솔루션에서 PSC의 레버리지를 활용하면 VMware vCenter 싱글 사인온, 라이센스 서비스, 검색 서비스 및 VMware 인증 기관을 포함하는 인프라 서비스 세트를 제공할 수 있습니다.
...
**참고**: 이러한 값은 배치 시에 설정되며 변경이 불가능합니다.
...
이 디자인을 사용하면 솔루션을 통해 프로비저닝된 vSphere ESXi 호스트를 클러스터링할 수 있습니다. 그러나 클러스터가 작성되기 전에, 데이터 센터 내의 팟(Pod)은 물론 vSphere ESXi 호스트의 위치를 나타내는 데이터 센터 오브젝트가 작성됩니다. 클러스터는 데이터 센터 오브젝트가 작성된 후에 작성됩니다. 클러스터는 VMware vSphere HA(High Availability) 및 VMware vSphere DRS(Distributed Resource Scheduler)가 사용되어 배치됩니다.

### vSphere Distributed Resource Scheduler

이 디자인에서는 초기 클러스터의 vSphere DRS(Distributed Resource Scheduling)를 사용하여 VM을 배치하며, 추가 클러스터의 DRS을 사용하여 VM을 동적으로 마이그레이션함으로써 밸런스가 유지된 클러스터를 구성합니다. 초기 배치와 마이그레이션 권장사항이 vSphere에 의해 자동으로 실행될 수 있도록, 자동화 레벨은 완전 자동화로 설정되어 있습니다. 또한 vCenter가 우선순위 1, 2, 3 권장사항을 적용하여 클러스터의 로드 밸런싱에서 최소한 적절한 개선이 이루어질 수 있도록, 마이그레이션 임계값은 적절하게 설정되어 있습니다.

**참고:** **분배 전원 관리** 기능을 통한 전원 관리는 이 디자인에서는 사용되지 않습니다.

### vSphere High Availability

이 디자인은 초기 클러스터와 추가 클러스터에서 vSphere High Availability(HA)를 사용하여 컴퓨팅 장애를 감지하고 클러스터 내에서 실행되는 VM을 복구합니다. 이 디자인에서 vSphere HA 기능은 클러스터 내에서 **호스트 모니터링** 및 **승인 제어** 옵션을 모두 사용하여 구성됩니다. 또한 초기 클러스터는 1개 노드의 리소스를 승인 제어 정책을 위한 여유 용량으로서 예약합니다.

**참고**: 클러스터가 나중에 확장되거나 축소될 때 사용자는 승인 제어 정책을 조정해야 합니다.
...
{{site.data.keyword.IBM}} CloudBuilder, IBM CloudDriver 및 SDDC Manager VM은 함께 작동하여 새 VMware 인스턴스를 가져오며 라이프사이클 관리 기능을 수행합니다.
...
* NSX VTEP, vSAN, 리소스 풀 등의 영향을 받는 서비스를 포함하여 클러스터 내에서 ESXi 호스트의 확장 및 축소.

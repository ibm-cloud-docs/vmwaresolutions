...
원래 vCenter Server 클러스터에는 vCenter Server 어플라이언스와 함께 NSX-V 및 NSX-T에 대한 모든 관리 컴포넌트가 포함됩니다. 이 클러스터의 호스트는 [NSX-V 개요](/docs/services/vmwaresolutions/archiref/vcsnsxt/vcsnsxt-overview-ic4vnsxv.html)에 설명된 대로 NSX-V을 위해 준비됩니다. 두 번째 클러스터의 호스트는 NSX-T를 위해 준비되고 전송 노드로 구성됩니다. 이 구성을 통해 고객은 NSX-V에서 NSX-T로 마이그레이션할 수 있습니다(원하는 경우).

...
HCX는 클라우드 측(대상 또는 VCD 환경)과 하나 이상의 클라이언트(소스)로 구성됩니다. HCX가 배치된 vCenter가 클라이언트나 클라우드 측의
...
HCX에는 클라우드 측(대상 또는 VCD 환경)과 클라이언트 측(소스)의 개념이 있습니다.

- 대상 측 - HCX 클라우드 관리자는 사전 배치되고 서비스 메시 작성을 위해 준비된 컴퓨팅 프로파일과 네트워크로 구성되어 있습니다.  
- 클라이언트측 - 설치 및 조작을 위한 전제조건을 충족하는 모든 vSphere 인스턴스. HCX의 클라이언트 측은
...
- 클라이언트 웹 UI - HCX 클라이언트 웹 포털은 HCX의 기본 UI입니다. 클라이언트 측 HCX 관리자가 설치된 후 vCenter 웹 UI에 스냅인으로 표시됩니다. 원격 클라우드 HCX 등록(사이트 페어링),

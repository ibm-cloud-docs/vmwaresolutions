...
- 일회성 사용 등록 키는 클라이언트 측 설치용으로 {{site.data.keyword.vmwaresolutions_full}} 콘솔에서 제공됩니다. 이 키는 각 클라이언트 측
HCX 설치에 필수입니다.
...
9. **완료 준비** 페이지에서 다음 단계를 수행하십시오.
  ...
  * **요약** 탭을 보십시오. 콘솔에 **전원 켜짐**이 표시되고 **재생** 아이콘은 녹색입니다.
...
3. 클라이언트 측 전제조건에서 수집된 라이센스 키를 제공하십시오.
...
2. **대시보드** 분할창에서 다음 단계를 완료하십시오.
  1. 왼쪽 분할창의 **시스템 구성**에서 vCenter를 선택하십시오.
  2. 오른쪽 상단의 **vCenter 추가**를 클릭하십시오.
  3. `https://vCenter-host-name` 또는 `https://vCenter-IP-address` 양식으로 vCenter Server의 IP 주소를 입력하십시오. 예를 들어, `https://My-vCenter` 또는 `https://10.108.26.211`입니다.

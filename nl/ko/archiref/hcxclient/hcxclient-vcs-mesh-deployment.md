...
다음 단계를 검토하여 HCX 클라이언트 인스턴스를 구성하십시오.

## IBM Cloud for VMware Solutions 환경의 사이트 페어링
{: #hcxclient-vcs-mesh-deployment-sitepair}

1. VMware vSphere Web Client에 로그인하십시오.
2. **홈** 메뉴에서 **HCX** 옵션을 선택하십시오.
3. **인프라**, **상호연결**에서 **사이트 페어링 추가**를 클릭하십시오.
  1. 사이트 URL을 HCX Cloud Manager URL로 설정하십시오(예: `https://x.x.x.x.x`).
  2. 사용자 이름 및 비밀번호를 HCX Manager 관리 세부사항(admin/password)으로 설정하십시오.

    이전 세부사항은 vCenter Server 인스턴스의 **서비스**, **HCX on IBM Cloud** 아래에 있는 VMware Solutions 콘솔의 IBM Cloud에서 확보할 수 있습니다.
...
사이트 페어링은 UI에서 등록되고 표시됩니다.

## HCX 온프레미스 프로파일 작성
{: #hcxclient-vcs-mesh-deployment-profiles}

### 온프레미스 서비스 메시 네트워크 프로파일 작성
{: #hcxclient-vcs-mesh-deployment-profiles-network}

1. vSphere Web Client에 로그인하십시오.
2. **홈** 메뉴에서 **HCX** 옵션을 선택하십시오.
3. **인프라**에서 **InterConnect**를 클릭하십시오.
4. **다중 사이트 서비스 메시** 아래에서 **네트워크 프로파일**을 클릭하십시오.
5. **네트워크 프로파일 작성**에서:
   1. 분배된 포트 그룹(예: 외부)을 선택하십시오.
   2. **외부 IP**, **외부 서브넷 접두부 길이**, **외부 게이트웨이** 및 **DNS 세부사항**의 IP 주소 범위를 제공하십시오.
   3. MTU를 1500으로 설정하십시오.
   4. **작성**을 클릭하십시오.
6. 관리 및 vMotion 네트워크에 대한 이전 단계를 반복하십시오.
   참고: MTU 값을 9000으로 설정하십시오.
...
### 온프레미스 서비스 메시 컴퓨팅 프로파일 작성
{: #hcxclient-vcs-mesh-deployment-profiles-compute}

1. vSphere Web Client에 로그인하십시오.
2. **홈** 메뉴에서 **HCX** 옵션을 선택하십시오.
3. **인프라**에서 **InterConnect**를 클릭하십시오.
4. **다중 사이트 서비스 메시**에서 **프로파일 계산**을 클릭하십시오.
5. **컴퓨팅 프로파일 작성**에서:
...
2. **홈** 메뉴에서 **HCX** 옵션을 선택하십시오.
3. **인프라**에서 **InterConnect**를 클릭하십시오.
4. **다중 사이트 서비스 메시**에서 **서비스 메시**를 클릭하십시오.
5. **서비스 메시 작성**에서:
   1. 사이트, 즉 온프레미스 및 vCloud 조직을 선택하고 **계속**을 클릭하십시오.
   2. 소스 컴퓨팅 프로파일을 선택하십시오.
   3. 원격 컴퓨팅 프로파일을 선택하십시오. 예를 들면, CloudCompute입니다.
   4. 모든 서비스를 선택하고 **계속**을 클릭하십시오.
   5. 고급 구성 - 대체 업링크 네트워크 프로파일 페이지에서 **계속**을 클릭하십시오(선택사항).
   6. **계속**을 클릭하십시오.
   7. **계속**을 클릭하고 고급 구성 - WAN 최적화 서비스 대역폭 한계 구성 페이지에서 기본값을 계속 사용하십시오.
   8. 서비스 이름을 제공하고 **완료**를 클릭하십시오.
6. 서비스 메시 작성을 위해 태스크 목록을 살펴보십시오. 성공적으로 완료되면 온프레미스 위치에 세 개의 HCX 어플라이언스가 있고 클라우드 위치에 세 개의 HCX 어플라이언스가 있습니다.

### 결과
{: #hcxclient-vcs-mesh-deployment-servicemesh-results}

HCX 서비스 메시는 소스 및 대상 사이트에 유효한 HCX 서비스 구성입니다. 서비스 메시는 두 사이트 모두에 유효한 컴퓨팅 프로파일이 작성된 연결된 사이트 쌍에 추가될 수 있습니다.
...
1. vSphere Web Client에 로그인하십시오.
2. **홈** 메뉴에서 **HCX** 옵션을 선택하십시오.
3. 왼쪽 메뉴의 **서비스**에서 **네트워크 확장**을 클릭하십시오.
4. **네트워크 확장**을 클릭하십시오.
   1. 확장할 네트워크를 선택하십시오.
...
클라이언트 측 네트워크를 클라우드 측 VXLAN에 연결하는 접점은 전용 HCX 기술로 구성된 정교한 다중 터널 VPN입니다. NSX를 기반으로 하지 않지만 NSX와 함께 작동하며 기능을 확장합니다. 이 프로세스는 클라이언트 측 vCenter 웹 사용자 인터페이스를 통해 제어되며 클라이언트와 클라우드 측의 두 엔드포인트 배치 및 시작을 자동화합니다. 확장할 네트워크는 개별적 또는 일괄처리를 통해 선택합니다.

또한 네트워크 확장 워크플로우의 일부로, 클라우드 측의 NSX에 VXLAN을 빌드하도록 권한이 부여됩니다. 그런 다음 VXLAN이 지정된 클라우드 측 L3 디바이스(연결되지 않은 상태로 남아 있는 DLR 또는 ESG) 및 클라우드 측 네트워크 확장 어플라이언스에 작성된 인터페이스에 연결됩니다.
...
클라이언트의 백업 네트워크 어댑터에 액세스되어 있지 않습니다. 이는 게스트 내 클라이언트 백업 스케줄을 종료하기 위해 각 VM에 액세스하는 것을 의미하기 때문입니다. 따라서 백업 네트워크가 사용되는 경우 백업에 실패할 수 있습니다. 이 상황은 모든 VM이 사후 마이그레이션에 도달하여 게스트 내 백업 클라이언트를 사용 안함으로 설정할 수 있을 때까지 임시로 발생합니다.

단일 네트워크 확장의 대역폭은 이론적으로 4Gbps입니다. 그러나 이 값은 단일 네트워크 확장 쌍에 있는 모든 확장 네트워크의 한계일 수 있으며 단일 확장 네트워크에서 달성할 수 없습니다. 언더레이 대역폭이 충분히 할당되어 있으며 대기 시간이 낮은 경우(<~10ms) 단일 확장 네트워크는 ~1Gbps를 달성할 수 있습니다.

### 근접 라우팅 옵션
{: #hcxclient-vcs-mesh-deployment-stretching-prox-routing}

어떠한 유형의 라우트 최적화도 없이 확장된 네트워크는 모든 L3 액세스를 위해 클라이언트 측으로 다시 라우팅됩니다. 이 Trombone-ing에서는 패킷이 클라이언트(소스)와 클라우드 사이를 왔다 갔다 이동해야 하므로 트래픽 패턴이 비효율적이게 됩니다. 이는 소스 및 대상 VM이 모두 클라우드에 있는 경우에도 해당됩니다. HCX의 근접 라우팅 기능은 이 문제와 트래픽의 로컬 유출을 해결하도록 설계되었습니다.

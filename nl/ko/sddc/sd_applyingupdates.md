...
2. VMware 컴포넌트 업데이트를 포함하는 업데이트 중에 유지보수 모드로 전환하기 위해 VM이 ESXi 서버에서 마이그레이션되어야 할 수 있습니다. VM에 로컬 데이터 저장소 또는 마운트된 CD-ROM이 있는 경우 VM 마이그레이션이 수행되지 않을 수 있습니다.
...
5. 업데이트 상태에 대한 세부사항은 다음 표를 참조하십시오.

...
초기 클러스터가 vSAN인 경우 추가 vSAN 클러스터는 초기 vSAN 클러스터와 동일한 vSAN 라이센스를 사용하고 동일한 구성을 갖습니다. 인스턴스의 클러스터가 클러스터(초기 또는 추가)에 배치되도록 선택된 vSAN을 보유하고 있는 경우에도 true입니다. 처음에는 vSAN 라이센스 및 에디션에 대한 프롬프트가 표시됩니다. 그 다음에는 추가 클러스터를 위한 vSAN을 선택할 때 초기에 선택한 vSAN이 재사용됩니다.
...
6. 스토리지 구성을 완료하십시오.
  * **vSAN 스토리지**를 선택한 경우 **vSAN 용량 디스크의 디스크 유형 및 크기**, **vSAN 용량 디스크 수** 및 VMware vSAN 라이센스 에디션을 선택하십시오.
  * **NFS 스토리지**를 선택한 경우 **공유 수**, **크기** 및 **성능**을 선택하십시오.
...
8. 클러스터를 추가하기 전에 **주문 요약** 페이지에서 클러스터 구성을 확인하십시오.
   1. 클러스터의 설정을 검토하십시오.
   2. 클러스터의 예상 비용을 검토하십시오. PDF 요약을 생성하려면 **가격 책정 세부사항**을 클릭하십시오. 주문 요약을 저장하거나 인쇄하려면 PDF 창의 오른쪽 상단 모서리에 있는 **인쇄** 또는 **다운로드** 아이콘을 클릭하십시오.

...
vCenter 내의 VMware Update Manager 기능은 ESXi 호스트를 6.7u1 레벨로 업그레이드하고 패치하는 데 활용됩니다. 이 문서의 NSX 업그레이드 절과 유사하게 vMotion을 다른 호스트로 마이그레이션하지 않거나 문제 없이 종료되지 않는 VM으로 인해 업그레이드 프로세스가 정지될 수 있습니다.
{:note}
...
1. vCenter 사용자 인터페이스에서 업그레이드할 ESXi 호스트가 포함된 클러스터의 이름으로 이동하십시오. 
2. **수정**을 클릭하십시오.
3. 선택 상자의 왼쪽에서 **기준선 업그레이드**를 선택한 후 선택 화면의 오른쪽에서 **VMware ESXi 6.7.0 U1** 기준선을 선택하십시오. **다음**을 클릭하십시오. 참고: ISO 이미지가 VUM 저장소에 업로드될 때 ESXi 6.7.0 U1의 VUM 기준선이 작성되었습니다. 
4. 업그레이드할 호스트를 선택하고 **다음**을 클릭하십시오.
5. EULA를 허용하는 옵션을 선택하고 **다음**을 클릭하십시오.
6. 고급 옵션 화면에서 **다음**을 클릭하고 기본값을 허용하십시오. 
7. 호스트 조치방안 설정 화면에서 선택적으로 이동식 매체를 사용 안함으로 선택하고 **다음**을 클릭하십시오.
8. 클러스터 조치방안 옵션 화면에서 **다음**을 클릭하여 기본값을 허용하십시오. 
9. 완료 준비 화면에서 **조치방안 사전 확인**을 클릭하십시오. HA 허가 제어에 대한 경고가 리턴됩니다.   
10. **완료**를 클릭하여 호스트 업그레이드를 시작하십시오. 
...
1. vCenter 사용자 인터페이스에서 패치될 호스트가 포함된 클러스터를 선택하고 **Update Manager** 탭을 선택하십시오. 
2. **수정**을 선택한 후 **중요하지 않은 호스트 패치**를 선택하십시오.
3. 이전에 ESXi 업그레이드에서 수행한 대로 계속 진행하십시오. 이 프로세스의 일부로 다시 Zerto ZVRA VM을 종료해야 할 수 있습니다. 
...
업그레이드가 완료되면 IBM 지원 센터를 통해 지원 티켓을 업데이트하십시오. 그러면 IBM 지원 센터는 {{site.data.keyword.vmwaresolutions_short}} 콘솔을 업데이트하여 6.7 레벨에서 {{site.data.keyword.vmwaresolutions_short}} 자동화를 재개합니다. 여기에는 서비스, 호스트, 클러스터 및 보조 vCenter 서버 인스턴스의 추가 및 제거가 포함됩니다. 

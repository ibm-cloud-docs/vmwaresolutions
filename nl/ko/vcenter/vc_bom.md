...
{: caption="표 1. vCenter Server 인스턴스의 VLAN에 대한 BOM" caption-side="top"}
...
{: caption="표 3. vCenter Server 인스턴스 및 클러스터에 대한 ESXi 서버 고급 구성 설정" caption-side="top"}
...
* 구성 설정에 대한 **설정되지 않음** 값은 새 설정이 자동으로 적용되지 않음을 나타내며, ESXi 서버를 다시 부팅해야 하므로 중단이 발생할 수 있습니다.
  ...
  자세한 정보는 [Increasing the default value that defines the maximum number of NFS mounts on an ESXi host](https://kb.vmware.com/s/article/2239){:external}를 참조하십시오.
...
{: caption="표 4. vCenter Server 인스턴스에 대한 NSX 및 포트 그룹 구성 설정" caption-side="top"}
...
{: caption="표 5. 인스턴스 버전에 따른 vCenter Server 인스턴스 및 클러스터의 MTU 구성 설정" caption-side="top"}
...
* [빌드 번호 및 VMware ESXi와 ESX(2143832)의 버전](https://kb.vmware.com/s/article/2143832){:external}
* [빌드 번호 및 VMware vCenter Server(2143838)의 버전](https://kb.vmware.com/s/article/2143838){:external}

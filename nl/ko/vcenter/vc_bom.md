...
|포트 그룹 SDDC-DPortGroup-Mgmt |**포트 바인딩**이 **Ephermeral - 바인딩 없음**으로 설정되고 **로드 밸런싱**이 **원래 가상 포트를 기반으로 한 라우트**로 설정됨 |**포트 바인딩**이 **정적 바인딩**으로 설정되고 **로드 밸런싱**이 **실제 NIC 로드를 기반으로 한 라우트**로 설정됨 |  

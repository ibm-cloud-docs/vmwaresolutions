...
4. 왼쪽 탐색 분할창에서 **업데이트 및 패치**를 클릭하십시오.
   ...
   **업데이트 및 패치** 페이지에는 VMware 업데이트가 아닌, IBM 관리 컴포넌트 업데이트를 위한 패키지만 포함되어 있습니다. {{site.data.keyword.vmwaresolutions_short}}에서는 다음 조작에 대한 VMware 업데이트를 적용합니다.
...
5. 라이센스 업그레이드의 경우, **업그레이드**를 클릭하십시오. 목록에서 어느 에디션으로 업그레이드할지 선택하고 **업그레이드**를 클릭하십시오. 라이센스 에디션 다운그레이드는 사용할 수 없습니다.

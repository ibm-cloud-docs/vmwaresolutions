...
4. 인스턴스 유형을 선택하십시오.
   ...
   * 고가용성을 위해 인스턴스를 환경의 기존(기본) 인스턴스에 연결하려면 **보조 인스턴스**를 클릭하고 다음 단계를 완료하십시오.
     ...
     2. 선택한 기본 인스턴스가 V2.5 릴리스로 업그레이드되거나 기본 인스턴스가 V2.4 및 이전 릴리스로
     배치되거나 업그레이드된 경우 미리 채워진 **기본 인스턴스 PSC의 관리자 비밀번호**가
     올바른지 확인하십시오.
     
        **참고:** V2.5 이상 릴리스에 배치된 기본 인스턴스에서는 **기본 인스턴스 PSC의 관리자 비밀번호** 필드를 사용할 수 없습니다.     
...
인스턴스가 성공적으로 배치된 경우에는 [vCenter Server 인스턴스의 기술 스펙](vc_vcenterserveroverview.html#technical-specifications-for-vcenter-server-instances)에서 설명된 컴포넌트가 VMware 가상 플랫폼에 설치됩니다. 기본적으로 주문한 ESXi 서버는 **cluster1**로 그룹화됩니다. 추가 서비스를 주문한 경우 주문이 완료된 후 서비스의 배치가 시작됩니다.

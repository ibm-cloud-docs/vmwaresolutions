...
* VMware vSphere Web Client에서 ESXi 서버를 추가하거나 제거하지 마십시오. vSphere Web Client에서 수행하는 변경사항은 {{site.data.keyword.vmwaresolutions_short}} 콘솔과 동기화되지 않습니다.
...
2. **vCenter Server 인스턴스** 테이블에서 용량을 확장하거나 축소할 인스턴스를 클릭하십시오.
3. 왼쪽 탐색 분할창에서 **인프라**를 클릭하십시오.
4. **클러스터** 테이블에서 ESXi 서버를 추가하거나 제거할 클러스터를 클릭하십시오.
5. ESXi 서버를 추가하려면 다음 단계를 완료하십시오.
   1. **ESXi 서버** 테이블에서 **추가**를 클릭하십시오.
   2. **서버 추가** 창에서 추가할 서버의 수를 입력하고, 가격 링크를 클릭하여 예상 비용을 검토한 후 **추가**를 클릭하십시오.
6. ESXi 서버를 제거하려면 **ESXi 서버** 테이블에서 제거할 서버를 선택한 후 **제거**를 클릭하십시오.
...
ESXi 서버를 추가하거나 제거하는 요청이 처리 중이라는 알림을 이메일로 받습니다. ESXi 서버와 연관된 클러스터의 상태가 **수정 중**으로 변경되었습니다. 서버를 제거하는 경우 일반적으로 30일인 {{site.data.keyword.cloud_notm}} 인프라 비용 청구 주기 종료 시 ESXi 서버가 {{site.data.keyword.cloud_notm}} 인프라에서 완전히 재확보되는 점에 유의하십시오.

...
## vCenter Server 인스턴스에 업데이트 및 패치 적용
...
4. 왼쪽 탐색 분할창에서 **업데이트 및 패치**를 클릭하십시오.
   ...
   **참고**: 인스턴스의 **업데이트 및 패치** 페이지는 IBM 관리 컴포넌트를 업데이트하기 위한 패키지만 포함하며, VMware 업데이트는 포함하지 않습니다. VMware 업데이트는 수동으로 적용해야 합니다.
...
## vCenter Server with Hybridity Bundle 인스턴스로 업그레이드
...
**참고:** Hybridity Bundle로 업그레이드하고 vCenter Server 인스턴스에 이미 NFS 파일 스토리지가 있는 경우 VMware vSAN 스토리지에 대한 비용이 청구되지 않습니다. vSAN 라이센스가 Hybridity Bundle에 포함되어 있으므로 이 라이센스에 대한 비용은 청구됩니다.

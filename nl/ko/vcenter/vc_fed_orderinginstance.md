...
인스턴스가 호스팅되는 IBM Cloud Data Center를 선택하십시오.
...
스토리지 설정은 vSAN, NFS 또는 사용자 정의 NFS 스토리지 선택에 따라 달라집니다.
...
7. 스토리지 구성을 완료하십시오.
      * **vSAN 스토리지**를 선택한 경우에는 **vSAN 용량 디스크의 디스크 유형 및 크기**, **vSAN 용량 디스크 수** 및 VMware vSAN 라이센스 에디션을 선택하십시오.
      * **NFS 스토리지**를 선택한 경우에는 **공유 수**, **크기** 및 **성능**을 선택하십시오.
...
인스턴스가 배치되면 [VMware Federal 인스턴스 컴포넌트](../vcenter/vc_fed_overview.html#vcenter-server-instance-components-for-vmware-federal-on-ibm-cloud)에 설명되어 있는 컴포넌트가 VMware 가상 플랫폼에 설치됩니다. 기본적으로 주문한 ESXi 서버는 **cluster1**로 그룹화됩니다.
...
**중요:** {{site.data.keyword.slportal}} 또는 콘솔 외부의 다른 방법이 아닌 {{site.data.keyword.vmwaresolutions_short}} 콘솔의 {{site.data.keyword.cloud_notm}} 계정에서만 작성된 {{site.data.keyword.vmwaresolutions_full}} 컴포넌트를 관리해야 합니다.

...
이 릴리스에서는 선택된 VMware 컴포넌트를 기반으로 VMware 호환 가능 컴퓨팅, 스토리지 및 네트워크 리소스를 사용자 정의하고 주문하여 고유한 IBM 호스팅 VMware 가상 환경을 빌드할 수 있는 VMware vSphere on IBM Cloud 오퍼링을 도입했습니다. vSphere on IBM Cloud가 선택적 VMware 컴포넌트의 설치, 구성 및 가져오기를 자동화하지 않지만 사용자는 비즈니스 요구사항에 가장 적합한 환경을 디자인하고 설계할 수 있는 최대한의 유연성을 갖고 있습니다. ESXi 서버의 새 vSphere 클러스터를 작성하거나 IBM Cloud 데이터 센터에서 기존 vSphere 클러스터를 스케일링하여 시작할 수 있습니다.
...
이 릴리스에서는 IBM Cloud의 데디케이티드 {{site.data.keyword.baremetal_short}}의 서비스로 NetApp ONTAP Select를 구현하는 소프트웨어 정의 스토리지에 대한 가상 어플라이언스인 NetApp ONTAP Select on IBM Cloud 오퍼링을 도입했습니다. NetApp ONTAP Select on IBM Cloud는 고성능(모든 SSD) 및 고용량(모든 SATA)의 두 가지 구성으로 제공됩니다.
...
F5 BIG-IP Virtual Edition(VE) on IBM Cloud 서비스는 이제 VMware Cloud Foundation 및 VMware vCenter Server 인스턴스 모두에 사용할 수 있습니다. 이 서비스는 로컬 및 글로벌 스케일의 인텔리전트 L4-L7 로드 밸런싱 및 트래픽 관리 서비스, 강력한 네트워크 및 웹 애플리케이션 방화벽 보호, 안전하고 연합된 애플리케이션 액세스를 제공합니다.
인스턴스를 주문할 때 포함된 F5 BIG-IP Virtual Edition(VE) on IBM Cloud 서비스와 함께 인스턴스를 주문하거나 {{site.data.keyword.vmwaresolutions_short}} 콘솔의 인스턴스 특성 세부사항 페이지에 있는 **서비스** 탭에서 나중에 이 서비스를 기존 인스턴스에 추가할 수 있습니다. 사용자의 요구사항에 따라 BIG-IP VE에 대한 세 개의 라이센스 부여 옵션 중 하나를 선택할 수 있습니다.

자세한 정보는 다음을 참조하십시오.
* [F5 on IBM Cloud에 대한 고려사항](../services/f5_considerations.html)
* [F5 on IBM Cloud 관리](../services/managing_f5.html)
...
호스팅된 VMware 환경의 스케일링을 개선하려면 이제 인스턴스에 배치된 초기 클러스터가 아닌 다른 {{site.data.keyword.cloud_notm}} 인프라(SoftLayer) 팟 또는 다른 IBM Cloud 데이터 센터에 새 클러스터를 작성할 수 있습니다.
...
* 공용 IBM Cloud 서비스를 사용하기 위해 VMware 애플리케이션을 확장하는 동안 사설 다중 사이트 VCF 인스턴스를 보호하는 방법을 알아보십시오. 자세한 정보는 [Securely connect your private VMware workloads in the IBM Cloud](https://www.ibm.com/developerworks/library/se-securely-connect-private-vmware-workloads-ibm-cloud/index.html){:new_window}를 참조하십시오.
* 추가 문서는 IBM CloudDriver 및 SDDC Manager 가상 머신에서 모든 프로토콜 통신을 허용하는 방화벽을 구성하기 위해 제공됩니다. 자세한 정보는 [Fortinet on IBM Cloud에 대한 컴포넌트 및 고려사항](../services/fsa_considerations.html)을 참조하십시오.

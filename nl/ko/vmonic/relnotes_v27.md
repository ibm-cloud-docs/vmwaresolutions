...
이제 VMware 인스턴스 이벤트 외에도 키 작성, 키 삭제 및 키 액세스 등의 KMIP for VMware on {{site.data.keyword.cloud_notm}}에 대한 이벤트가 {{site.data.keyword.cloud_notm}} Activity Tracker 인스턴스와 통합됩니다.
...
* **유형** 열은 이제 vCenter Server, vCenter Server with Hybridity Bundle 및 vCenter Limited Test Drive 인스턴스를 식별하기 위해 **리소스** 페이지의 **vCenter Server 인스턴스** 테이블에 포함됩니다.

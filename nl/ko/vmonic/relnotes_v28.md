...
자세한 정보는 [마이그레이션 및 앱 현대화를 위한 단일 노드 평가판 개요](../vcenter/cloud_modern_bundle_overview.html)를 참조하십시오.
...
또한 새 인스턴스를 작성하거나 NFS 스토리지를 기존 인스턴스에 추가할 때 0.25IOPS/GB를 선택할 수 있는 옵션이 제공됩니다. 

자세한 정보는 [vCenter Server 인스턴스에 대한 용량 확장 및 축소](../vcenter/vc_addingremovingservers.html#adding-nfs-storage-to-vcenter-server-instances)의 *NFS 스토리지를 vCenter Server 인스턴스에 추가* 및 *vCenter Server 인스턴스에서 NFS 스토리지 제거* 섹션을 참조하십시오. 
...
자세한 정보는 다음의 *{{site.data.keyword.baremetal_short_sing}} 설정* 섹션을 참조하십시오.
* [vCenter Server 인스턴스 주문](../vcenter/vc_orderinginstance.html#bare-metal-server-settings)
* [vCenter Server with Hybridity Bundle 인스턴스 주문](../vcenter/vc_hybrid_orderinginstance.html#bare-metal-server-settings)
...
자세한 정보는 [vCenter Server 개요](../vcenter/vc_vcenterserveroverview.html#vcenter-server-architecture)에서 *vCenter Server 아키텍처* 섹션을 참조하십시오.
...
이전에는 KMIP for VMware on {{site.data.keyword.cloud_notm}} 서비스가 포함된 Cloud Foundation 또는 vCenter Server 인스턴스를 주문하거나 초기 배치 후 서비스를 기존 Cloud Foundation 또는 vCenter Server 인스턴스에 추가할 수 있었습니다. V2.8 릴리스부터 서비스는 VMware 인스턴스와 연관되지 않고 독립형 서비스로 사용할 수 있습니다. 서비스의 각 인스턴스는 하나 이상의 Cloud Foundation 인스턴스, vCenter Server 인스턴스 또는 vSphere 클러스터를 제공할 수 있습니다. 

자세한 정보는 [KMIP for VMware on IBM Cloud 개요](../services/kmip_standalone_considerations.html)를 참조하십시오.

## 참조 아키텍처 문서

KMIP for VMware on {{site.data.keyword.cloud_notm}} 솔루션 아키텍처에 대한 새 기술 문서는 사용자 문서의 *참조* 섹션에서 할 수 있습니다. 자세한 정보는 [KMIP for VMware 개요](../archiref/kmip/overview.html)를 참조하십시오.

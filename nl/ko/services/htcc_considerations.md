...
각 HTCC 어플라이언스 쌍은 NSX Manager, vCenter Server Appliances 및 Platform Services Controller와 같은 관리 가상 머신(VM)에 대해 지정된 개인용 포터블 서브넷과 동일한 서브넷에 배치됩니다.

어플라이언스의 쌍은 vSphere 호스트의 프록시, vCenter Server Appliance 및 인스턴스의 NSX Manager 역할을 합니다. 따라서 사용자는 {{site.data.keyword.cloud}}에서 지정한 실제 IP(RIP) 주소 대신 관리자가 지정한 공개 IP(PIP) 주소를 통해 vSphere 호스트, vCenter Server Appliance 및 NSX Manager에 액세스합니다. 

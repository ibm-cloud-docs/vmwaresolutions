...
서비스가 포함된 새 인스턴스를 주문할 때 또는 기존 인스턴스에 서비스를 추가하여 IBM Spectrum Protect Plus on {{site.data.keyword.cloud}} 서비스를 주문할 수 있습니다.
...
* IBM Spectrum Protect Plus의 라이센스를 주문하려면 **라이센스 주문** 탭에 **라이센스를 부여할 VM 수**를 지정하십시오. 라이센스 관리에는 최소 10개의 가상 머신(VM)이 필요합니다. 
...
2. 지정한 설정에 따라, {{site.data.keyword.cloud_notm}} 인프라에서 IBM Spectrum Protect Plus에 대한 여러 라이센스가 주문됩니다. IBM Spectrum Protect Plus on {{site.data.keyword.cloud_notm}} 서비스를 주문할 때 라이센스를 부여하도록 지정한 VM의 수에 따라 10개 VM 라이센스 팩 단위로 라이센스가 주문됩니다. 기존 IBM Spectrum Protect Plus 라이센스를 가져오도록 선택한 경우에는 {{site.data.keyword.cloud_notm}} 인프라에서 라이센스가 주문되지 않습니다. 
...
5. IBM Spectrum Protect Plus VM이 인스턴스의 기본 클러스터에 배치되어 활성화되고 구성됩니다. 
...
8. (V2.3 이상 인스턴스의 경우) IBM Spectrum Protect Plus에서 기본 관리 백업 작업이 작성됩니다. 자세한 정보는 [IBM Spectrum Protect Plus on IBM Cloud 관리](managingspp.html)를 참조하십시오.

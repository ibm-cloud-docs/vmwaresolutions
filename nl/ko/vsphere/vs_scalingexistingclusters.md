...
ESXi 서버를 추가하거나 기존 VMware vSphere 클러스터를 위한 FortiGate 300 Series Security Appliance HA 쌍을 주문하여 {{site.data.keyword.vmwaresolutions_full}} 콘솔에서 주문하거나 저장한 기존 VMware vSphere 클러스터를 스케일 확장할 수 있습니다.
...
클러스터 스케일링 배치가 자동으로 시작되며, 주문이 처리 중이라는 이메일 확인을 수신합니다. 클러스터를 사용할 준비가 되면 이메일로 알림을 받습니다.

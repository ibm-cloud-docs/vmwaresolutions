...
KMIP for VMware on {{site.data.keyword.cloud_notm}} utiliza el servicio IBM Key Protect para IBM Cloud para crear, cifrar y descifrar claves de cifrado. Por lo tanto, antes de instalar el servicio, asegúrese de que haya solicitado un servicio Key Protect que pueda utilizar. Mientras tanto, se ha creado un ID de servicio de {{site.data.keyword.cloud_notm}} siguiendo los pasos del apartado [Creación de un ID de servicio](https://console.bluemix.net/docs/iam/serviceid.html#creating-a-service-id) para acceder a la instancia del servicio Key Protect que ha creado.

Asegúrese de otorgar los siguientes niveles de acceso al ID de servicio:
...
La clave de API para el ID de servicio creado es necesaria al solicitar el servicio.

El servicio da por supuesto que ya ha creado al menos una clave raíz de cliente (CRK) desde la interfaz de usuario de Key Protect siguiendo los pasos del apartado [Creación de claves raíz](https://console.bluemix.net/docs/services/keymgmt/keyprotect_create_root.html#create_root_keys) o utilizando la API RESTful de [IBM Key Protect](https://console.bluemix.net/apidocs/639-ibm-key-protect).

No puede solicitar el servicio sin CRK. Se recomienda utilizar el método para crear una CRK utilizando el material de claves existente y hacer una copia de seguridad del material de claves que se crea. De este modo se asegura de que puede recuperar sus claves en el caso de que se produzca un fallo total del centro de datos en el que se aplica IBM Key Protect para almacenar los CRK.

## Consideraciones sobre la utilización de KMIP for VMware on IBM Cloud

* Para utilizar un servicio KMIP for VMware on {{site.data.keyword.cloud_notm}} solicitado como Servidor de gestión de claves (KMS) registrado ante VMware vCenter Server, debe asegurarse de que las configuraciones se hayan realizado de modo que la conectividad de red entre vCenter Server y el punto final del servicio KMIP for VMware on {{site.data.keyword.cloud_notm}} solicitado resulte funcional.
* Para utilizar el servicio para el cifrado de VMware vSAN, debe asegurarse de que la conectividad de red entre los hosts del vSAN de destino y el punto final del servicio KMIP for VMware on {{site.data.keyword.cloud_notm}} resulte funcional.
...
Por lo tanto, antes de eliminar el servicio, debe asegurarse de que no se cifre ninguna máquina virtual ni vSAN mediante las claves creadas por el servicio KMIP.

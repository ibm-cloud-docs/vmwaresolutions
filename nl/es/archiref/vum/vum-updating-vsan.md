...
* Realice una carga de prueba y valide que la carga ha funcionado.

El primer paso consiste en añadir las credenciales de my.vmware.com al motor de recomendaciones de compilación vSAN. Después de un inicio de sesión satisfactorio, vSAN generará un grupo de línea base de actualizaciones recomendadas para cada clúster vSAN. Las líneas base de sistema vSAN se muestran en el panel de líneas base del separador Líneas y grupos.
...
Tenga en cuenta que ha habido informes en los que parece que la información de proxy solo se establece para HTTP pero no para HTTPS. Para poder configurar la información de proxy también para el tráfico HTTPS, debe habilitarse en primer lugar. Después de iniciar sesión en el VCSA mediante SSH, utilice el mandato proxy.get para ver la configuración y confirme que los parámetros HTTPS no están establecidos.
...
2. Pulse el botón **Volver a probar** y espere a que se complete el proceso.
3. Aparecerá una nueva comprobación en Estado denominada _"Conectividad de estado en línea"_ y el botón Habilitar estado en línea cambia a Volver a probar con el estado en línea.
4. Pulse el botón **Volver a probar con Estado en línea** para desencadenar la primera carga y esperar a que se complete el proceso, revisando el estado en el panel Tareas recientes. El nombre de la prueba debe cambiar a Estado en línea (última comprobación: ahora mismo).
...
7. Pulse el **separador Update Manager**. Ahora debería ver que el clúster vSAN se ha añadido a las líneas base.

## Requisitos previos

Antes de iniciar el proceso de actualización de vSAN, asegúrese de que se cumplan los requisitos siguientes:
...
Para obtener más información, consulte [Actualización de VCSA y vCenters enlazados con SSO](vum-updating-vcsa.html).
...
3. Especifique la contraseña del administrador que se proporciona en la consola de ICVS. Ahora estará en la raíz del sistema de archivos virtual, escriba ls y, a continuación, pulse **Intro**, debería ver:
  `0 /
  1 localhost/``

5. Escriba `cd 1`, especifique y, a continuación, ` ls ` y pulse **Intr **. Debería ver: `0/datacenter1 (centro de datos)`

6. Escriba `cd 0`, especifique y, a continuación, `ls` y pulse **Intro**. Debería ver:

  `0 storage/  1 computers [host]/  2 networks [network]/  3 datastores [datastore]/  4 vms [vm]/`

7. Escriba `cd 1`, pulse **Intro** y, a continuación, `ls` y pulse **Intro**. Debería ver el clúster: `0 cluster1 (clúster)` `

8. Ahora utilizaremos los mandatos VSAN para este clúster. Para comprobar el tipo de estado de disco escriba `vsan.disks_stats 0 ` y pulse **Intro**.

...
為了確保安全，已設定防火牆規則，僅容許管理虛擬機器所起始的出埠 HTTPS 通訊。這個 ESG 部署在「大型」配置中，只有「IBM 支援中心」才能修改此配置。如需相關資訊，請參閱下列主題：

* [vCenter Server 技術規格](/docs/services/vmwaresolutions/vcenter?topic=vmware-solutions-vc_vcenterserveroverview#specs)
...
* 對於 Cloud Foundation 實例，您可以在實例詳細資料頁面上，檢視各種實例元件的使用者名稱及密碼。

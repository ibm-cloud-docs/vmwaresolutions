...
如果由於授權問題而導致移轉失敗，則現行版本的 HCX 會在 vCenter 使用者介面中用戶端 Web 使用者介面的錯誤訊息中明確顯示此問題。
...
3. 將目錄切換至 `/opt/vmware/bin`，並執行 `./ccli` 指令。如果由於環境不是針對 root 使用者設定的而導致此嘗試失敗，請執行 `./ccliSetup.pl` 指令。
4. 在 `ccli` Shell 內執行 `list` 指令來列出向 HCX Manager 登錄的機群元件。
5. 輸入針對機群元件列出的 ID，來指定 `ccli` 的機群 ID。例如，`go 8`。
6. 執行 `debug remoteaccess enable` 指令，以使用 SSH 連接至所需的機群元件。

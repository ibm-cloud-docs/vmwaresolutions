...
請檢閱下列配置 HCX 用戶端實例的步驟。

## 為 IBM Cloud for VMware Solutions 環境進行網站配對
{: #hcxclient-vcs-mesh-deployment-sitepair}

1. 登入 VMware vSphere Web Client。
2. 從**首頁**功能表，選取 **HCX** 選項。
3. 在**基礎架構**、**InterConnect** 下，按一下**新增網站配對**。
  1. 將網站 URL 設為 HCX Cloud Manager URL，例如 `https://x.x.x.x.x`。
  2. 將使用者名稱及密碼設為 HCX Manager 管理詳細資料：admin/password。

    您可以從 IBM Cloud from VMware Solutions 主控台的**服務**、vCenter Server 實例的 **HCX on IBM Cloud** 下取得先前的詳細資料。
...
2. 從**首頁**功能表，選取 **HCX** 選項。
3. 在**基礎架構**下，按一下 **InterConnect**。
4. 在**多站台服務網**下，按一下**網路設定檔**。
5. 從**建立網路設定檔**：
   1. 選取「分散埠群組」，例如，「外部」。
   2. 提供**外部 IP** 的 IP 位址範圍、**外部子網路字首長度**、**外部閘道**和 **DNS 詳細資料**。
   3. 將 MTU 設定為 1500。
   4. 按一下**建立**。
...
5. 從**建立服務網**：
   ...
   8. 提供「服務名稱」，按一下**完成**。
6. 觀察建立服務網的作業清單。成功完成後，內部部署位置會有三個 HCX 應用裝置，並且雲端位置也會有三個 HCX 應用裝置。
...
橋接用戶端網路至雲端 VXLAN 的中介，是一個複雜的多通道 VPN，其包含專利的 HCX 技術。它不是以 NSX 為基礎，但會與 NSX 搭配運作並延伸其功能。此處理程序是由用戶端 vCenter Web 使用者介面所控制，並自動部署且同時啟動用戶端和雲端的兩個端點。選取延伸的網路可個別執行或批次處理。

此外，在網路延伸工作流程中，雲端上的 NSX 獲得授權建置 VXLAN。VXLAN 之後將連接至所指定雲端 L3 裝置上建立的介面（DLR 或 ESG 處於未連接狀態），以及雲端網路延伸應用裝置。
...
理論上，單一網路延伸的頻寬為 4 Gbps。不過，此值會是針對單一網路延伸配對中所有延伸網路的限制，單一延伸網路並無法達到此頻寬。假設有分配到足夠的基礎頻寬，且延遲很低（<~10 毫秒），則單一延伸網路可以達到 ~1 Gbps。

### 近似性遞送選項
{: #hcxclient-vcs-mesh-deployment-stretching-prox-routing}

如果沒有任何類型的遞送最佳化，則延伸網路遞送回到用戶端以存取任何 L3。這樣的轉接會產生沒有效率的資料流量模式，因為封包需要在用戶端（來源）與雲端之間來回傳送。即使來源與目的地 VM 都在雲端內的情況下也是如此。HCX 的「近似性遞送」特性是設計來處理這項以及資料流量的本端出口。

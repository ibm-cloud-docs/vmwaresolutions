...
![管理流程圖](../../images/opsmgmt-mgmtflows.svg "管理流程圖"){: caption="圖 1. 作業工具管理流程" caption-side="bottom"}
...
* 記載 - vRealize Log Insight (vRLI) 提供即時日誌管理和日誌分析，具有以機器學習為基礎的智慧型分組、高效能搜尋，以及在 {{site.data.keyword.vmwaresolutions_short}} 架構中跨實體和虛擬物件進行疑難排解。vRLI 會使用 syslog 通訊協定從 vSphere 主機收集資料，以及使用其他 VMware 產品（例如 vCenter Server）來收集事件、作業及警示資料。它會與 vRops 整合，以便傳送通知事件和啟用在環境定義中啟動。{{site.data.keyword.vmwaresolutions_short}} 架構中能夠傳送 syslog 資料的其他物件會指向 vRLI。用戶端可以選擇性地配置能夠傳送 syslog 資料的任何系統，以便將此資料轉遞給 vRLI。

* 網路性能 - vRealize Network Insight 是聚焦於主動啟用下列各項的分析工具：
  * 網路性能及效能監視。
  * 端對端疑難排解。
  * 360° 可見性和分析。
  * 以微斷詞法為基礎的合規性管理。
...
![網路圖](../../images/opsmgmt-network.svg "網路圖"){: caption="圖 2. 作業工具網路" caption-side="bottom"}

* 會佈建工具專用可攜式子網路，以提供用於工具 VM 起始佈建的 {{site.data.keyword.cloud_notm}} IP 位址空間，但之後客戶必須負責管理 IP 位址空間，以便橫向擴充工具。這個子網路上的 VM 需要能存取「管理」和「內部管理」子網路上所管理的元件。
...
vROps 會從環境中的物件收集資料。收集到的每一項資料，都稱為「度量值觀察」，或是「值」。vROps 使用 vCenter 配接器來從 vCenter 收集原始度量值。除了它收集的度量值之外，vROps 還會計算容量度量值、徽章度量值，以及度量值，以監視系統的性能。警示定義是症狀及建議的組合，用來識別問題區域並產生那些區域的警示，供您採取行動。
...
為了監視 vSAN，會使用 vROps 及 vRLI。在 vCenter 中，有額外的一組 vSAN 性能檢查可以使用。安裝 Management Pack for vSAN 能提供額外的儀表板，以輔助監視 vSAN。
...
這可讓系統管理者監視、管理及疑難排解 VMware NSX。vROps Management Pack for VMware NSX 提供網路拓蹼的可見性。NSX 儀表板提供 NSX 環境及其元件性能的快速概觀。NSX 物件與 vSphere 物件之間的相關性可讓您輕鬆進行疑難排解。

vROps 使用管理套件來輪詢 VMware NSX 以取得配置、效能及支援資料。Management Pack 會代表 vROps，將輪詢要求轉換成 REST API 呼叫，以從 NSX Manager 擷取所需的資料。
...
VMware SDDC Health Management Pack 會監視下列項目：
* 邏輯交換器 - 監視邏輯交換器的管理狀態。

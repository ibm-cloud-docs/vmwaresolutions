...
本文件聚焦在設計的「作業管理」層上，不過，部分企業可能想要整合此層與其「服務管理」層。本節提供此整合的指引。在此設計中，vROps 是所有警示顯示的中心點。
...
* 問題單工具，用來管理突發事件、問題及變更等的問題單。此工具也是企業的「配置管理資料庫 (CMDB)」。工具能夠傳送電子郵件給作業團隊，也能傳送 SMS 訊息。
* 企業記載系統，它會從所有系統擷取日誌，並且由安全團隊管理。

既然他們擁有 vROps，他們便將藉由使用「SNMP 設陷」外掛程式的北向通知，來整合此工具。為了整合 vROps，需要以客戶事件管理環境能夠建立警示及予以強化的方式，剖析 vROps 所傳送的設陷。管理工具團隊已從 VMware 下載 VMware MIB，並已將其安裝在事件管理環境中。

根據客戶的原則，vRLI 已配置為將所有事件轉遞給企業記載系統。

客戶想要使用其現有的 OS、中介軟體及應用程式監視工具，因此他們在 {{site.data.keyword.cloud}} 中使用了 Proxy 來收集和轉遞度量值與警示。

...
本節會建立新的 VM 儲存空間原則，容許選取**確保可存取性資料撤除**模式來縮減維護時間，但也降低了當某個主機處於維護模式時另一個主機的失敗導致 VM 或應用裝置無法存取的風險。對於不接受 vSAN 主機進入維護模式而變成非備用的任何 VM 或應用裝置，在執行任何補救動作之前，請先執行下列處理程序。需要至少有 6 部主機的叢集。

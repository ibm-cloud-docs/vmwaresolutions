...
VMware Update Manager (VUM) 包括您無法編輯或刪除的預先定義基準線。您可以使用預先定義的基準線，或建立符合您準則的修補程式、延伸規格及升級基準線。您建立的自訂基準線以及預先定義的基準線可以結合至基準線群組。
...
2. 按一下您要掃描的叢集物件。
3. 移至 VUM、按一下**連接基準線**，然後選取兩個預先定義的「修補程式基準線」。按一下**確定**。

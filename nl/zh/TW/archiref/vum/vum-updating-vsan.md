...
vSAN 會產生系統基準線及基準線群組以與 VUM 搭配使用，而且您可以使用這些建議的基準線，以利用 vSAN 來更新 VCS 實例中 vSphere ESXi 主機的軟體、修補程式及延伸規格。vSAN 6.6.1 以及更新版本會針對 vSAN 叢集產生自動化建置建議。vSAN 會結合《VMware 相容性手冊》及「vSAN 版本」型錄中的資訊與已安裝 vSphere ESXi 版本的相關資訊。
...
* **必要條件** – 瞭解必要條件、處理程序及限制
* **升級 vCenter Server Appliance**。如需相關資訊，請參閱 [VCSA 更新及 SSO 鏈結的 vCenter](vum-updating-vcsa.html)。
* **升級 vSphere ESXi 主機** – 如需相關資訊，請參閱[建立基準線並連接至庫存物件](vum-baselines.html)。
* **升級 vSAN 磁碟格式** - 請參閱「升級 vSAN 磁碟格式」。升級磁碟格式是選用作業，但若要獲得最佳的結果，請升級物件以使用最新版本。磁碟內存格式會向完整 vSAN 特性集公開您的環境。

## 啟用 vSAN 線上性能工作流程

遵循本節中的作業，將會讓 vSAN 基準線可在 VUM 中使用。vSAN 6.6.1 以及更新版本會提供無縫自動更新處理程序，以確保 vSAN 叢集保持最新的可用版本，讓 VCS 實例保持下列項目的支援狀態：
* **vSAN 版本建議** - 使用《VMware 相容性手冊》、「vSAN 版本型錄」及基礎硬體配置狀態提示中的資訊自動產生。這也會包括其系統基準線中建議版本的必要驅動程式及修補程式更新。
* **vSAN 建置建議** - 確保叢集保持為現行硬體相容性狀態或更好的狀態。

確定 VCSA 是 vCenter 6.5 Patch 2 或更新版本，再繼續執行，因為這會修正一些 Proxy 使用問題。如需相關資訊，請參閱 [VCSA 更新及 SSO 鏈結的 vCenter](vum-updating-vcsa.html)。

若要查看 VUM 中的 vSAN 更新，請遵循 vSAN 線上性能工作流程。因此，「vSAN 線上性能」需要連接至 https://vcsa.vmware.com 及 http://www.vmware.com，才能執行這些線上性能檢查。為啟用「vSAN 線上性能工作流程」，我們需要：
* 配置 VCSA 來使用 Proxy。
* 配置 vSAN 來使用 Proxy。
* 啟用「客戶體驗改進計畫 (CEIP)」。
* 執行測試上傳，並驗證上傳已作用。
...
請注意，已報告似乎只針對 HTTP 設定 Proxy 資訊，但未針對 HTTPS。為了能夠同時配置 HTTPS 資料流量的 Proxy 資訊，必須先啟用它。透過 SSH 登入 VCSA 之後，請使用 proxy.get 指令來檢視配置，並確認未設定 HTTPS 參數。
...
2. 按一下**重新測試**按鈕，並等待處理程序完成。
3. 新的檢查會出現在稱為 _"Online health connectivity"_ 的「性能」中，而且「啟用線上性能」按鈕會變更為「使用線上性能重新測試」。
4. 按一下**使用線上性能重新測試**按鈕來觸發第一次上傳，並等待處理程序完成，方法是檢閱「最近作業」窗格中的狀態。「測試名稱」應該會變更為 Online health (Last check: just now)。
5. 完成時，請在「性能」視窗中捲動並展開「vSAN 建置建議」，然後按一下 **vSAN 建置建議引擎性能**。
6. 按一下**登入 my.vmware.com**，然後輸入您的認證。處理程序完成時，**測試結果**會變更為**通過**狀態。
7. 按 **Update Manager** 標籤，現在您應該會看到「vSAN 叢集」已新增至「基準線」。

## 必要條件

啟動 vSAN 升級處理程序之前，請確保符合下列需求：
* 檢閱 VMware 知識庫文章，並檢閱現行 vSAN 版本與所需目標 vSAN 版本之間的任何已知相容性問題
* **vSphere 環境是最新的**：
  - VCSA 的修補程式層次必須等於或高於 vSphere ESXi 主機的修補程式層次。更新 VCSA（如有必要）
  - 所有主機都應該執行相同的 ESXi 建置。如果不符合 vSphere ESXi 主機版本，則更新
* **所有 vSAN 磁碟都應該性能良好**：
  - 磁碟不應該失敗或不存在。這可以透過 vSphere Web Client 中的 **vSAN 磁碟管理**視圖來決定。**首頁** > **主機及叢集**，選取 **vSAN 叢集**，然後按一下 **vSAN** 標籤，再按一下**實體磁碟**。捲動所有磁碟，並檢閱「vSAN 性能狀態」。
  - 應該沒有無法存取的 vSAN 物件。這可以使用 **vSAN 性能服務**進行驗證，方法是按一下**首頁** > **主機及叢集**，然後選取 **vSAN 叢集**。按一下**監視**標籤，並按一下 **vSAN**，然後按一下**性能**。檢閱「測試結果」。
  - 在升級處理程序開始時，不應該有任何作用中重新同步，方法是按一下**首頁** > **主機及叢集**，選取 **vSAN 叢集**，然後按一下 **vSAN** 標籤，再按一下**重新同步元件**。_「重新同步元件」計數應該為 0_。請注意，在升級處理程序期間，預期會有一些重新同步活動，因為在主機重新開機之後需要同步處理資料。
* **vSphere ESXi 主機準備** - 當您將主機移至 vSAN 叢集裡的維護模式時，有三個選項可供選擇：
  - **無資料移轉** - 如果您選取這個選項，vSAN 不會從此主機撤除任何資料。如果您關閉主機電源或從叢集移除主機，則部分虛擬機器可能會變成無法存取。
  - **確保可用性** - 如果您選取這個選項，vSAN 可讓您將主機移至維護模式的速度比「完整資料移轉」還要快，並容許存取環境中的虛擬機器。
  - **完整資料移轉**
* **結束維護模式及重新同步** - vSphere ESXi 主機已升級並移出維護模式後，將會發生重新同步。您可以透過 Web 用戶端來查看此項目。請先確定此作業已完成，再移至下一部主機。已更新的主機現在可以再次提出至 vSAN 資料儲存庫時，會進行重新同步。請務必要等到此重新同步完成，以確保不會遺失任何資料。
* **啟動「vSAN 叢集」升級之後**：
...
如需相關資訊，請參閱 [VCSA 更新及 SSO 鏈結的 vCenter](vum-updating-vcsa.html)。

##	升級 vSphere ESXi 主機

如需相關資訊，請參閱[建立基準線並連接至庫存物件](vum-baselines.html)。

##	升級 vSAN 磁碟格式

「Ruby vSphere 主控台 (RVC)」是 vSphere 的 Ruby 型指令行介面，並且可以用來管理 VMware vSphere ESXi 及 vCenter。vSphere 庫存會以樹狀結構呈現，並可讓您針對 vCenter 物件導覽及執行指令。
...
3. 輸入「ICVS 主控台」上提供的「管理者」密碼。您現在將位於虛擬檔案系統的根目錄中，鍵入 ls，然後按 **Enter** 鍵，您應該會看到：
  `0 /
  1 localhost/``

5. 鍵入 `cd 1` 並按 Enter 鍵，然後鍵入 `ls` 並按 **Enter** 鍵。您應該會看到：
  `0 / datacenter1 (datacenter)`

6. 鍵入 `cd 0` 並按 Enter 鍵，然後鍵入 `ls` 並按 **Enter** 鍵。您應該會看到：
...
7. 鍵入 `cd 1` 並按 **Enter** 鍵，然後鍵入 `ls` 並按 **Enter** 鍵。您應該會看到叢集：
  `0 cluster1 (cluster)``

8. 現在，我們將針對此叢集使用 VSAN 指令。若要檢查磁碟狀態，請鍵入 `vsan.disks_stats 0`，然後按 **Enter** 鍵。

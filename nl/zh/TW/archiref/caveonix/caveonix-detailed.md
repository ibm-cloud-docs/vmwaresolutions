...
-	關聯式資料儲存庫 - 維護下列類型的 meta 資料：
    ...
    - 軟體
    - 每日或每週聚集資料集
-	傳訊資料儲存庫 - RiskForesight 使用持續傳訊佇列，以提供零資料流失，並將背壓卸載至元件。送入的資料有效負載一開始會持續保存，然後再從持續性儲存庫中取得，以供未來處理。
-	索引資料儲存庫 - 它會檢索並儲存每一個承租戶送入的原始資料以進一步分析，藉此支援多方承租戶功能。
-	外掛程式 - 位於「應用程式遞送伺服器」中，並包括 VMware 元件的設定與整合，藉此使所有 VM 連同其承租戶資訊同步化。

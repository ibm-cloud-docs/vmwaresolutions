...
有三種主要方式可以讓外部資料流量/存取進到您的 Kubernetes 叢集應用程式：
- NodePort
- LoadBalancer
- Ingress
...
2.	DNS 用來判斷 IP 位址。這個 IP 位址將會是已指派給「VCS 實例」之可攜式子網路上的 {{site.data.keyword.cloud}} 公用位址。
...
6.	如果應用程式位於同一個工作者節點上，則會使用 iptables 來判定用來轉遞要求的內部介面。如果應用程式位於不同的工作者節點上，則 Calico vRouter 會使用 IP-in-IP 封裝，遞送至適用的工作者節點。IP-in-IP 封包會封裝在 VXLAN 訊框中，以傳輸至工作者節點所在的 vSphere ESXi 主機。
...
1.	連接至企業內部網路的企業使用者向 VCS 中管理之 VM 上的資源提出要求。
...
## 公用存取網路
依預設，ICP 和 CAM 需要網際網路連線功能，才能擷取 Docker 映像檔、Helm 圖表、Terraform 範本和作業系統套件管理程式。在最新版本中，針對未直接連接至網際網路，且可選擇以離線模式進行安裝的安裝環境，現在可以支援以 Proxy 為基礎的安裝。

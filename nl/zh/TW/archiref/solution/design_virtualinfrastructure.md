...
此設計容許您透過「直接主控台使用者介面 (DCUI)」、ESXi Shell 及「安全 Shell (SSH)」來存取虛擬主機。
...
如圖 1 所示，vSAN 會將本端儲存空間聚集至 vSphere 叢集內的多部 ESXi 主機，並將聚集的儲存空間當成單一 VM 資料儲存庫管理。在此設計內，運算節點包含 ESXi OS 及 vSAN 資料儲存庫的本端磁碟機。不論節點屬於哪一個叢集，每個節點都會包含兩個 1-TB SATA 磁碟機，以存放 ESXi 安裝。

圖 1. vSAN 概念

![vSAN 概念](virtual_vSAN.svg "vSAN 會將本端儲存空間聚集至 vSphere 叢集內的多部 ESXi 主機，並將聚集的儲存空間當成單一 VM 資料儲存庫管理")
...
RAID 5 配置至少需要 4 部主機。或者，您也可以選擇 RAID 6 配置，而此配置將**失敗容錯方法**設為 **RAID-5/6（消除編碼）- 容量**，並將**主要失敗層次**設為 2。
...
vSAN 設定的設定是根據在 {{site.data.keyword.cloud_notm}} 內部署 VMware 解決方案的最佳作法。vSAN 設定包括 SIOC 設定、明確失效接手設定埠群組及磁碟快取設定。
...
|:------------- |:------------- |:------------- |:------------- |:------------- |:------------- |
| SDDC-Dswitch-Private | ESXi 管理、vSAN、vSphere vMotion、VXLAN 通道端點、NFS (VTEP) | 啟用 | 根據明確失效接手 (vSAN、vMotion) 原始虛擬埠遞送（所有其他項目）|2 | 9,000<br>（巨大訊框）|

...
這些設定適用於 2.2 版或更新版本的新實例及新實例中的新叢集。這些設定不適用於 2.1 版或更早版本現有實例中的新叢集，或升級至 2.2 版或更新版本之現有實例中的新叢集。
...
請檢閱下表，以取得 vCenter Server 實例之 VMware NSX 及埠群組配置設定的概觀，以及版本之間的差異。

這些設定適用於 2.2 版或更新版本的新實例及新實例中的新叢集。這些設定不適用於 2.1 版或更早版本的現有實例中的新叢集，或升級至 2.2 版或更新版本的現有實例中的新叢集。

表 4. vCenter Server 實例的 NSX 及埠群組配置設定

|配置設定|2.1 版或更早版本|2.2 版或更新版本|   
|:------------- |:------------- |:------------- |
...
在 2.1 版或更新版本中，公用網路連線使用標準乙太網路 MTU 1500。必須維護此設定 1500；任何變更都可能會導致透過網際網路傳送時發生封包片段化。
...
這些設定適用於部署在 2.1 版或更新版本的新實例以及實例中的新叢集。這些設定也適用於跨 {{site.data.keyword.CloudDataCents_notm}}，升級至 2.1 版或更新版本之實例的新叢集。

這些設定不適用於相同 {{site.data.keyword.CloudDataCent_notm}} 中，2.0 版或更早版本現有實例的新叢集，或升級至 2.1 版或更新版本之現有實例的新叢集。

對於已部署在 2.0 版或更早版本的實例，建議您將公用交換器 MTU 設定更新為 1500。
...
3. 確定**最大 MTU** 值已設為 1500。
   ...
   **附註**：當 vDS 中的 MTU 大小變更時，會將連接的上行鏈路（實體 NIC）卸下再重新連接。因此，使用此上行鏈路的 VM 會短暫地中斷。所以，建議您規劃在排定的關閉時間進行 MTU 設定更新。

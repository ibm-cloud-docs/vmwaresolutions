...
# 刪除 vCenter Server with NSX-T 實例
{: #vc_nsx-t_deletinginstance}

若要釋出您在 VMware vCenter Server with NSX-T 實例中訂購的元件，請刪除該實例。

當您刪除 vCenter Server with NSX-T 實例時，會循序釋出下列元件：
2. 支援費用
3. VMware 產品授權
4. ESXi 伺服器
5. 子網路
...
3. 按一下 **vCenter 主控台**旁的溢位功能表圖示，然後按一下**刪除實例**。實例的狀態會變更為**刪除中**。順利刪除實例後，會釋出該實例的元件，且該實例的狀態會變更為**已刪除**。
4. 如果您要從 {{site.data.keyword.vmwaresolutions_short}} 主控台移除實例記錄，請完成下列步驟：

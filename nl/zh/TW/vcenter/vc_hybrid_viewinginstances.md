...
檢視您已訂購的 VMware vCenter Server with Hybridity Bundle 實例及其詳細資訊。

## 檢視 vCenter Server with Hybridity Bundle 實例摘要

若要檢視環境中所有 vCenter Server with Hybridity Bundle 實例的摘要，請完成下列步驟：
...
## 檢視 vCenter Server with Hybridity Bundle 實例內容詳細資料
...
|vCenter 版本|VMware vCenter Server with Hybridity Bundle 版本。<br><br>**附註**：{{site.data.keyword.vmwaresolutions_short}} 主控台及 VMware vSphere Web Client 上所顯示的 vCenter Server 版本之間有輕微變化。兩者皆正確。|
...
|DNS、子網域|子網域是本端 vCenter Server with Hybridity Bundle 實例主機名稱所在的根網域名稱的 DNS 子網域名稱。子網域名稱的格式為 <samp class="ph codeph"><var class="keyword varname">vcenter_server_instance_name</var>.<var class="keyword varname">root.domain_name</var></samp>。|
...
## 檢視 vCenter Server with Hybridity Bundle 實例的存取資訊
...
## 檢視 vCenter Server with Hybridity Bundle 實例的部署歷程

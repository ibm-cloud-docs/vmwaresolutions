...
如果起始叢集是 vSAN 叢集，則任何其他 vSAN 叢集都使用相同的 vSAN 授權，且與起始 vSAN 叢集具有相同的配置。如果實例中的任何叢集都選擇在其上（起始或其他）部署 vSAN，則也是這種情況。第一次系統會提示您提供 vSAN 授權（BYOL 或購買）及版本。下次為新叢集選取 vSAN 時，就會重複使用最初選擇的授權。

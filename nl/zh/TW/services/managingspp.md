...
1. 使用 {{site.data.keyword.cloud_notm}} 基礎架構 VPN 或跳躍伺服器，以容許存取 IBM Spectrum Protect Plus 虛擬機器的 IP 位址。您可以在 {{site.data.keyword.vmwaresolutions_short}} 主控台的 IBM Spectrum Protect Plus on {{site.data.keyword.cloud_notm}} 服務詳細資料頁面上找到 IP 位址。
...
## 套用 IBM Spectrum Protect Plus on IBM Cloud 的修正程式

您負責將 IBM Spectrum Protect Plus on {{site.data.keyword.cloud_notm}} 服務維護為最新版本。您可以從 [IBM Spectrum Protect Plus on {{site.data.keyword.cloud_notm}} 支援中心](https://www.ibm.com/mysupport/s/topic/0TO50000000IQWtGAO/spectrum-protect-plus)頁面下載必要更新。
...
## 更新 IBM Spectrum Protect Plus 虛擬機器的作業系統

若要更新 IBM Spectrum Protect Plus 虛擬機器的作業系統，您必須以 **root** 使用者身分登入。**root** 使用者密碼與您可以在 IBM Spectrum Protect Plus on {{site.data.keyword.cloud_notm}} 服務詳細資料頁面上找到的密碼相同。

## 備份及還原已安裝 IBM Spectrum Protect Plus on IBM Cloud 之實例的管理元件

 **附註**：下列資訊適用於部署在（或升級至）2.3 版或更新版本之實例上的 IBM Spectrum Protect Plus 安裝。對於 2.2 版實例，此服務僅為工作負載虛擬機器提供資料保護。

IBM Spectrum Protect Plus on {{site.data.keyword.cloud_notm}} 服務已預先配置管理備份工作，而此管理備份工作會自動執行以使用最多七個還原點來每日備份下列管理元件：
...
如果管理元件失敗，您可以透過[與 IBM 支援中心聯絡](../vmonic/trbl_support.html)來開立問題單，要求將管理元件還原至前一個備份。

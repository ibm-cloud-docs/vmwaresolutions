...
1. 訂購 {{site.data.keyword.cloud_notm}} 基礎架構中 HCX 的三個子網路：
   * 一個專用可攜式子網路，以進行 HCX 管理。
   * 一個專用可攜式子網路，以進行 HCX 交互連接（如果針對 **HCX 交互連接類型**選取**專用網路**）。
   * 一個公用可攜式子網路，以進行 HCX 交互連接（如果針對 **HCX 交互連接類型**選取**公用網路**）。此子網路也用於使用 VMware 啟動及維護。
...
2. 訂購 VMware 中的 HCX 啟動金鑰。
3. 為 HCX 建立三個資源儲存區及 VM 資料夾，而 HCX 交互連接、本端 HCX 元件及遠端 HCX 元件需要這些資源儲存區及 VM 資料夾。
4. 針對 HCX 管理資料流量，部署及配置一組「VMware NSX Edge Services 閘道 (ESG)」：

...
部署后，最多可以创建四个集群。如果选择使用 VMware vSAN 存储器的**定制**裸机服务器配置，那么初始集群和部署后集群都需要 4 个服务器。

### 存储设置

对于存储器，可以使用 **vSAN** 或 **NFS**。
...
7. 填写存储配置。
   * 如果选择的是 **vSAN 存储器**，请指定 **vSAN 容量磁盘数**、**vSAN 容量磁盘的磁盘类型和大小**以及要如何提供 **vSAN 许可证**。
   * 如果选择的是 **NFS 存储器**，请指定**共享数**、**大小**和**性能**。

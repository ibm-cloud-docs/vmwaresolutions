...
|状态|实例的状态。状态可以是下列其中一个值：
    <br>正在创建：正在创建实例。<br>正在构建：正在配置实例。<br>可供使用：实例准备就绪，可供使用。<br>正在修改：正在修改实例。<br>失败：创建、配置或修改过程失败。<br>正在删除：正在删除实例。<br>删除错误：删除实例时发生错误。<br>已删除：实例已删除。|
{: caption="表 1. NetApp ONTAP Select 实例项" caption-side="top"}
...
|状态|实例的状态。|
{: caption="表 2. NetApp ONTAP Select 实例属性" caption-side="top"}
...
{: caption="表 3. 与 NetApp ONTAP Select 实例相关的组件的访问信息" caption-side="top"}
...
|日期|更改实例状态的日期和时间|
|摘要|更改的详细信息|
{: caption="表 4. NetApp ONTAP Select 实例部署历史记录" caption-side="top"}
...
|:---- | :-----------|
|名称|集群的名称。|
|ESXi 服务器|集群中的 ESXi 服务器数。|
|CPU|集群中 ESXi 服务器的 CPU 规范。|
|有效存储|集群中 ESXi 服务器的总磁盘容量。|
|内存|集群中 ESXi 服务器的内存总大小。|
|数据中心位置|托管集群的数据中心。与实例的数据中心位置相同。|
|pod|在其中创建集群的 pod。|
|状态|集群的状态。状态可以是下列其中一个值：
    <br>正在初始化：正在创建并配置集群。<br>正在修改：正在修改集群。<br>可供使用：集群准备就绪，可供使用。|
|操作|单击**删除**图标以删除集群。|
{: caption="表 5. NetApp ONTAP Select 集群项" caption-side="top"}
...
{: caption="表 6. NetApp ONTAP Select 集群的 ESXi 服务器详细信息" caption-side="top"}

...
|标题|描述|
|---|---|
|ESXi 命令|有关 vSphere 中的命令行界面、ESXi Shell 命令以及 vCLI（VMware® vSphere 命令行界面）命令的概述，请参阅 [Getting Started with vSphere Command-Line Interfaces](https://vdc-download.vmware.com/vmwb-repository/dcr-public/bc4fa31a-40ac-4aa9-a6a1-7171d1fff7f4/740990ee-4d65-4627-a9d4-0f046cb78aec/vsphere-esxi-vcenter-server-67-command-line-interface-getting-started-guide.pdf){:new_window}。|
...
|vSphere ESXi 主机处于非响应状态|有关对显示为“未响应”或“已断开连接”的 vSphere ESXi 主机进行故障诊断，或者对主机上在 vCenter 中显示为不可用的 VM 进行故障诊断的信息，请参阅 [ESX/ESXi 主机不响应且灰显 (1019082)](https://kb.vmware.com/s/article/1019082){:new_window}。|
...
|vSAN 重新均衡|如果磁盘在运行状况检查中报告错误，指示集群不均衡，一些磁盘的空间使用率高，而另一些磁盘的使用率却很低，那么您应该运行主动重新均衡。这将手动启动 vSAN 集群中对象的重新均衡。有关 vSAN 主动重新均衡以及此操作可能的适用情况的更多信息，请参阅 [vSAN 主动再平衡 (2149809)](https://kb.vmware.com/s/article/2149809){:new_window}。|

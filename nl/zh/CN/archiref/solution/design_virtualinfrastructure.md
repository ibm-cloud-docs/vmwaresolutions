...
此设计允许您通过直接控制台用户界面 (DCUI)、ESXi Shell 和安全 Shell (SSH) 来访问虚拟主机。
...
vSphere 集群容纳用于管理中央云以及用于计算用户工作负载资源的虚拟机 (VM)。

对于 Cloud Foundation 实例：
* 初始部署时，实例包含 4 个 ESXi 主机。
* 部署后，可将该实例向上扩展到最多 32 个 ESXi 主机。
...
* 使用其自己的 vCenter Server Appliance 部署新的 vCenter Server 或 Cloud Foundation 实例。

有关集群的更多信息，请参阅 [运行 VMware 集群的 {{site.data.keyword.cloud_notm}} 解决方案体系结构文档](https://www.ibm.com/cloud/garage/files/IBM-Cloud-for-VMware-Solutions-Multicluster-Architecture.pdf)。

## VMware vSAN 设计

在此设计中，Cloud Foundation 实例中采用了 VMware vSAN 存储器，在 vCenter Server 实例中可选择采用 VMware vSAN 存储器，从而为 vSphere 主机提供共享存储器。

如图 1 中所示，vSAN 跨 vSphere 集群内的多个 ESXi 主机聚集本地存储器，并将聚集的存储器作为一个 VM 数据存储进行管理。在此设计中，计算节点包含用于 ESXi 操作系统和 vSAN 数据存储的本地磁盘驱动器。无论节点属于哪个集群，每个节点中都包含两个 1 TB SATA 驱动器用于容纳 ESXi 安装。
...
RAID 6 配置至少需要 6 个主机。在缺省存储策略中，还启用了**重复**和**压缩**。
...
vSAN 设置是根据在 {{site.data.keyword.cloud_notm}} 中部署 VMware 解决方案的最佳实践设置的。这包括 SIOC 设置、显式故障转移设置端口组和磁盘高速缓存设置。
...
网络虚拟化提供了存在于虚拟层中的网络覆盖。这为体系结构提供了快速供应、部署、重新配置和销毁随需应变虚拟网络等功能。此设计使用 vSphere 分布式交换机 (vDS) 和 VMware NSX for vSphere 来实现虚拟联网。
...
初始部署后，{{site.data.keyword.cloud_notm}} 自动化会在初始集群中部署 3 个 NSX Controller。将从指定用于管理组件的专用 A 可移植子网中为每个控制器分配支持 VLAN 的 IP 地址。此外，此设计还会创建 VM 到 VM 反亲缘关系规则，以在集群中的各主机之间分隔控制器。初始集群必须至少包含 3 个节点，以确保控制器的高可用性。

除了控制器之外，{{site.data.keyword.cloud_notm}} 自动化还会为部署的 vSphere 主机准备 NSX VIBS，以支持通过 VXLAN 隧道端点（VTEP）使用虚拟化网络。将从为 VTEP 指定的专用 A 可移植 IP 地址范围（如 [VLAN 和子网摘要](design_physicalinfrastructure.html#table_vlan_subnet_summary)中所列）中为 VTEP 分配支持 VLAN 的 IP 地址。VXLAN 流量位于未标记的 VLAN 上，并分配给专用 vSphere 分布式交换机 (VDS)。

随后，将分配分段标识池，并且将集群中的主机添加到传输区域。由于在 {{site.data.keyword.cloud_notm}} 中未配置因特网组管理协议 (IGMP) 监听，因此在传输区域中仅使用单点广播。

在此之后，将部署 NSX Edge 服务网关对。在所有情况下，都会使用一个网关对来处理位于专用网络中的自动化组件的出站流量。对于 vCenter Server，会部署第二个网关（称为客户管理的 Edge），并配置连接到公用网络的上行链路以及分配给专用网络的接口。有关作为解决方案的一部分部署的 NSX Edge 服务网关的更多信息，请参阅 [NSX Edge on {{site.data.keyword.cloud_notm}} 解决方案体系结构](https://www.ibm.com/cloud/garage/files/IBM_Cloud_for_VMware_Solutions_NSX_Edge_Services_Gateway.pdf)。

云管理员可以配置任何必需的 NSX 组件，例如分布式逻辑路由器 (DLR)、逻辑交换机和防火墙。可用的 NSX 功能部件取决于在订购实例时选择的 NSX 许可证版本。有关更多信息，请参阅 [VMware NSX 版本比较](appendix.html#vmware-nsx-edition-comparison)。对于 vCenter Server 实例，{{site.data.keyword.cloud_notm}} 自动化会将 vCenter Server Appliance 和 Platform Services Controller (PSC) 添加到 NSX Manager 分布式防火墙排除列表。

### 分布式交换机设计

此设计使用最小数量的 vSphere 分布式交换机 (VDS)。集群中的主机会连接到公用和专用网络。这些主机均配置有两个分布式虚拟交换机。两个交换机的使用遵循的是用于将公用和专用网络分隔开的 {{site.data.keyword.cloud_notm}} 网络实践。下图显示了 VDS 设计。
...
|:----- |:----------- |:------------ |
|VLAN1|公用|可用于因特网访问|
|VLAN2|专用 A|ESXi 管理、管理和 VXLAN (VTEP)|
|VLAN3|专用 B|vSAN、NFS 和 vMotion|

来自工作负载的流量将在支持 VXLAN 的逻辑交换机上传输。

vSphere 集群使用如下表中所示进行配置的两个 vSphere 分布式交换机。

表 4. 融合集群分布式交换机

|vSphere 分布式<br>交换机名称|功能|网络<br>I/O 控制|负载均衡<br>方式|物理 NIC<br>端口|MTU|
|:------------- |:------------- |:------------- |:------------- |:------------- |:------------- |
|SDDC-Dswitch-Private|ESXi 管理、虚拟 SAN、vSphere vMotion、VXLAN 隧道端点 和 NFS (VTEP)|已启用|基于显式故障转移（vSAN 和 vMotion）发起虚拟端口进行路由（其他所有）|2|9,000<br>（巨型帧）|
|SDDC-Dswitch-Public|外部管理流量（南北）|已启用|基于发起虚拟端口进行路由|2|1,500<br>（缺省值）|

**注：**根据 {{site.data.keyword.CloudDataCent_notm}} 和主机硬件选择，主机 NIC 的名称、编号和顺序可能有所不同。
...
* [运行 VMware 集群的 {{site.data.keyword.cloud_notm}} 解决方案体系结构](https://www.ibm.com/cloud/garage/files/IBM-Cloud-for-VMware-Solutions-Multicluster-Architecture.pdf)

...
图 1. 系统上下文图
![系统上下文图 - vCenter Server ICP CAM](vcsicp-syscontext-vcs-icp-cam.svg)
...
开发者|容器技能熟练的资源，使用 CAM 控制台来创建和管理容器。开发者可在应用程序现代化过程中创建新服务。通过使用 CAM，开发者可在 vCenter Server、ICP 或 IKS 上供应工作负载，编写和编排使用 VM 和容器构建的服务，并可集成 DevOps 工具链和 Day-2 ITSM 解决方案。
...
管理控制台|通过 ICP 管理控制台，可在一个集中、安全的管理控制台中对应用程序和集群进行管理、监视和故障诊断。
...
应用程序迁移、联网和安全性通常是应用程序现代化中最难对付的方面。vCenter Server、VMware Hybridity、VMware NSX、ICP 和 {{site.data.keyword.cloud_notm}} Kubernetes Service 能应对这些困难，并支持您构建弹性、安全和稳健的现代应用程序。

...
IC4VS 自动化配置了缺省网关设置为 {{site.data.keyword.cloud}} 后端客户路由器 (BCR) 的 VCSA。但是，没有通过 BCR 连到因特网的路由。从 VMware vCenter Server on {{site.data.keyword.cloud_notm}} 实例到因特网的标准路由是通过管理 ESG 进行的。由于不建议更改 VCSA 或管理 ESG 的配置，因此推荐在客户子网上实现代理服务器来启用 VUM。

此方法意味着不必重新配置 VCSA 或管理 ESG，但是需要安装小型虚拟机 (VM) 或设备。代理服务器是一个系统，位于两个端点设备之间，充当中间设备。在本例中，代理服务器位于 VUM 与 VMware 的更新服务器之间。

VUM 向 VMware 的更新服务器请求资源时，会先将请求发送到代理服务器，然后代理服务器再将请求发送到更新服务器。代理服务器获得资源后，会将该资源发送给 VUM。代理服务器的使用有利于安全性、管理控制和高速缓存服务。

本文档描述了如何使用基于 CentOS 和 Squid 的代理服务器。Squid 代理是用于 Web 的开放式源代码高速缓存代理，支持许多协议，包括 HTTP 和 HTTPS。有若干 VM 和基于设备的代理可用，您应该根据企业需求选择适当的代理，并遵循供应商的指导进行安装和配置。选择使用 CentOS/Squid 实现的客户应该继续执行下面的过程。
...
|主机名| Proxy01 | |
|地址|代理 IP |应该使用来自供应过程中分配的客户专用可移植子网的备用 IP 地址。此子网上只保留有两个 IP 地址：一个用于 BCR，另一个用于客户 ESG
...
2.	按**开始**按钮以打开 VM 的电源。
3.	现在，VM 将打开电源并从 CentOS 7 ISO 进行引导。对 VM 启动**远程控制台或 Web 控制台**。请注意，必须安装远程控制台，并且运行 Web 浏览器的系统必须按名称解析 vSphere ESXi 主机。
...
9.	单击**配置**按钮，单击 **IPv4 设置**，然后在**方法**框中选择**手动**。
...
15.	安装完成后，以该用户身份登录，然后输入 _ping vmware.com_ 命令。该名称应解析为 IP 地址，并且您应该会得到响应。如果未得到响应，请检查 IP 地址、防火墙规则和 NAT 设置。
...
2. 在安装任何软件包之前，建议先使用以下命令来更新系统和软件包：
  `yum -y update`
...
8. 通过运行以下命令来确保 Squid 正在运行：`systemctl status squid`。

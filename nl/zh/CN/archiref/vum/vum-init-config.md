...
IC4VS 自动化配置了缺省网关设置为 IBM Cloud 后端客户路由器 (BCR) 的 VCSA。但是，没有通过 BCR 到因特网的路由。从 VCS 实例到因特网的标准路由是通过管理 ESG 进行的。由于不建议更改 VCSA 或管理 ESG 的配置，因此推荐在客户子网上实现代理服务器来启用 VUM。

此方法意味着不必重新配置 VCSA 或管理 ESG，但是需要安装小型 VM 或设备。代理服务器是一个系统，位于两个端点设备之间，充当中间设备。在本例中，代理服务器位于 VUM 与 VMware 的更新服务器之间。
...
|网关| 	customer-nsx-edge private uplink ip |	这是代理服务器的缺省网关设置，即 customer-nsx-edge 专用上行链路地址。可通过查看 customer-nsx-edge 的“设置”选项卡来找到该 IP|
|DNS 服务器|	AD/DNS IP |可以在 [IC4VS 云控制台](https://console.bluemix.net/infrastructure/vmware-solutions/console/vcenters/)中“已部署的实例”页面上找到此 IP 地址|
|BCR IP|	BCR IP |这是 IBM Cloud 后端客户路由器的 IP 地址，并且是 10.0.0.0/8 和 161.26.0.0/16 的网关。此地址在代理服务器的静态路由中使用，以便可以访问 VCSA 和 AD/DNS 服务器|
...
在跳板机上使用浏览器从 [CentOS](https://www.centos.org/download/) 下载 CentOS-Minimal ISO 文件。请注意，IBM Cloud 维护许多常用 Linux 分发版的镜像。此镜像仅在专用网络上可用，并且 CentOS ISO 可从以下地址获取：http://mirrors.service.softlayer.com/centos/7/isos/x86_64/。

### 配置内容库并使用 CentOS ISO 文件填充该内容库

创建本地 vCenter 内容库。只能在创建该内容库的 vCenter Server 实例中访问该库。使用部署虚拟机所需的模板和 ISO 来填充库。
...
3.	选择**下载设置**，然后在_代理设置_下，单击**编辑**按钮。
4.	勾选**使用代理**框，并输入_代理服务器 IP 地址_和_端口 3128_，然后单击**确定**。
5.	“连接状态”应该会更改为_正在验证_，然后更改为_已连接_。
6.	单击**立即下载**按钮，然后在_最近任务_窗格中，您应该会看到此活动已完成。

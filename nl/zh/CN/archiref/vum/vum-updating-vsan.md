...
vSAN 会生成系统基线和基线组以与 VUM 配合使用，并且您可以使用这些建议的基线来更新使用 vSAN 的 VCS 实例中 vSphere ESXi 主机的软件、补丁和扩展。vSAN 6.6.1 和更高版本会为 vSAN 集群生成自动构建建议。vSAN 将《VMware 兼容性指南》和 vSAN 发行版目录中的信息与有关已安装的 vSphere ESXi 发行版的信息结合使用。
...
* **先决条件** - 了解先决条件、过程和限制
* **升级 vCenter Server Appliance** - 有关更多信息，请参阅 [VCSA 更新和 SSO 链接的 vCenter](vum-updating-vcsa.html)。
* **升级 vSphere ESXi 主机** - 有关更多信息，请参阅[创建基线并连接到库存对象](vum-baselines.html)。
* **升级 vSAN 磁盘格式** - 请参阅“升级 vSAN 磁盘格式”。升级磁盘格式是可选操作，但为了获得最佳结果，请升级对象以使用最新版本。通过磁盘上格式，环境可使用完整的 vSAN 功能集。

## 启用 vSAN 联机运行状况工作流程

执行本部分中的任务将使 vSAN 基线在 VUM 中可用。vSAN 6.6.1 和更高版本提供了无缝的自动更新过程，可确保 vSAN 集群通过最佳可用发行版更新，从而使 VCS 实例始终保持受支持的状态，同时提供：
...
1.	从跳板机 Web 浏览器连接到 VCSA 管理界面 `https://<vCenter ip>:5480`。
...
请注意，有报告称，似乎只会为 HTTP 设置代理信息，而不会为 HTTPS 设置代理信息。为了配置也适用于 HTTPS 流量的代理信息，必须先启用 HTTPS。通过 SSH 登录到 VCSA 后，使用 proxy.get 命令来查看配置并确认是否未设置 HTTPS 参数。
...
2. 单击**重新测试**按钮并等待此过程完成。
3. “运行状况”中会显示名为“_联机运行状况连接_”的新检查，并且“启用联机运行状况”按钮将更改为“重新测试联机运行状况”。
4. 单击**重新测试联机运行状况**按钮来触发第一次上传，然后通过在“最近的任务”窗格中查看状态来等待此过程完成。“测试名称”应该更改为“联机运行状况（上次检查时间：刚才）”。
5. 完成后，在“运行状况”窗口中滚动至“vSAN 构建建议”并将其展开，然后单击 **vSAN 构建建议引擎运行状况**。
6. 单击**登录到 my.vmware.com**，并输入凭证。此过程完成后，**测试结果**将更改为**已通过**状态。
7. 按 **Update Manager** 选项卡，现在应该会看到 vSAN 集群已添加到基线。
...
* **vSphere 环境是最新的**：
  ...
  - 所有主机都应该运行的是相同的 ESXi 构建。如果 vSphere ESXi 主机版本不匹配，请进行更新
* **所有 vSAN 磁盘都应该运行正常**：
  - 不应有磁盘处于故障或缺失状态。这可以通过 vSphere Web Client 中的 **vSAN 磁盘管理**视图进行确定。单击**主页** > **主机和集群**，选择 **vSAN 集群**，单击 **vSAN** 选项卡，然后单击**物理磁盘**。滚动浏览所有磁盘并查看 vSAN 运行状态。
  - 不应有不可访问的 vSAN 对象。这可以通过 **vSAN 运行状况服务**来进行验证，方法是单击**主页** > **主机和集群**，然后选择 **vSAN 集群**。单击**监视**选项卡，单击 **vSAN**，然后单击**运行状况**。复查测试结果。
  - 在升级过程启动时，不应有任何活动的再同步，检查方法是单击**主页** > **主机和集群**，选择 **vSAN 集群**，单击 **vSAN** 选项卡，然后单击**再同步组件**。_再同步组件计数应为 0_。请注意，在升级过程中，由于数据需要在主机重新引导后同步，因此应该会有一些再同步活动。
* **vSphere ESXi 主机准备** - 在 vSAN 集群中将主机移至维护模式后，有三个选项可供选择：
  - **无数据迁移** - 如果选择此选项，那么 vSAN 不会从此主机转移任何数据。如果关闭主机的电源或从集群中除去主机，那么某些虚拟机可能会变得不可访问。
  - **确保可用性** - 如果选择此选项，那么 vSAN 允许您比“完全数据迁移”更快地将主机移至维护模式，并允许访问环境中的虚拟机。
  - **完全数据迁移**
* **退出维护模式并再同步** - vSphere ESXi 主机升级并退出维护模式时，将执行再同步。您可以通过 Web 客户机来查看此操作。请确保此操作完成后，再移至下一个主机。由于已更新的主机现在可再次提供给 vSAN 数据存储，因此将执行再同步。请务必等待此再同步操作完成，以确保不会发生数据丢失。
* **启动 vSAN 集群升级后**：
...
有关更多信息，请参阅 [VCSA 更新和 SSO 链接的 vCenter](vum-updating-vcsa.html)。

##	升级 vSphere ESXi 主机

有关更多信息，请参阅[创建基线并连接到库存对象](vum-baselines.html)。
...
3. 输入在 ICVS 控制台上提供的管理员密码。您现在将位于虚拟文件系统的根目录中，请输入 ls，然后按 **Enter** 键，您应该会看到：
  `0 /
  1 localhost/``

5. 输入 `cd 1` 并按 Enter 键，然后输入 `ls` 并按 **Enter** 键。您应该会看到：
  `0 / datacenter1 (datacenter)`

6. 输入 `cd 0` 并按 Enter 键，然后输入 `ls` 并按 **Enter** 键。您应该会看到：
...
7. 输入 `cd 1` 并按 **Enter** 键，然后输入 `ls` 并按 **Enter** 键。您应该会看到集群：
  `0 cluster1 (cluster)``

8. 现在，我们将针对此集群使用 VSAN 命令。要检查磁盘状态，请输入 `vsan.disks_stats 0` 并按 **Enter** 键。

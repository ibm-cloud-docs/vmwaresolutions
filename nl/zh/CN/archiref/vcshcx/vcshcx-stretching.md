...
此外，作为网络延伸工作流程的一部分，将命令云端的 NSX 构建 VXLAN，然后将构建的 VXLAN 连接到在指定的云端 L3 设备（处于未连接状态的 DLR 或 ESG）上创建的接口以及云端 L2C 设备。

迁移特定应用程序时，通常情况下，适用虚拟机 (VM) 所使用的所有网络都必须延伸到 {{site.data.keyword.cloud}} 实例。

为什么是通常情况，而不是始终这样？在迁移 VM 后，断开与客户机端特定流量的连接比较有利。例如，VM 访客备份客户机可能会在移至云时导致很高的带宽使用量。VM 迁移时不需要访客中的备份客户机，因为在云端有更现代的块级别备份会自动选取该客户机。

若是不访问每个 VM 来关闭访客中客户机备份安排，那么不连接客户机的备份网络适配器（如果使用了备份网络）会导致备份失败。这是临时情况，在迁移后可以访问所有 VM 来禁用访客中的备份客户机之后，会解决此情况。
...
同时，vMotion 在通过该隧道收到来自源 CGW 的数据时，会从远程 ESXi 代理主机启动到目标 vSphere 物理 ESXi 主机。应用 vMotion 时，一次只能运行一个 VM 迁移操作，这与批量迁移选项不同。因此，对于要迁移大量 VM 的情况，建议仅在无法停机时或在重新引导 VM 有风险时，才使用此选项。但是，与标准 vMotion 一样，VM 在此过程中可以保持活动状态。

据观察，在 LAN 上，单个 vMotion 在约 1.7 Gbps 的速度时会达到极限，如果在 WAN 上通过 WAN Optimizer，那么在 300 到 400 Mbps 时达到极限。这并不意味着 LAN 上的 1.7 Gbps 等于 WAN 上的 400 Mbps，而只是说明观察到的环境的最大观察值。观察到此情况的环境由 10 GB LAN vMotion 网络和 1 GB 因特网上行链路（与生产 Web 流量共享）组成。

对于以下情况，请使用 vMotion：
- VM 关闭或启动有问题，或者关闭 VM 时正常运行时间可能很长而引入风险。
- 任何需要磁盘 UUID 的集群类型应用程序，如 Oracle Rac 集群。vMotion 不会更改目标上的磁盘 UUID。
- 您希望尽快移动单个 VM。
- 不需要安排的迁移。
...
- 关闭原始 VM 的电源。
- 在关闭电源期间，会执行对任何更改数据的最终复制。
- 在目标端打开新 VM 的电源。
- 对原始 VM 重命名，并将其移至云文件夹。
...
- 安排批量迁移可在安排的中断时段内翻转到新迁移的 VM。
- 可以允许迁移因 vMotion 失败而导致当前使用的虚拟 CPU 功能不同于云端的 VM。
...
Oracle RAC、MS Exchange 和 MS-SQL 集群是其中有两个或多个 VM 参与需要跨所有 VM 或集群节点共享磁盘的集群的应用程序示例。对于属于应用程序集群的磁盘（非操作系统虚拟盘），其所有 VM 节点上都需要启用 VMware 多写程序标志。不支持任何虚拟盘的启用了多写程序标志的 VM。

迁移启用了多写程序虚拟盘的集群：
- 使用 vMotion 作为原始 VM 磁盘并保留 UUID 映射。
- 迁移期间，集群会以降级状态（单节点）保持运行。
- 迁移开始之前和迁移完成之后，集群会产生停机时间，以便跨集群 VM 节点重新组合多写程序配置。

要迁移启用了多写程序磁盘的集群，请完成以下步骤：
1. 按照应用程序最佳实践停止运行集群和所有节点。
2. 在配置了多写程序的虚拟盘的每个节点 VM 中，记下磁盘顺序（如果应用程序需要）。
3. 对于 Oracle 和其他任何使用虚拟盘 UUID 功能的应用程序，请登录到特定 ESXi 主机，然后运行 `vmkfstools -J getuuid /vmfs/volumes/datastore/VM/vm.vmdk` 命令以获取每个虚拟盘文件的 UUID，该文件要求为集群设置多写程序标志。如果最佳实践是使磁盘顺序与路径在操作系统中的显示方式保持一致，那么此操作是必需的。vMotion 可以对磁盘重新排序 (disk1, disk2, disk3)，但 UUID 保持不变。使用记下的 UUID 对信息执行磁盘映射，以重新创建磁盘命名顺序。迁移完成时可根据需要显示 SCSI 标识。应用程序应该以任一方式运行。这适用于 Oracle 实例具有多个虚拟盘，并已映射这些虚拟盘以便对应用程序进行故障诊断的情况。
4. 从除了主节点之外的其他所有集群 VM 节点中除去虚拟盘。
5. 从主 VM 集群节点（应该是当前唯一拥有集群磁盘的节点）中除去多写程序标志。
6. 根据需要重新启动主集群节点，以最大限度缩短停机时间。
7. 使用 vMotion 迁移所有集群节点。首先迁移主集群，然后迁移已关闭电源的其他所有节点。
8. 拥有节点的主磁盘完成迁移时，请关闭其电源。
9. 如果需要，请使用正确的磁盘 UUID 和 SCSI 标识来重新映射磁盘顺序。无需重新映射，应用程序即可正常运行。
10. 在主节点上重新启用多写程序标志。
11. 启动主节点并验证运行情况。
12. 在其他所有集群节点 VM 上映射磁盘/启用多写程序标志，然后打开这些 VM 的电源。
13. 验证其他集群节点运行情况。
...
迁移后，可以使用操作系统将 iSCSI 卷镜像到其他本地云存储解决方案，并且可以将 NFS 数据复制到任何云中解决方案。注意事项如下：
- 等待时间（iSCSI 的等待时间，或者对于 NFS，为应用程序容许的值）
- 带宽（每个延伸网络约 1 Gbps）
- 下层链路带宽

执行完迁移生命周期后，确保在尝试用于生产之前，使用开发或编译打包应用程序进行测试。对于利用等待时间敏感性 L2 延伸网络的 L2C HCX 设备之间的下层隧道流量 (udp 500 / 4500)，可以应用 QoS。
...
- 在 UI 中运行 HCX 取消延伸网络工作流程，并选择相应的路由 NSX 设备来接管取消延伸的网络缺省网关。
- 运行任何外部路由更改，其中可能包括：为已迁移网络插入更改的路由，将源站点的路由移除到已迁移网络，并确保根据需要，通过 WAN 将未迁移的应用程序路由到这一已迁移的子网。
- 应用程序所有者从所有可能的访问点（因特网、内部网和 VPN）对已迁移应用程序进行测试。

对于其所有 VM 已完全迁移到云的特定应用程序执行网络摆动时的注意事项：
如果要在专用网络端使用 Vyatta，请将路径插入到 MPLS 云中，然后通过隧道传输到 MPLS 上的边缘路由设备，以避免使用 {{site.data.keyword.cloud_notm}} IP 空间。您的帐户已设置为使用 {{site.data.keyword.cloud_notm}} VRF。某些应用程序位于网络负载均衡的虚拟 IP (vIP) 后面。这些 vIP 位于您拥有的子网上，该子网位于 Vyatta 后面的虚拟 F5 上。对于通过 HCX 摆动到 {{site.data.keyword.cloud_notm}} 的网络，将更具体的路由公告给 MPLS 这种方法虽然对其他网络非常有效，但在插入 /32 路径时，对于单个 vIP 却不起作用。

解决方案：WAN 提供商通常会过滤掉所公告的 /32 路径。请与 WAN 供应商合作以允许使用这些路径。

下面是注意事项和影响：
- 共享子网、vLAN 和 VXLAN 的应用程序需要一起移动。
- 对于使用内部可路由 IP 的负载均衡器后面的应用程序，如果不能或不宜一起移动，可能需要路由更改。例如，如果在一次摆动中涉及的应用程序太多，那么感知到的风险会太高。
- 需要 VMware 管理员、网络管理员（包括客户/WAN 供应商）和应用程序所有者的参与。即使未计划会影响特定系统或网络设备的更改，也需要这些人员的参与。

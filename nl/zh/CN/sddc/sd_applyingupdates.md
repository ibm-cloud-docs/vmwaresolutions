...
2. 在包含 VMware 组件更新的更新期间，可能需要从 ESXi 服务器迁移 VM 以使其进入维护模式。如果 VM 安装了本地数据存储或 CD-ROM，这可能会阻止 VM 迁移。
...
6. 如果更新过程在特定步骤失败，请[联系 IBM 支持](../vmonic/trbl_support.html)以获取帮助。IBM 支持将向您提供如何解决问题的建议，并指导您从失败的步骤重新启动升级。

...
# 将 V2.5 之前的 Cloud Foundation 实例迁移到 IBM Cloud 帐户
{: #sd_addinstancetousraccount}

在 {{site.data.keyword.cloud}} 帐户中 V2.5 和更高发行版中部署的 VMware Cloud Foundation 实例会自动添加到帐户，并由 {{site.data.keyword.cloud_notm}} Identity and Access Management (IAM) 进行管理。
...
3. 在 **Cloud Foundation 实例**表中，找到 V2.5 之前的实例。
...
2. 实例迁移完成后，实例将显示在**已部署的实例**页面上它所迁移到的帐户下。迁移的实例不会再显示在从中迁移该实例的原始帐户中。

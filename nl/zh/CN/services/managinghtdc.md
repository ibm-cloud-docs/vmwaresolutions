...
要管理 HyTrust DataControl on {{site.data.keyword.cloud}} 服务 (HTDC)，请通过 {{site.data.keyword.vmwaresolutions_short}} 控制台访问 HTDC WebGUI，或通过 vSphere Web Client 访问 HTDC 控制台。

## 通过 IBM Cloud for VMware Solutions 控制台访问 HyTrust DataControl WebGUI
{: #managinghtdc-accessing-webgui}

要登录到主或辅助 HTDC 设备的 WebGUI，请使用在 HyTrust DataControl on {{site.data.keyword.cloud_notm}} 服务详细信息页面上找到的 WebGUI 凭证。

## 通过 vSphere Web Client 访问 HyTrust DataControl 控制台
{: #managinghtdc-accessing-console}

要通过 vSphere Web Client 访问 HTDC 控制台，请使用以下过程：
1. 在 vSphere Web Client 中，找到名为 **KC1** 和 **KC2** 的虚拟机。
2. 右键单击 **KC1** 或 **KC2**，并单击**打开控制台**。
3. 使用可在 HyTrust DataControl on {{site.data.keyword.cloud_notm}} 服务详细信息页面上找到的控制台凭证，登录到控制台。
...
## 为 HyTrust DataControl VM 启用因特网访问
{: #managinghtdc-internet-access}

对于 HTDC 4.3.2 和更高版本，{{site.data.keyword.vmwaresolutions_short}} 为已启用“回拨”功能的 HyTrust 许可证提供自动更新支持。对于不是仅专用的 vCenter Server 实例，HTDC 使用防火墙和 SNAT（源网络地址转换）规则进行部署，这些规则是在管理服务 ESG **mgmt-nsx-edge** 上定义的。

这些规则允许您为 HyTrust 虚拟机 (VM) 启用因特网访问。如果未启用因特网访问，那么应用于 HTDC 安装的许可证将在一年后到期。
...
4. 使用可在 {{site.data.keyword.vmwaresolutions_short}} 控制台上的 HyTrust DataControl on {{site.data.keyword.cloud_notm}} 服务详细信息页面上找到的控制台凭证，登录到控制台。

...
确保已完成以下任务：
*  已在**设置**页面上配置 {{site.data.keyword.cloud_notm}} 基础架构凭证。有关更多信息，请参阅[用户帐户和设置](/docs/services/vmwaresolutions/vmonic/useraccount.html)。
*  已查看[安装 KMIP for VMware on {{site.data.keyword.cloud_notm}} 实例时的注意事项](/docs/services/vmwaresolutions/services/kmip_standalone_considerations.html)中的所有注意事项。
...
添加所订购 KMIP for VMware on {{site.data.keyword.cloud_notm}} 实例的客户机证书。有关更多信息，请参阅[添加、查看和删除 KMIP for VMware on {{site.data.keyword.cloud_notm}} 实例的证书](/docs/services/vmwaresolutions/services/kmip_standalone_addingdeletingcert.html)。

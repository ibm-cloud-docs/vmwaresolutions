...
1. 使用 {{site.data.keyword.cloud_notm}} 基础架构 VPN 或跳跃服务器来允许访问 IBM Spectrum Protect Plus 虚拟机的 IP 地址。可以在 {{site.data.keyword.vmwaresolutions_short}} 控制台的 IBM Spectrum Protect Plus on {{site.data.keyword.cloud_notm}} 的服务详细信息页面上找到该 IP 地址。
...
## 应用 IBM Spectrum Protect Plus on IBM Cloud 的修订

您负责维护 IBM Spectrum Protect Plus on {{site.data.keyword.cloud_notm}} 服务，使其保持最新版本。可以从 [IBM Spectrum Protect Plus on {{site.data.keyword.cloud_notm}} 支持](https://www.ibm.com/mysupport/s/topic/0TO50000000IQWtGAO/spectrum-protect-plus)页面中下载必需的更新。
...
## 更新 IBM Spectrum Protect Plus 虚拟机的操作系统

要更新 IBM Spectrum Protect Plus 虚拟机的操作系统，您必须以 **root** 用户身份登录。**root** 用户密码与可在 IBM Spectrum Protect Plus on {{site.data.keyword.cloud_notm}} 的服务详细信息页面上找到的密码相同。

## 备份和复原安装了 IBM Spectrum Protect Plus on IBM Cloud 的实例的管理组件

 **注**：以下信息适用于部署在（或已升级到）V2.3 或更高版本中的实例上的 IBM Spectrum Protect Plus 安装。对于 V2.2 实例，此服务仅为工作负载虚拟机提供数据保护。

IBM Spectrum Protect Plus on {{site.data.keyword.cloud_notm}} 服务已预配置一个管理备份作业，该作业会自动运行以每天备份以下管理组件，最多可有七个复原点：

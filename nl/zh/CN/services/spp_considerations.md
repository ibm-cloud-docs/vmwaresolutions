...
**可用性**：此服务仅可用于部署在（或已升级到）V2.2 或更高发行版中的实例。

**注：**
* 如果接受为部署在 V2.4 或更高发行版中的实例缺省选择的服务，那么会安装 IBM Spectrum Protect Plus V10.1.1 补丁 1。此服务会自动为管理虚拟机 (VM) 提供恢复支持。
* 如果接受为部署在 V2.3 中的实例缺省选择的服务，那么会安装 IBM Spectrum Protect Plus V10.1.1。该服务会自动为管理 VM 提供备份。
...
* 确保根据 ESXi 服务器版本，ESXi 服务器上的可用 NFS 安装足够。
  ...
  部署在或已升级到 V2.2 或更高发行版中的 Cloud Foundation 实例和 vCenter Server 实例在 VMware 中有 `NFS.MaxVolumes` 参数设置。此参数定义 ESXi 服务器上的最大 NFS 安装数，并可设置为最大值 256（特定于 ESXi 服务器版本）。有关更多信息，请参阅[增大定义 ESXi/ESX 主机上 NFS 最大挂载量的默认值](https://kb.vmware.com/s/article/2239)。

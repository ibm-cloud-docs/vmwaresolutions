...
6. Under **Licensing**, complete the license settings for the listed components.
...
12. If you want to use **vSAN storage**, select the corresponding option.
    ...
    If you will pay for a license, select an option from the menu.
...
24. Complete the information for the **Edge services cluster**.
    1. Accept or change the default **Cluster name** of the edge services cluster.
    2. For **Compute capacity**, the values are:
       - Clusters hosts: 2
       - Storage: 2 * 3.8 TB SSD
    3. Select the **CPU model** and **RAM**.
    4. Select the **Uplink speed**. 25 Gb is available only for Cascade Lake servers and specific data centers. For more information, see [Uplink speed](/docs/vmwaresolutions?topic=vmwaresolutions-vc_orderinginstance-edge-services-cluster#vc_orderinginstance-edge-cluster-uplink).

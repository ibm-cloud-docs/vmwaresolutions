...
VMware Solutions Shared now supports both public and private network endpoints. The Private Network Endpoint service is available and ready-to-use in all virtual data center instances. Use the service to connect your {{site.data.keyword.cloud}} account resources to virtual machines (VMs) in your virtual data center over the {{site.data.keyword.cloud_notm}} private network.
...
4. Complete the following configuration in the **Create new private network endpoint** pane.
  ...
  2. Provide the IPs or subnets to whitelist.
  3. Click **Create**.
...
3. View the **Private network endpoint** details. Expand **Whitelisted IPs and subnets** to view the list of IPs and subnets that you added when you ordered the private network endpoint.
...
| Whitelisted IPs and subnets | The list of all whitelisted IPs and subnets for the instance. |
...
You must open an IBM Support ticket to modify or adjust whitelisted IPs or subnets. Provide the following details when you contact IBM Support:
...
## Procedure to delete a private network endpoint from a virtual data center instance
{: #shared_creatingendpoints-delete}
{: help}
{: support}

You might want to delete a private network endpoint from a virtual data center instance when it's no longer needed.

1. From the {{site.data.keyword.vmwaresolutions_short}} console, click **Resources** from the left navigation pane.
2. In the **VMware Solutions Shared** table, click the instance that you want to delete the private network endpoint from.

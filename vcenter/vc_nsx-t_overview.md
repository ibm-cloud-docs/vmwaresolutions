...
VMware vCenter Server with NSX-T is a hosted private cloud that delivers the VMware vSphere stack as a service. The VMware environment is built on top of a minimum of three {{site.data.keyword.cloud_notm}} {{site.data.keyword.baremetal_short}}. The environment offers shared network-attached storage and dedicated software-defined storage options. It also includes the automatic deployment and configuration of an easy-to-manage logical edge firewall, which VMware NSX-T powers.
{: shortdesc}

...
* Whenever possible, remove ESXi servers by using the IBM Cloud® for VMware Solutions console because changes that you make on the VMware vSphere® Web Client are not synchronized with the IBM Cloud for VMware Solutions console. Therefore, remove ESXi servers from Cyber Recovery only for on-premises ESXi servers or ESXi servers that you don't manage in the IBM Cloud for VMware Solutions console.
...
2. In the **Cyber Recovery instances** table, click the instance for which you want to contract capacity.
...
* [Deleting clusters from vCenter Server instances](/docs/vmwaresolutions?topic=vmwaresolutions-cr_deletingclusters)

...
|--------------|-----------------|
| [Caveonix RiskForesight](/docs/vmwaresolutions?topic=vmwaresolutions-caveonix_considerations) | 2.4 |
| [HyTrust CloudControl®](/docs/vmwaresolutions?topic=vmwaresolutions-htcc_considerations) | 6.2.1 |
| [HyTrust DataControl](/docs/vmwaresolutions?topic=vmwaresolutions-htdc_considerations)  | 5.1.1 |
| [Juniper vSRX](/docs/vmwaresolutions?topic=vmwaresolutions-juniper-overview) | 3.0 (20.1R2) |
...
[^nsxtver]: If adding the service to an existing instance, you must upgrade NSX-T from 2.5.1 to 3.1.

## Promotions for add-on services
{: #vc_addingremovingservices-service-promotions}

{{site.data.keyword.vmwaresolutions_full}} offers promotion pricing for some add-on services. The promotions offer a number of months free of charge for service licenses, if the service has license charges. For information about the promotions that are currently in effect, see the [pricing options](https://www.ibm.com/cloud/vmware/pricing){:external}.
...
Some services require a minimum amount of resources in order to be installed.

For certain services, the system performs a capacity check on the targeted clusters before it installs the service in your environment. The check is performed against the available capacity of the targeted cluster in the environment to ensure that the service components can fit.
...
To install the service, you must increase the capacity in your cluster either by adding more ESXi servers or by freeing up RAM, CPU, or disk space. After that, you can add the service or services again. To remove the existing service in the **Capacity Validation Failed** state, click the delete icon next to it.
...
| vRealize Operations and Log Insight | CPU: 18 vCPUs<br>RAM: 208 GB<br><br>If you install vRealize Operations with vSAN storage, you must have an estimated 750GB of vSAN storage. This storage requirement is only applicable to vSAN because the install of vROps on a NFS instance orders its own NFS storage. |
...
For specific add-on services, if you installed the add-on service before the IBM Cloud for VMware Solutions 4.0 release and you then delete that add-on service, you must manually remove the DNS entries.

After you delete the add-on service, unused DNS entries remain in the Active Directory. These entries do not cause any problems. However, in the future, they could create conflicts with reverse lookups. It is recommended that you remove the entries as soon as possible.

The following table shows the add-on services that are affected by this. The table also shows the pattern for host names for various service offerings. The actual host name might differ from the pattern in various ways. Use the pattern to locate all of the registered DNS hostnames associated with a service. Many services have more than one registered hostname.
...
2. Navigate to the instance subdomain. This is the subdomain under the root domain, which resembles the name of the vCenter Server instance.
3. Remove the hostname entries for the specific deleted add-on service appliances.

...
# Cyber Recovery consolidated and workload clusters
...
You also have the option to order an additional workload cluster.

## Cluster name
{: #cr_orderinginstance-consolidwkld-cluster-name}

By default, for the consolidated cluster, the cluster name is set to the value **_instance name_-consolidated**.

By default, for the workload cluster, the cluster name is set to the value **_instance name_-workload**.
...
Table 2. Options for Cascade Lake bare metal servers - VMware NSX-T™ instances needs to be changed. Cyber Recovery does **not** support NSX-V, so we are removing that information.
{: note}
...
## RAM
{: #cr_orderinginstance-consolidwkld-ram}

Select the RAM value from the drop down menu. The default is 768 GB.
...
When you select **NFS storage**, you can decide on one of the following:

...
* Whenever possible, add ESXi servers by using the IBM Cloud® for VMware Solutions console because changes that you make on the VMware vSphere® Web Client are not synchronized with the IBM Cloud for VMware Solutions console. Therefore, add ESXi servers to Cyber Recovery only for on-premises ESXi servers or ESXi servers that you don't manage in the IBM Cloud for VMware Solutions console.

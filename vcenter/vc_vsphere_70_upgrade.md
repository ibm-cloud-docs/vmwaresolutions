...
# Upgrading vCenter Server vSphere software from VMware vSphere 6.5 or 6.7 to 7.0
...
This VM is used as a jump box into the vCenter Server instance for the upgrade that downloads the binary files from https://my.vmware.com. While it is possible to place this VM on the vCenter Server instance that is being upgraded, it is not recommended.

Complete the following steps to download files to your jump box.

1. From the VMware Portal, download the following product files onto your jump box:
...
Complete the following steps from the vCenter user interface.
...
You must temporary install the new vCenter Server Appliance that you deploy onto a vSphere Standard Switch. One of the existing vmnics is reassigned from the distributed switch during the upgrade.
...
4. Return to the **Virtual Switches** panel and click **Add Networking**.
  ...
  4. For **Connection Settings**, keep the **VM Network** and **VLAN ID None** defaults. Click **Next**, then **Finish**. *Standard Switch: vSwitch0* displays in the list of switches.
5.  Make a note of the Network Setting for the vCenter vCenter Server Appliance virtual machine. You must update the new vCenter appliance to match.
  * From the vCenter user interface, click the virtual machine for the vCenter appliance. Note the name, ending with ``vc``.
...
* You must use the IP and credentials (administrator and root) for the current vCenter appliance. Use the IP and root password for the ESXi host of the current vCenter Server Appliance and the host where you want to deploy the new 7.0 vCenter Server Appliance. You must use a new temporary IP address, gateway, and subnet mask for the new vCenter Server Appliance during installation.
...
1. Extract the ``007.1316.0000.0000_Unified_StorCLI_PUL.zip`` file to a directory on your windows jump box.
2. Locate the ``vmware-storcli.vib`` file in the extracted file contents.
3. Copy the ``.vib`` file to either a vSAN or NFS data store that is mounted on the ESXi hosts for the instance. Use vCenter to reference the extracted file on your jump box.
4. SSH into each ESXi host and run the following VIB Upgrade command:  
  ...
  The following installation results display.
...
1. From the vCenter user interface, go to **LCM menu > LifeCycle Manager**.
...
1. Go to the new vCenter appliance.
...
6. Select **vmnic2** and click the red **X** to delete the adapter. Click **OK**. The ``There are no active physical network adapters for the switch.`` warning displays. Click **OK**.
...
9. Select **uplink1** and click the **+**. `vmnic2` displays.

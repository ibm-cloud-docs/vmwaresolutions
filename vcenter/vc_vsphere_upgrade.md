...
# Upgrading vCenter Server vSphere software from vSphere 6.5 to 6.7
{: #vc_vsphere_upgrade}
...
* Confirm the following requirements for vSAN based instances:
   ...
   * Ensure that each node has the space to handle rebuilding redundancy of vSAN objects in case a VMware ESXi™ host fails to come back up during the upgrade. You might need to either reduce disk usage or add an ESXi host before the upgrade.  
...
4. Download and install the Google Chrome™ browser. Mozilla® Firefox® has a cache issue when it uses the VUM screens on the vCenter Server user interface.
...
* For more information about backing up vCenter Server and PSCs, see [vCenter file-based backup](/docs/vmwaresolutions?topic=vmwaresolutions-solution_backingup#solution_backingup-vcenter) and [Overview of backup and restore options in vCenter Server 6.x](https://kb.vmware.com/s/article/2149237?lang=en_US){: external}.
...
1. Go to [VMware Product Interoperability Matrices](https://www.vmware.com/resources/compatibility/sim/interop_matrix.php){: external}. Ensure that you’re on the **Interoperability** tab.
2. Under **Select a Solution**, complete the following tasks:
...
3. Under **Add Platform/Solution**, complete the following tasks:
...
Upgrading VMware NSX can take some time as the upgrade process updates the vSphere installation bundles on the ESXi hosts and it requires a restart of each host. Therefore, plan your maintenance window in advance.
...
* For VMs on which VMware Tools are not installed, manually shut down or forcefully power off before the NSX ESXi host module installation. These VMs keep the target ESXi host from going into maintenance mode.
...
1. Log in to both the PSC, `https://<psc-fqdn>:5480`, and vCenter appliance management user interfaces to confirm whether the root password is expired. If the password expiration date is `1970`, then it is expired and you must enable SSH and the bash shell in the PSC appliance management user interface.
    1. SSH into the PSC with the root ID and password. You can log in, even though the password is expired.
    2. Use the shell `passwd` command to set a new root password for both the PSC and vCenter.
    3. Save the passwords that were displayed on the {{site.data.keyword.vmwaresolutions_short}} console or given to you by IBM Support. These passwords are reused later when you upgrade the appliances.
...
1. Ensure that you configured VUM to download patches from https://www.vmware.com. For more information, see [VMware Update Manager introduction](/docs/vmwaresolutions?topic=vmwaresolutions-vum-intro).
...
1. From the vCenter user interface, go to the cluster that contains the ESXi hosts to upgrade.
...
3. From the vCenter Server UI, select the cluster that contains the ESXi hosts to be patched.
...
4. Click **Upgrade**. Because the upgrade processes a disk group at a time, it can take some time to complete depending on the size of the cluster. Optionally, select **reduced redundancy** during the process. While this setting might significantly reduce the time that it takes for the upgrade, it might result in data loss if a disk failure happens during the upgrade process.
...
1. If you deployed HCX™, you must undeploy the Cloud Gateway portion of HCX before you upgrade the VDS it resides upon. Ensure that HCX is updated to the latest version before you redeploy the Cloud Gateway.
...
Use the vCenter Server user interface to complete VMware guest tools upgrades. Some VMware tools might be required on some older VMs that were migrated into the vCenter Server environment.  
...
You might need to update your firmware and drivers to a version that is compatible with vSphere 6.7 on the ESXi servers. Check your firmware and drivers versions. If required, complete the necessary upgrades.

...
This service is available only to instances deployed in V2.2 or later releases.
...
KMIP for VMware on {{site.data.keyword.cloud_notm}} uses the IBM Key Protect for IBM Cloud service to create, encrypt, and decrypt encryption keys. Therefore, before installing the KMIP for VMware on {{site.data.keyword.cloud_notm}} service, ensure that you have ordered a usable Key Protect service. Meanwhile, an {{site.data.keyword.cloud_notm}} service ID has been created by following the steps in [Creating and managing service IDs](https://console.bluemix.net/docs/iam/serviceid.html#creating-a-service-id) to access the Key Protect service instance that you created.
...
The API key for the created service ID is required when ordering the KMIP for VMware on {{site.data.keyword.cloud_notm}} service.

The KMIP for VMware on {{site.data.keyword.cloud_notm}} service assumes that you have already created at least one customer root key (CRK) from the Key Protect user interface by following the steps in [Creating root keys](https://console.bluemix.net/docs/services/keymgmt/keyprotect_create_root.html#create_root_keys), or by using the RESTful API in [IBM Key Protect](https://console.bluemix.net/apidocs/639-ibm-key-protect).

You cannot order the KMIP for VMware on {{site.data.keyword.cloud_notm}} service without CRKs. It is highly recommended that you use the method to create a CRK using existing key material, and backup the key material that you are creating.

By doing so, you ensure that you can recover your keys in case of total loss of the data center where IBM Key Protect is storing your CRKs.
...
* To use the KMIP for VMware on {{site.data.keyword.cloud_notm}} service for VMware vSAN encryption, you must ensure that the network connectivity between the hosts on the target vSAN and the endpoint of the ordered KMIP for VMware on {{site.data.keyword.cloud_notm}} service is functional.
* {{site.data.keyword.cloud_notm}} maintains a highly available KMIP for VMware on {{site.data.keyword.cloud_notm}} service endpoint in each region where the service is available. You select the region for the service when you deploy it. In vCenter Server, you must configure a KMS cluster with connections to the endpoint in your selected region. See the following table for the endpoint in each region. These endpoints use self-signed certificates that are maintained by the {{site.data.keyword.vmwaresolutions_short}} team. The thumbprint for the certificates is `a9 d0 ff 15 df 85 10 6b 61 88 fe 2e 8b d3 1a af 48 c8 a0 7a`.
...
When ordering or using the KMIP for VMware on {{site.data.keyword.cloud_notm}} service, you must provide the VMware public certificate that is used as the client certificate to communicate with the service instance.

When the KMIP for VMware on {{site.data.keyword.cloud_notm}} service is removed, all the encryption keys created by this service instance for the associated VMware public certificate are removed together.

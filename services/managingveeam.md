...
The Veeam on {{site.data.keyword.cloud_notm}} service is preconfigured with a management backup job that runs automatically. This job backs up the management components daily with up to 14 points of restoration.

You can also back up the management components manually by using the Veeam console.

For instances deployed in (or upgraded to) V1.8 or later releases, the configuration changes to your environment are not automatically backed up. Therefore, before you change the configuration of your environment, it is recommended that you back up the management components manually by running the preconfigured management backup job in the Veeam console. For more information about backing up manually, see the [Veeam technical instructions](https://helpcenter.veeam.com/backup/vsphere/scheduing_manual.html){:new_window}.
...
You are responsible for maintaining Veem to keep it updated to the most recent version. To upgrade Veeam to the latest version, download the Veeam updates from the Veeam website, copy the updates to the Veeam VSI, and then install them.

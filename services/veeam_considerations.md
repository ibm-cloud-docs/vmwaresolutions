...
* The management components that are backed up by the Veeam on {{site.data.keyword.cloud_notm}} service include the VMware vCenter Server, the Platform Services Controller (PSC), the IBM CloudDriver, and VMware SDDC Manager (applies to Cloud Foundation instances only).

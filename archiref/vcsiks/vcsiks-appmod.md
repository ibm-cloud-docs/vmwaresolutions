...
- **{{site.data.keyword.cloud_notm}} for VMware Solutions** – The vCenter Server instance provides the fundamental VMware building blocks, such as vSphere, vCenter Server, NSX-V, and storage options that include vSAN or {{site.data.keyword.cloud_notm}} Endurance storage, needed to automatically deploy a VMware Software Defined Data Center (SDDC) solution. The VMware cluster is the target for the migrated VMs and some of the modernized applications in containers hosted in ICP. The key components in vCenter Server are as follows:
...
private cloud, including private endpoint deployment to {{site.data.keyword.cloud_notm}}
...
management. The management preserves your existing VMware tools, scripts, and
...
{{site.data.keyword.cloud_notm}} Kubernetes Service (IKS) is a managed Kubernetes offering to
...
all while using Cloud Services including cognitive capabilities
...
-   Intelligent scheduling maximizes use of the underlying
...
which the orchestrator uses to ensure that the workloads have the
required capacity.
-   Service discovery and load balancing provide a lightweight DNS within
the Kubernetes cluster that allows services to register themselves and
eliminate the need for static coding of your microservices. Load
balancing distributes incoming requests to the number of instances
that run in your architecture.
-   Automated rollouts and rollbacks support teams who deploy new
features and fixes in a controlled manner. If something breaks, we
...
routers decrease application time to value. This reference architecture
shows how vCenter Server, ICP, and IKS securely move Acme Skateboards along the

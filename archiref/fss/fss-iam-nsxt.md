...
T0 - Customer   | Read | Full |
T0 - Transit    | Read| Full |
T0 - Management | Read | Full |
T1s - Customer  | Read | Full |
...
|:---------|:-------------|:------------|
| ISV      | admin | Used post-deployment to manage NSX VTEP IP addresses and to manage host and cluster configuration when hosts and clusters are added and removed. Also used to manage ESG configuration for add-on services that require public network access for licensing, activation, or usage reporting. |
| ISV | `cloudadmin` | Through the HyTrust Cloud Controller URL, customers authenticate with the `cloudadmin` user who is defined in the IBM Active Directory. More users can be added from customer space through HyTrust. |

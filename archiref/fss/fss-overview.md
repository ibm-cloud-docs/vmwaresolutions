...
* vSAN and NSX-T
* A four ESXi host management cluster
...
* vCenter, which manages all hosts in the entire IBM Cloud for VMware Regulated Workloads instance.
...
* Veeam is an available option, which provides continuous backup of the management stack for protection against disasters and rapid restoration to known good states if corruption of any management stack component were to occur. Veeam can also provide backup services for the workload cluster. The single site deployment should use the Veeam bare metal option to provide an acceptable backup repository.

## Edge cluster
{: #fss-overview-edge}

The FortiGate physical 10 Gb HA appliance is the only point of entry into the IBM Cloud for VMware Regulated Workloads. The edge services cluster with the vSRX HA appliance or a suitable bring your own gateway appliance is an alternative to the physical fortigate appliance. The edge services cluster uses local storage only (no vSAN, no NFS) with each host running a virtual appliance node. The edge services cluster requires only two ESXi hosts. Affinity rules are used to pin each virtual appliance node to its associated host. The nodes are configured as a highly available cluster. The cluster is sized to support 10 Gb networking.

## Workload cluster
{: #fss-overview-workloads}

The sizing of four ESXi servers for the workload cluster is based on the required compute, memory, and storage needs of the customer applications. The minimum host count of four is a vSAN requirement.
...
Add more workload clusters to deliver dedicated resources for additional applications or to satisfy business or compliance requirements.

## Optional management by IBM
{: #fss-overview-ibm-services}

While the IBM Cloud for VMware Regulated Workloads is available as a self-managed or independent services vendor-managed solution, a customer might choose to deploy the IBM Cloud for VMware Regulated Workloads as an IBM-Managed offering by bundling in IBM GTS services.

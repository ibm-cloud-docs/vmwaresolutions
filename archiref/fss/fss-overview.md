...
vSAN is the storage provider. vSAN is fully contained within the workload environment thus all the regulated data is isolated from external access and threats.
...
While the IBM Cloud for VMware Regulated Workloads is available as a self-managed or SaaS provider-managed solution, a SaaS provider might choose to deploy the IBM Cloud for VMware Regulated Workloads as an IBM-managed offering by bundling in IBM GTS services.

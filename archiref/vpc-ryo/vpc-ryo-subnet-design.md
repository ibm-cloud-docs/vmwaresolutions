...
[VMware vSphere vMotion](https://www.vmware.com/products/vsphere/vmotion.html){: external} refers to live migration of workloads from one ESXi host to another. If the VM needs to support vMotion, you must set the VLAN interface to `floatable` when provisioning it in VPC.

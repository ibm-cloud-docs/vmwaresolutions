...
FortiGate–VM does not replace NSX–T™ but rather complements and enhances the existing VMware NSX® architecture when installed into your VMware vCenter Server® (VCS) instance on {{site.data.keyword.cloud}}. FortiGate–VM is deployed as a pair of virtual appliances with one network interface (port1) configured for management access and nine available network interfaces in the client’s data plane. With appropriate configuration of networks and routes, you can use FortiGate–VM to provide network security between all tiers of your network topology.
...
You can also deploy FortiGate–VM as a virtual firewall appliance directly within your management and workload networks. With this approach, your FortiGate–VM appliances can perform the following functions:

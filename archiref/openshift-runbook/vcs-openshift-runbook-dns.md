...
1. Create a table, based on the following example, to record your values.
...
- **SubDomain** - Sub Domain of the {{site.data.keyword.vmwaresolutions_short}} deployment, for example, dallas
...
The following table is for an example deployment. Your values will be different.

| DNS Description | DNS Example Name | DNS Example IP address |
| --- | --- | --- |
| DNS Reverse Lookup for OpenShift VXLAN  | 192.168.133.0/24 | |
| DNS Reverse Lookup for OpenShift IBM Cloud Subnet  | 10.208.242.128/26 | |
...
| compute-0 Host | compute-0.ocp.dallas.ibm.local | 192.168.133.20 |
| compute-1 Host | compute-1.ocp.dallas.ibm.local | 192.168.133.21 |
| compute-2 Host | compute-2.ocp.dallas.ibm.local | 192.168.133.22 |
...
Add-DnsServerResourceRecordA -Name "compute-0.ocp.dallas" -ZoneName "ibm.local" -AllowUpdateAny -IPv4Address "192.168.133.20" -CreatePtr -TimeToLive 00:00:10
Add-DnsServerResourceRecordA -Name "compute-1.ocp.dallas" -ZoneName "ibm.local" -AllowUpdateAny -IPv4Address "192.168.133.21" -CreatePtr -TimeToLive 00:00:10
Add-DnsServerResourceRecordA -Name "compute-2.ocp.dallas" -ZoneName "ibm.local" -AllowUpdateAny -IPv4Address "192.168.133.22" -CreatePtr -TimeToLive 00:00:10
...
## Related links
{: #openshift-runbook-runbook-dns-related}

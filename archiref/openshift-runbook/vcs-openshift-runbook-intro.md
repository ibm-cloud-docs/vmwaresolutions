...
* Build Process - This document. The process and steps that are needed to install Red Hat OpenShift 4.6 on to an existing vCenter Server instance.
...
* You can have a remote device, laptop/desktop, connected via the {{site.data.keyword.cloud_notm}} SSL VPN to the {{site.data.keyword.cloud_notm}} Private network. This remote device has access to the internet to download the required files and can connect to vCenter and the bastion node via the SSL VPN.
...
This documentation describes the process to install Red Hat OpenShift v4.6 on to an existing vCenter Server instance. The process installs and configures:
...
* Phase 2 - Red Hat OpenShift installation. These steps are described in [Red Hat OpenShift 4.6 user provider infrastructure installation](/docs/vmwaresolutions?topic=vmwaresolutions-openshift-runbook-runbook-install-intro).
...
* Phase 3 - Post deployment activities:
  * Configure a persistent volume for use by the OpenShift cluster. This step is described in [Red Hat OpenShift 4.6 additional configuration](/docs/vmwaresolutions?topic=vmwaresolutions-openshift-runbook-runbook-config-intro).

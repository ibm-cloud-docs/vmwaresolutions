...
Red Hat OpenShift on IBM Cloud for VMware Solutions is a reference architecture and a manual build process to deploy a Red Hat OpenShift Cluster 4.1 on to an existing vCenter Server instance. The Red Hat OpenShift Cluster on IBM Cloud components are deployed as virtual machines and appliances by using NSX software defined networking.
...
* Control-plane nodes - The control-plane nodes run services that are required to control the Kubernetes cluster and are the control plane as they contain more than just the Kubernetes services for managing cluster. The terms "master" and "control-plane" are used interchangeably.
...
* Load-balancer - An NSX ESG load-balancer service is used to front end the Red Open Shift APIs, both internal and external, and the Open Shift router. The load balancer is configured so that Port 6443 and 22623 point to the bootstrap and master nodes, while ports 80 and 443 are configured to point to the worker nodes.
...
* You can have a Microsoft Windows or Linux Virtual Server Instance (VSI) installed alongside your vCenter Server instance to provide administrative access to the environment. This has access to the internet to provide access for the remote connection to the server and to download files and private network access to access vCenter and the bastion node.
...
* govc -  govc is a vSphere CLI and pre-compiled for Linux, OSX, and Windows. It is useful to complete various vCenter/vSphere operations from the command line. It is used in this process to:
...
## Related links
{: #openshift-runbook-runbook-intro-related}

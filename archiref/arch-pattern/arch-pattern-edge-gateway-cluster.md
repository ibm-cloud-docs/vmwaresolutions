...
# Architecture pattern for using edge gateway cluster with NSX-T
{: #arch-pattern-edge-gateway-cluster}

When you deploy a VMware vCenter Server® instance in your {{site.data.keyword.cloud}} classic infrastructure, you might optionally deploy an edge gateway cluster. 

The edge gateway cluster might host, for example, a Juniper® vSRX, Fortinet® FortiGate® or other third-party router or firewall. You can control which VLANs are associated with the router or firewall that is running on it through the [gateway appliance](/docs/gateway-appliance?topic=gateway-appliance-about#firewall) configurations.

The edge gateway cluster must not be mixed with the NSX-T edge cluster, which consists of VMware NSX-T™ edge transport nodes. The edge gateway cluster provides compute capacity for hosting Juniper vSRX, Fortinet FortiGate, or other third-party routers or firewalls. The VLAN routing can be controlled by [gateway appliance configuration](/docs/gateway-appliance?topic=gateway-appliance-managing-vlans-and-gateway-appliances) though {{site.data.keyword.cloud_notm}} Classic portal.
{: important} 

## Deploying an edge gateway cluster with NSX-T
{: #arch-pattern-edge-gateway-cluster-overview}

The following diagram presents an overview for an architecture pattern for deploying an edge gateway cluster with NSX-T.

![Edge gateway cluster with NSX-T](../../images/arch-pattern-nsx-t-edge-services-cluster.svg "Edge gateway cluster with NSX-T"){: caption="Figure 1. Edge gateway cluster with NSX-T" caption-side="bottom"}

The following list is a summary of the architecture pattern deployment:

1. You can optionally deploy an edge gateway cluster to host Juniper vSRX or your own routing or firewall device. This cluster is attached to both {{site.data.keyword.cloud_notm}} private and public networks through transit networks. 
...
![Adding VLANs with edge gateway cluster](../../images/arch-pattern-nsx-t-edge-services-cluster-vlans.svg "Adding VLANs with edge gateway cluster."){: caption="Figure 2. Adding VLANs with edge gateway cluster" caption-side="bottom"}

1. In this example, you deployed an optional edge gateway cluster to host Juniper vSRX (or your own routing or firewall device). This cluster is attached to both {{site.data.keyword.cloud_notm}} private and public networks through untagged transit networks. These transit networks are used for static routes to route public and private traffic through the firewall that is hosted on the edge gateway cluster.
2. You can associate and route traffic through vSRX (or the third-party device) by assigning VLANs to the edge gateway cluster. You can choose the VLANs to be assigned and routed through it by using {{site.data.keyword.cloud_notm}} classic portal. You must manually configure the required VLAN interfaces and IP addresses to the vSRX or third-party devices, the related firewall zones, the rules, and the policies. Every interface that is associated and routed though VLAN is tagged with the specific VLAN ID in the edge gateway cluster's trunked-distributed port group. Therefore, VLAN tagged interfaces must also be used on the firewall appliance configuration.

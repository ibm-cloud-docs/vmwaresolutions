...
5. Remove the multiwriter flag from the primary cluster VM (the only one that owns the cluster disks).
...
11. Start the primary node and verify the operation.
...
* Run any external routing changes, which can include inserting changed routing for networks that were migrated, removing routes to the source site from the migrated network, and ensuring that routing to the migrated subnet across the WAN still works for the applications that were not migrated.
...
* Some applications are behind a network load balanced virtual IP (vIP). Those vIPs are on your owned subnet, which is located on a virtual F5 behind the Vyatta.
...
* Applications behind a load balancer that uses an internal routable IP can require routing changes if they cannot be moved together or it is not desirable to do so. For example, involving too many applications in one swing can be perceived as too much of a risk.
* VMware admins, network admins (including customers and WAN vendors), and application owners need to be involved, even if there is no planned impact on a particular system or network equipment.

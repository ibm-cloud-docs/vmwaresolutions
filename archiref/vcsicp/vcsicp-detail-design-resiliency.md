...
Backups for an {{site.data.keyword.cloud_notm}} Private (ICP) deployment are crucial to restore the system to its working state if failure occurs. In addition to a traditional VM backup, every ICP master node runs etcd. The etcd documentation clearly states not to use traditional VM backup to restore it.

For ICP at the platform level, you need to back up these components:
...
-	**Persistent storage** used for management services that use ICP or Kubernetes Persistent Volume and storage provider. You can use the backup or restore technology that is provided by the storage provider. A similar process can be extended into your user’s application as well.
...
####	ICP expansion
ICP worker VM nodes are scaled to expand the compute or application:

  - Customer provisions a new VM, in the same VXLAN that ICP is deployed.
  ...
  - Use CAM to automate the provisioning of the VM and running the commands to add to the ICP Cluster.

###  IKS expansion
Users can provision an IKS environment via the {{site.data.keyword.cloud_notm}} Portal to extend or use a container environment.

Application deployments into IKS can be done via:

- IKS connection or services are developed in CAM and published to ICP catalog.

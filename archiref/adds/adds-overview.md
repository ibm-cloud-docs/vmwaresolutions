...
This section describes typical domain models available to a customer when they designate their domains either on-premises or in {{site.data.keyword.cloud}}:
...
Creating a multiple domain model design involves identifying what domain is the forest root domain and determining the number of extra domains that are required to meet your replication requirements. These domains can be parent or child depending on how the domain structure needs to reflect the organizational structure. However, generally, it is better to have as few domains as possible to ease management and reduce complexity the enterprise needs to balance complexity and separating out resources into different domains and subdomains has security benefits.

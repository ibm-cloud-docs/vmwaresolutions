...
The following firewall rules are configured on the management ESG when the HTDC on {{site.data.keyword.cloud_notm}} service is deployed. This configuration allows outbound connectivity to the Internet for the HyTrust VMs so that the license that is applied to the HTDC installation does not expire after a year.
...
These rules also allow automatic vitals reporting, if you enable it, as HDKC must be able to send the encrypted vitals bundle outbound to `https://vitals.hytrust.com` by using TCP/443.
...
* KeyControl webGUI - A browser-based HTTPS interface to KeyControl to configure and manage KeyControl. You can log in to the webGUI on any KeyControl node in the cluster. Any changes that you make on that node are automatically disseminated to all other KeyControl nodes in the cluster.
* KeyControl System Console menu - The System Console menu is available on each KeyControl in the cluster. You can set local configuration options for the node, join the node to an existing KeyControl cluster, and manage support access to the node.
...
2. Wrapping DEK with KEK request flow - HTDC PA requests the HTKC server to create a new DEK when it encrypts a disk or rekeying a disk.
3. Unwrapping DEK with KEK request flow - HTDC PA requests the HTKC server for a DEK when it attaches an encrypted disk.

### Cloud VM Set KEK setup flow
{: #htdc-hpcs-detail-processes-kek}

1. In the IBM HPCS instance, an HPCS service user creates or imports a Root Key. This root key is used by HTDC to wrap the KEK of a Cloud VM Set. The Root Key is cannot be extracted or exported and it never leaves the IBM HPCS instance.
2. In HTDC, the HTDC Cloud administrator configures a KEK for a Cloud VM Set with a number of characteristics.
...
HyTrust DataControl Policy Agent (HTDC PA) requests the HTKC server to create a new Data Encryption Key (DEK) when it encrypts or rekeys a disk.

1. The HTDC PA requests the HTKC server to create a new Data Encryption Key (DEK).
2. HTKC validates the status of the KEK to ensure it is not expired or revoked. If the KEK is expired or revoked, HTKC might fail the DEK creation request.
...
HTDC PA requests the HTKC server for a DEK when it attaches an encrypted disk.
1. HTDC PA requests the HTKC server to create a new DEK.
2. HTKC validates the status of the KEK to ensure it is not expired or revoked. If the KEK is expired or revoked, HTKC might fail the key request.

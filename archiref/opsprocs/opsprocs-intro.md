...
2. Specialized documentation written for the enterprise. This documentation is specific to a system, an application or suite of applications and is not covered by vendors documentation. When you document your specialized documentation, we advise the following structure:
...
{{site.data.keyword.vmwaresolutions_full}} enables you to leverage you team's existing skills, toolsets and runbooks you have on-premise to manage your instances in {{site.data.keyword.cloud_notm}}. However, we have created the following general documentation to capture common procedures, guides. and tasks:
...
* Proactive daily checks - These checks enable system administrators to keep the environment healthy. When carried out on a daily basis, it should prevent many common issues related to capacity and performance from impacting your workloads.
...
Many of the tasks above are automated in Operations Management on {{site.data.keyword.cloud_notm}}, and for those tasks that are not this tooling makes the manual processes so much easier for your systems administrators. It is imperative that you have the core components of the VMware environment monitored, in Operations Management on {{site.data.keyword.cloud_notm}}, this is achieved as follows:

## Operations management on IBM Cloud
{: #opsprocs-intro-ops-mgmt}

You may have enterprise tooling in place that you can leverage to monitor and manage your vCenter Server instance. Table 1 describes the core components of the vCenter Server environment, why they need to be monitored and how they are monitored using Operations Management on {{site.data.keyword.cloud_notm}}. For more information, see the reference architecture documentation.
...
| NSX | NSX provides the virtualized network components utilized by the compute VMs, any failures of the network can impact the applications running on these VMs. | vROps and the vROps Management Pack for VMware NSX provides visibility into the network topology. vRLI collects the log data from the NSX components such as Controllers, ESG and logical switches. vRealize Network Insight (vRNI) provides in-depth troubleshooting of network issues. |

In addition to monitoring, Operations Management on {{site.data.keyword.cloud_notm}}, provides assistance with configuration, compliance and many of the proactive tasks detailed in this documentation.

...
| Title | Description |
|---|---|
| ESXI Commands |For an overview of command-line interfaces in vSphere, ESXi Shell commands, and VMware vSphere Command-Line Interface (vCLI) commands, see [Getting Started with vSphere Command-Line
Interfaces](https://vdc-download.vmware.com/vmwb-repository/dcr-public/bc4fa31a-40ac-4aa9-a6a1-7171d1fff7f4/740990ee-4d65-4627-a9d4-0f046cb78aec/vsphere-esxi-vcenter-server-67-command-line-interface-getting-started-guide.pdf){:new_window}. |
...
| Initiate vSAN health test  |  If you suspect there is an issue with vSAN, you can initiate a health test to verify that the cluster components are working as expected. Running the VM creation test creates a VM on each host in the cluster and then deletes it. If these tasks are successful, then the cluster components are working as expected and the cluster is functional. Then network performance test is used to detect and diagnose connectivity issues, and to make sure the network bandwidth between the hosts is adequate. For more information, see [Proactive Tests
](https://docs.vmware.com/en/VMware-vSphere/6.7/com.vmware.vsphere.vsan-monitoring.doc/GUID-B88B5900-33A4-4821-9659-59861EF70FB8.html){:new_window}.  |
